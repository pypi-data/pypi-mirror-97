...
class BaiduSpider(BaseSpider):
    ...
    def search_web(self, query: str, pn: int = 1, exclude: list = [], time: tuple = None) -> dict:
        """百度网页搜索.
        ...
        if type(time) == str:
            ...
            from_ = datetime.datetime(to.year, to.month, to.day, to.hour, to.minute, to.second, to.microsecond)
            if time == 'day':
        ...
        if type(to) == datetime.datetime and type(from_) == datetime.datetime:
            ...
            from_ = int(time_lib.mktime(time_lib.strptime(from_.strftime(FORMAT), FORMAT)))
        try:
