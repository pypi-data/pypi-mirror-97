...
class Message(object):
    ...
    * :attr:`prepath`, :attr:`postpath`: Not sure, will probably go away when
    ...
    def __init__(self, *, mtype=None, mid=None, code=None, payload=b'', token=None, uri=None, **kwargs):
        self.version = 1
        if mtype is None:
    ...
    def decode(cls, rawdata, remote=None):
        ...
        msg.token = int.from_bytes(rawdata[4:4 + token_length], 'big')
    ...
    def encode(self):
        ...
        token = self.token.to_bytes(8, byteorder='big') if self.token else b''
        rawdata = bytes([(self.version << 6) + ((self.mtype & 0x03) << 4) + (len(token) & 0x0F)])
