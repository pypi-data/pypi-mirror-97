...
class UIWindow(tk.Tk):
    ...
    def __init__(self, radio_dir=None, settings_path=None):
        ...
        self.gbutton_font = tkfont.Font(family='Helvetia', size=9, weight='bold', slant='roman')
        self.gtext_font = tkfont.Font(family='Helvetia', size=8, weight='bold', slant='roman')

        # main containers create
        self.top_frame = tk.Frame(master=self, bg='azure3', width=620, height=50, pady=3)
        self.center = tk.Frame(master=self, bg='white', width=620, height=360, padx=3, pady=3)
        # self.info = tk.Frame(master=self, bg='azure3', width=50, height=2, padx=1, pady=1)
        self.btm_frame = tk.Frame(master=self, bg='azure3', width=620, height=50, pady=3)
        ...
        self.start_page = tk.Canvas(self.center, bg='cornflowerblue', width=570, height=360, bd=0)
        self.ctr_mid = tk.Frame(self.center, bg='azure3', width=570, height=360, padx=3, pady=3)
        self.ctr_right = tk.Frame(self.center, bg='green', width=25, height=360, padx=3, pady=3)
        # center grid
        self.ctr_left.grid(row=0, column=0, sticky="ns")
        self.start_page.grid(row=0, column=1, sticky="nsew")
        ...
        self.btn_editor = tk.Button(master=self.btm_frame, text="Radio Editor", font=self.gbutton_font,
                                    width=10,  padx=2, pady=1, command=UItextEditor)
        self.btn_stop = tk.Button(master=self.btm_frame, text="Stop", width=10, padx=2, pady=1, font=self.gbutton_font,
                                  command=self.exit_app)
        self.btn_rec = tk.Button(master=self.btm_frame, text="Record", width=10,
                                 padx=2, pady=1, command=self.list_is_checked, font=self.gbutton_font)
        self.lbl_info = tk.Label(master=self.btm_frame, bg='azure3', anchor="s", font=self.gtext_font)
        ...
        self.start_page.create_rectangle(0, 0, 250, 190, fill='cornflowerblue', width=0)
        self.start_page.create_line(0, 125, 2000, 125, fill='orange', width=10)
        self.start_page.bind("<Enter>", self.on_enter)
        self.start_page.bind("<Leave>", self.on_leave)
    ...
    def on_enter(self, event):
        self.start_page.create_line(0, 225, 2000, 225, fill='orange', width=10, tag="line")

    def on_leave(self, enter):
        self.start_page.delete("line")
    ...
    def exit_app(self):
        ...
        self.btn_rec = tk.Button(master=self.btm_frame, text="Record", width=10, command=self.list_is_checked,
                                 font=self.gbutton_font)
        self.btn_rec.grid(row=0, column=3, sticky='w')
    ...
    def load_settings(self, list_of_widgets, frame):
        ...
        self.print_data_dir_test_write()
        self.box_list = []
        self.search_list = []
        # remove start screen
        self.ctr_mid.grid()
        # self.ghetto_canvas.grid()
        self.start_page.grid_remove()
        tk.Label(frame, text='Record', underline=0, bg='azure3', font=self.gtext_font).grid(row=0, column=2, sticky="e")
        tk.Label(frame, text='Search multiple words (record if match): podcast concert elvis', underline=0,
                 bg='azure3', font=self.gtext_font).grid(row=0, column=1, sticky="n")
        tk.Label(frame, text='Search', bg='azure3', font=self.gtext_font).grid(row=0, column=0, sticky="w")
        tk.Label(frame, text='Radio', bg='azure3', font=self.gtext_font).grid(row=0, column=3, sticky="w")
        tk.Label(frame, text='', bg='azure3').grid(row=1, column=0, sticky="e")
        tk.Label(frame, text='', bg='azure3').grid(row=1, column=1, sticky="e")
        tk.Label(frame, text='', bg='azure3').grid(row=1, column=2, sticky="e")
        ...
        for idx, text in enumerate(list_of_widgets):
            ...
            tk.Checkbutton(frame, variable=self.box_list[-1], command=self.cb_checked,
                           bg='azure3', font=self.gtext_font).grid(row=idx + 2, column=2, sticky='e')

        # draw the scrollbar
        ghetto_scroll = tk.Scrollbar(self.ctr_mid, orient=tk.VERTICAL)
        ghetto_scroll.config(command=self.ghetto_canvas.yview)
        self.ghetto_canvas.config(yscrollcommand=ghetto_scroll.set)
        ghetto_scroll.grid(row=0, column=5, sticky="ns")

        self.canvas_frame.bind("<Configure>", self.update_scrollregion)

    def list_is_checked(self):
        ...
        #  collect parameters for title pull and file suffix, json, html, stream
        self.coll_srv_params()

        for key in term.GIni.ini_keys:  # start recording and title pull threads
            term.GBase.pool.submit(self.start_records, key)

        term.GBase.pool.submit(self.display_title)  # display titles on window
        self.start_timer()
    ...
    def coll_srv_params():
        ...
        with ThreadPoolExecutor() as executor:
            futures = {executor.submit(term.GNet.test_stream_server, key) for key in term.GIni.ini_keys}
    ...
    def add_server_to_data_base_ui(str_key, str_val):
        # def is for extracting play list server names from url, add to GIni.ini_keys dict like the other server
        is_playlist_server = ''
...
class UIUtils(UIWindow):
    ...
    info_ger = [
        ...
        'Zuerst im linken Kasten den Haken setzen. "Search"',
        'Worte wie: Elvis Anthrax Heino Beton Kammermusik.',
        'Es ist egal, ob das Wort GROSS oder klein geschrieben ist.',
        'Das Radio spielt den Titel mit dem Wort, Aufnahme startet.',
        'Bei Titelwechseln kann noch ein Teil des folgenden',
        'Titels aufgenommen werden. Einfach löschen.',
        '"Timer" oben rechts, keine Zeit Stop zu drücken?',
