...
def get_simbert_vectors(titles='bert向量化', max_workers=1):
    if isinstance(titles, str):
        titles = [titles]

    titles_list = np.array_split(titles, max_workers)  # list
    request_func = lambda titles: request(f"{zk_cfg.simbert_url}", json={"texts": list(titles)}).get('vectors')

    vectors_list = titles_list | xThreadPoolExecutor(request_func, max_workers) | xlist
...
if __name__ == '__main__':
    ...
    print(get_simbert_vectors(['bert向量化'] * 10, max_workers=5))
