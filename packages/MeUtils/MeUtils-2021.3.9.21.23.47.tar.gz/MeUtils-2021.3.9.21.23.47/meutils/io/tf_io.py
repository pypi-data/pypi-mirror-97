...
def df2write(df, file, num_partitions=1, overwrite=True, index=False, header=False, **kwargs):
    """仅支持单文件，支持多线程写入
    """
    file = _process_hdfs_path(file)
    dir, name = Path(file).parent, Path(file).name
    if_not_exist_makdir(str(dir))  # Expected binary or unicode string, HDFS上不支持pathlib.PosixPath，避免抛错

    if overwrite: rm(file)

    if num_partitions == 1:
        with tf.io.gfile.GFile(file, 'w') as f:
            df.to_csv(f, index=index, header=header, **kwargs)
    else:
        ...
        def writer(args):
            ...
            with tf.io.gfile.GFile(file, 'w') as f:
                df.to_csv(f, index=index, header=False, **kwargs)
