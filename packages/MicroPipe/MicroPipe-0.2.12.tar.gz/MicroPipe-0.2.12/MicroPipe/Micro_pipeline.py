...
class Draw_Graph(WorkflowRunner):
    ...
    def workflow(self):
        ...
        if end_list:
            return ""
        self.addTask("Draw",
                     command="nohup " + scripts_path + "/Circular_Graph_Draw.py -g %s -o %s -a %s >run.log 2>&1" % (
                     self.Gbk, self.Output, self.Anno)
                     )
...
class AlignmentRun(WorkflowRunner):
    ...
    def workflow(self):
        ...
        self.addTask("Annotation",
                     command="nohup AnnotationPipe2.py -p %s -n %s -e  %s   -o %s >run.log 2>&1" % (
                     self.Protein, self.Nuleotide, self.Eval, self.Output)
...
class Gene_Prediction(WorkflowRunner):
    ...
    def workflow(self):
        ...
        self.addTask("Database",
                     ...
                     "create_database.py " + " -d " + self.TotalDatabase + " -g " + self.Gff + " -n " + self.ffn + " -p " + self.Protein + ' -f %s' % (
                         self.Genome),
...
class Data_prapare(WorkflowRunner):
    ...
    def workflow(self):
        ...
        split_flow = Split_Sequence(self.Contig, self.splited_path, config_hash["Threshold"]["genomesize"],
                                    config_hash["Taxon"]["strain"])
...
class Total_AnnoAll(WorkflowRunner):
    ...
    def workflow(self):
        ...
        self.addTask(
            ...
            "totalFinal_annotationTable",
            ' XLS_Combine.py -i %s -o %s -g %s' % (
                self.Output + '/Detail/',
                self.Output + '/Table/',
                self.TotalPep
            )
            , dependencies= [ "totalFinal_annotation"]
