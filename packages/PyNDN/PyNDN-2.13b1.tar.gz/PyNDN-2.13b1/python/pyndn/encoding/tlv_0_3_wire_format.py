...
class Tlv0_3WireFormat(WireFormat):
    ...
    def encodeName(self, name):
        ...
        if haveModule_pyndn:
            # Use the C bindings.
            result = _pyndn.Tlv0_3WireFormat_encodeName(name)
    ...
    def decodeName(self, name, input, copy = True):
        ...
        if haveModule_pyndn:
            # Use the C bindings.
            _pyndn.Tlv0_3WireFormat_decodeName(name, input)
            return
    ...
    def encodeInterest(self, interest):
        ...
        if haveModule_pyndn:
            # Use the C bindings.
            result = _pyndn.Tlv0_3WireFormat_encodeInterest(interest)
    ...
    def decodeInterest(self, interest, input, copy = True):
        ...
        if haveModule_pyndn:
            # Use the C bindings.
            return _pyndn.Tlv0_3WireFormat_decodeInterest(interest, input)

        try:
            return Tlv0_3WireFormat._decodeInterestV03(interest, input, copy)
        except Exception as exceptionV03:
            try:
                # Failed to decode as format v0.3. Try to decode as v0.2.
                return self._decodeInterestV02(interest, input, copy)
            except:
                # Ignore the exception decoding as format v0.2 and raise the
                # exception from trying to decode as format as format v0.3.
                raise exceptionV03
    ...
    def encodeData(self, data):
        ...
        if haveModule_pyndn:
            # Use the C bindings.
            result = _pyndn.Tlv0_3WireFormat_encodeData(data)
            return (Blob(result[0], False), result[1], result[2])
    ...
    def decodeData(self, data, input, copy = True):
        ...
        if haveModule_pyndn:
            # Use the C bindings.
            return _pyndn.Tlv0_3WireFormat_decodeData(data, input)
    ...
    def encodeSignatureInfo(self, signature):
        ...
        if haveModule_pyndn:
            # Use the C bindings.
            result = _pyndn.Tlv0_3WireFormat_encodeSignatureInfo(signature)
            return Blob(result, False)
    ...
    def decodeSignatureInfoAndValue(self, signatureInfo, signatureValue, copy = True):
        ...
        if haveModule_pyndn:
            # Use the C bindings.
            return _pyndn.Tlv0_3WireFormat_decodeSignatureInfoAndValue(
    ...
    def encodeSignatureValue(self, signature):
        ...
        if haveModule_pyndn:
            # Use the C bindings.
            result = _pyndn.Tlv0_3WireFormat_encodeSignatureValue(signature)
            return Blob(result, False)
    ...
    def _encodeSelectors(interest, encoder):
        ...
        if interest.getExclude().size() > 0:
            Tlv0_3WireFormat._encodeExclude(interest.getExclude(), encoder)
    ...
    def _decodeSelectors(interest, decoder, copy):
        ...
        if decoder.peekType(Tlv.Exclude, endOffset):
            Tlv0_3WireFormat._decodeExclude(interest.getExclude(), decoder, copy)
        else:
            interest.getExclude().clear()
    ...
    def _encodeExclude(exclude, encoder):
        ...
        for i in range(len(exclude) - 1, -1, -1):
            ...
            if entry.getType() == Exclude.COMPONENT:
                Tlv0_3WireFormat._encodeNameComponent(entry.getComponent(), encoder)
