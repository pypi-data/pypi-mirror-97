...
def extract_tar(links, req_input):
    path = os.getcwd() + "/" + links[0].split("/")[-1]
    print(path)
    download_url(url=links[0], save_path=path,name="GeckoDriver")
    if req_input:
...
def download_url(url, save_path, chunk_size=128,name=""):
    logging.info(str(name)+"...(Required to use WebProcessor)", url)
    r = requests.get(url, stream=True)
    with open(save_path, 'wb') as fd:
        for chunk in r.iter_content(chunk_size=chunk_size):
            fd.write(chunk)
    logging.info("Finished Installing "+str(name))
