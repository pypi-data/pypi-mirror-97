...
def main():
    ...
    def download():
        ...
        try:
            ...
            root.update()
            if platform == "win32":
                YouTube(link.get()).streams.first().download('C:\//Users//'+username+'//Downloads//YTDownl')
            else:
                YouTube(link.get()).streams.first().download(os.path.expanduser("~")+"/Downloads/YTDownl")
