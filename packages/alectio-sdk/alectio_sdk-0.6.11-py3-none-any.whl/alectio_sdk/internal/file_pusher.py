...
import alectio_helpers
import alectio_helpers.util
from alectio_helpers.compat import tempfile

from alectio_helpers.filesync import stats
from alectio_helpers.filesync import step_checksum
from alectio_helpers.filesync import step_upload
...
warnings.filterwarnings(
    "ignore", "Implicitly cleaning up", RuntimeWarning, "alectio_helpers.compat.tempfile"
)
...
# uploading it.
TMP_DIR = tempfile.TemporaryDirectory("alectio_helpers")
...
class FilePusher(object):
    ...
    def __init__(self, api):
        ...
        self._tempdir = tempfile.TemporaryDirectory("alectio_helpers")
    ...
    def print_status(self, prefix=True):
        ...
        while True:
            ...
            step += 1
            alectio_helpers.termlog(line, newline=False, prefix=prefix)
            if stop:
                break
            time.sleep(0.25)
        dedupe_fraction = (
        ...
        if dedupe_fraction > 0.01:
            alectio_helpers.termlog(
                "alectio sync reduced upload amount by %.1f%%             "
                % (dedupe_fraction * 100),
                prefix=prefix,
            )
        # clear progress line.
        alectio_helpers.termlog(" " * 79, prefix=prefix)
    ...
    def file_changed(
        ...
        """
        # Tests in linux were failing because alectio_helpers-events.jsonl didn't exist
        if not os.path.exists(path) or not os.path.isfile(path):
            return
        if os.path.getsize(path) == 0:
            return

        save_name = alectio_helpers.util.to_forward_slash_path(save_name)
