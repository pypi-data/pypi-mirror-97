...
class Package(object):
    ...
    def load(self, filename):
        ...
        if self.type not in ['solution', 'common', 'board', 'chip', 'arch', 'peripheral', 'kernel', 'sdk']:
            put_string(
                '%s: `type` field must be "solution" or "common" or "board" or "chip" or "arch" or "peripheral" or "kernel" or "sdk".' % filename)
            exit(-1)
