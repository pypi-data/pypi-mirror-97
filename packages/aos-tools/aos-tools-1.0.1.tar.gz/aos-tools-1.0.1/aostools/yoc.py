...
class YoC:
    def __init__(self):
        ...
        package_yamls = self.conf.search_pacakge_yaml(
            [os.path.join('hardware', 'board'), os.path.join('hardware', 'chip'), os.path.join('hardware', 'arch'),
            os.path.join('hardware', 'peripherals'),'kernel', 'components', 'examples', 'test'], ['solutions'])
    ...
    def check_conditions(self, components=[]):
            ...
            for c in comp.depends_on:
                if c in cmpts:
                    for cond1 in comp.conditions:
                        if cond1 not in c.conditions:
                            c.conditions.append(cond1)
        # 添加所有依赖的组件的条件
        for comp in cmpts:
            depends, _ = self.components.get_depend(comp)
            for dep in depends:
        ...
        for comp in components:
            if comp not in cmpts and (len(comp.conditions) == 0):
                depends, _ = self.components.get_depend(comp, cond_break=True)
                for dep in depends:
                    # print("ck3##-%s,%s" % (comp.name, dep.name))
                    dep.conditions = []

    def getSolution(self, board_name=None, compiler='gcc'):
        for component in self.components:
            if component.path == os.getcwd():
                ...
                self.check_conditions(components)
