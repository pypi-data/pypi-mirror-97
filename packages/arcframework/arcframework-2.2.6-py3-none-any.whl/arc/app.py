...
class App:
    ...
    async def handle_request(self, request):
        try:
            handler, kwargs, methods = self.find_handler(request_path=request.url.path)
            
            assert request.method.lower() in methods, f"Method {request.method.lower()} not allowed"
