...
class App:
    ...
    async def handle_request(self, request, websocket):
        try:
            handler, kwargs, methods, type = self.find_handler(
                ...
                if handler is not None:
                    ...
                    if inspect.iscoroutinefunction(handler):
                        ...
                        if type == "route":
                            ...
                            response = handler(websocket, **kwargs)
                else:
    ...
    def find_handler(self, request_path):
        for path, items in self.routes.items():
            ...
            if parse_result is not None:
                return handler, parse_result.named, methods, type

        return None, None, methods, None
    ...
    def add_websocket(self, path, handler):
        ...
        self.routes[path] = {"handler": handler, "type": "websocket"}
