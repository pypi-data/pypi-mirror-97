...
def get_paidclaims_psql(groupnums, startdate, enddate, conn=None, renamecols=True, convertdtypes=True):
    import pandas as pd
    ...
    conn | connection object | connection object to use for SQL query, if not specified it will attempt to use the defualt
                    (
    ...
    """
    #gets a defualt connection to psql if none provided
    if conn == None:
        conn = get_connection()
...
def get_patients_psql(groupnums, conn=None):
    import pandas as pd
    """Establishes a connection to psql database (REQUIRES conda ENVIROMENT VARIABLE for host and pwd), 
