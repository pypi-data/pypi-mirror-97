...
class ForecastMAPE(ForecastingMetric, NonScalarMetric):
    ...
    def aggregate(
        ...
        """
        if not Metric.check_aggregate_scores(scores, constants.FORECASTING_MAPE, logger=logger):
...
class ForecastResiduals(ForecastingMetric, NonScalarMetric):
    ...
    def aggregate(
        ...
        :return: Aggregated score.
        """
        if not Metric.check_aggregate_scores(scores, constants.FORECASTING_RESIDUALS, logger=logger):
            return NonScalarMetric.get_error_metric()
