...
class Spearman(RegressionMetric, ScalarMetric):
    ...
    def compute(self):
        ...
        if np.unique(self._y_test).shape[0] == 1:
            self._logger.warning("Skipping Spearman calculation. All targets are equal.")
            return worst_spearman

        if np.unique(self._y_pred).shape[0] == 1:
            self._logger.warning("Skipping Spearman calculation. All predictions are equal.")
            return worst_spearman
    ...
    def compute(self):
        ...
        if self._y_min == self._y_max:
            ...
            if np.array_equal(self._y_test, self._y_pred):
                self._logger.warning(warning_format.format(name, 'best'))
                return constants.REGRESSION_RANGES[name][0]
            else:
                self._logger.warning(warning_format.format(name, 'worst'))
...
class RMSLE(RegressionMetric, ScalarMetric):
    ...
    def compute(self):
        ...
        if self._has_negatives:
            self._logger.warning("Skipping {} calculation. y_test/y_pred contain negative values.".format(name))
            return np.nan
...
class Residuals(RegressionMetric, NonScalarMetric):
    ...
    def aggregate(
        ...
        """
        if not Metric.check_aggregate_scores(scores, constants.RESIDUALS, logger=logger):
...
class PredictedTrue(RegressionMetric, NonScalarMetric):
    ...
    def aggregate(
        ...
        :return: Aggregated score.
        """
        if not Metric.check_aggregate_scores(scores, constants.PREDICTED_TRUE, logger=logger):
            return NonScalarMetric.get_error_metric()
...
def _mape(y_true: np.ndarray,
    ...
    if len(y_true_safe) < 1:
        if logger is not None:
            logger.warning("Skipping MAPE calculation. Not enough non-zero, non-nan values.")
...
def _smape(y_true: np.ndarray,
    ...
    if len(y_true_safe) < 1:
        if logger is not None:
            logger.warning("Skipping SMAPE calculation. Not enough non-zero, non-nan values.")
        return cast(np.float64, np.nan)
