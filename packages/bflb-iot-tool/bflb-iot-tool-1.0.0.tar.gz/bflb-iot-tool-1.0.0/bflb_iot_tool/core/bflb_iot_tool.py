...
class BflbIotTool(object):
    ...
    def bl60x_image_gen_cfg(self,
        ...
        if cfg_ini in [None, '']:
            if chiptype == "bl60x":
                f_org = os.path.join(app_path, chipname, "img_create/img_create_cfg_dp.conf")
            else:
                f_org = os.path.join(app_path, chipname, "img_create/img_create_cfg.conf")
            f = os.path.join(app_path, chipname, "img_create/img_create_cfg.ini")
        ...
        if bintype == "fw":
            if cpu_type is None:
                bootinfo_file = os.path.join(app_path, chipname, "img_create/bootinfo.bin")
                img_file = os.path.join(app_path, chipname, "img_create/img.bin")
            else:
                bootinfo_file = os.path.join(app_path, chipname,
                                             "img_create/bootinfo_{0}.bin".format(cpu_type.lower()))
                img_file = os.path.join(app_path, chipname,
                                        "img_create/img_{0}.bin".format(cpu_type.lower()))
        else:
            bootinfo_file = os.path.join(app_path, chipname,
                                         "img_create/bootinfo_{0}.bin".format(bintype))
            img_file = os.path.join(app_path, chipname, "img_create/img_{0}.bin".format(bintype))
    ...
    def bl60x_mfg_ota_bin_gen(self, chipname="bl60x", chiptype="bl60x", cpu_type=None):
        ...
        if cpu_type is None:
            ...
            bootinfo_fw_path = os.path.join(app_path, chipname,
                                            "img_create/bootinfo_" + cpu_type.lower() + ".bin")
        ...
        if cpu_type is None:
            ...
            img_fw_path = os.path.join(app_path, chipname,
                                       "img_create/img_" + cpu_type.lower() + ".bin")
    ...
    def bl_write_flash_img(self, d_addrs, d_files, flash_size):
        ...
        whole_img_file = os.path.join(app_path, self.chipname, "img_create/whole_flash_data.bin")
    ...
    def bl60x_mfg_flasher_eflash_loader_cfg(self,
        ...
        if boot2 is True:
            if bind_bootinfo is True:
                whole_img = chipnamedir + "/img_create/whole_img_boot2.bin"
                whole_img_len = 0x2000 + os.path.getsize(
                    chipnamedir + "/img_create/img_boot2.bin")
                whole_img_data = bl_create_flash_default_data(whole_img_len)
                filedata = bl_get_file_data([chipnamedir + "/img_create/bootinfo_boot2.bin"])[0]
                whole_img_data[0:len(filedata)] = filedata
                filedata = bl_get_file_data([chipnamedir + "/img_create/img_boot2.bin"])[0]
                whole_img_data[0x2000:0x2000 + len(filedata)] = filedata
                fp = open(whole_img, 'wb+')
                fp.write(whole_img_data)
                fp.close()
                d_files.append(chipnamedir + "/img_create/whole_img_boot2.bin")
                d_addrs.append("00000000")
            else:
                d_files.append(chipnamedir + "/img_create/bootinfo_boot2.bin")
                d_addrs.append("00000000")
                d_files.append(chipnamedir + "/img_create/img_boot2.bin")
                d_addrs.append("00002000")
        elif chiptype == "bl702":
            bflb_utils.printf("========= copy bootinfo_boot2.bin =========")
            bflb_utils.copyfile(chipnamedir + "/img_create/bootinfo.bin",
                                chipnamedir + "/img_create/bootinfo_boot2.bin")
            d_files.append(chipnamedir + "/img_create/bootinfo_boot2.bin")
            d_addrs.append("00000000")
        ...
        if bin_file is True and 'fw_cpu0_addr' in pt_parcel:
            if bind_bootinfo is True:
                whole_img = chipnamedir + "/img_create/whole_img_cpu0.bin"
                whole_img_len = 0x1000 + os.path.getsize(
                    chipnamedir + "/img_create/img_cpu0.bin")
                whole_img_data = bl_create_flash_default_data(whole_img_len)
                filedata = bl_get_file_data([chipnamedir + "/img_create/bootinfo_cpu0.bin"])[0]
                whole_img_data[0:len(filedata)] = filedata
                filedata = bl_get_file_data([chipnamedir + "/img_create/img_cpu0.bin"])[0]
                whole_img_data[0x1000:0x1000 + len(filedata)] = filedata
                fp = open(whole_img, 'wb+')
                fp.write(whole_img_data)
                fp.close()
                d_files.append(chipnamedir + "/img_create/whole_img_cpu0.bin")
                d_addrs.append(hex(pt_parcel['fw_cpu0_addr'])[2:])
            else:
                d_files.append(chipnamedir + "/img_create/bootinfo_cpu0.bin")
                d_addrs.append(hex(pt_parcel['fw_cpu0_addr'])[2:])
                d_files.append(chipnamedir + "/img_create/img_cpu0.bin")
                d_addrs.append(hex(pt_parcel['fw_cpu0_addr'] + 0x1000)[2:])
    
        if bin_file is True and 'fw_addr' in pt_parcel:
            if bind_bootinfo is True:
                whole_img = chipnamedir + "/img_create/whole_img.bin"
                whole_img_len = 0x1000 + os.path.getsize(
                    chipnamedir + "/img_create/img.bin")
                whole_img_data = bl_create_flash_default_data(whole_img_len)
                filedata = bl_get_file_data([chipnamedir + "/img_create/bootinfo.bin"])[0]
                whole_img_data[0:len(filedata)] = filedata
                filedata = bl_get_file_data([chipnamedir + "/img_create/img.bin"])[0]
                whole_img_data[0x1000:0x1000 + len(filedata)] = filedata
                fp = open(whole_img, 'wb+')
                fp.write(whole_img_data)
                fp.close()
                d_files.append(chipnamedir + "/img_create/whole_img.bin")
                d_addrs.append(hex(pt_parcel['fw_addr'])[2:])
            else:
                d_files.append(chipnamedir + "/img_create/bootinfo.bin")
                d_addrs.append(hex(pt_parcel['fw_addr'])[2:])
                d_files.append(chipnamedir + "/img_create/img.bin")
        ...
        if media is True and pt_parcel['media_addr'] is not None:
            d_files.append(chipnamedir + "/img_create/media.bin")
            d_addrs.append(hex(pt_parcel['media_addr'])[2:])
    
        if mfg is True:
            if bind_bootinfo is True:
                whole_img = chipnamedir + "/img_create/whole_img_mfg.bin"
                whole_img_len = 0x1000 + os.path.getsize(
                    chipnamedir + "/img_create/img_mfg.bin")
                whole_img_data = bl_create_flash_default_data(whole_img_len)
                filedata = bl_get_file_data([chipnamedir + "/img_create/bootinfo_mfg.bin"])[0]
                whole_img_data[0:len(filedata)] = filedata
                filedata = bl_get_file_data([chipnamedir + "/img_create/img_mfg.bin"])[0]
                whole_img_data[0x1000:0x1000 + len(filedata)] = filedata
                fp = open(whole_img, 'wb+')
                fp.write(whole_img_data)
                fp.close()
                d_files.append(chipnamedir + "/img_create/whole_img_mfg.bin")
                d_addrs.append(hex(pt_parcel['mfg_addr'])[2:])
            else:
                d_files.append(chipnamedir + "/img_create/bootinfo_mfg.bin")
                d_addrs.append(hex(pt_parcel['mfg_addr'])[2:])
                d_files.append(chipnamedir + "/img_create/img_mfg.bin")
        ...
        if len(d_files) > 0 and len(d_addrs) > 0:
            ...
            ret = img_create.compress_dir(chipname, "img_create", self.efuse_load_en)
            if ret is not True:
                return bflb_utils.errorcode_msg()
            return True
        else:
    ...
    def flasher_download_cfg_ini_gen(self, chipname, chiptype, cpu_type, config):
        ...
        if config["check_box"]['boot2_download'] is True:
            ...
            if boot2 is not None and len(boot2) > 0:
                ...
                f_org = os.path.join(app_path, chipname, "img_create/img_create_cfg_boot2.conf")
                f = os.path.join(app_path, chipname, "img_create/img_create_cfg_boot2.ini")
                if os.path.isfile(f) is False:
                    shutil.copy(f_org, f)
        ...
        if config["check_box"]['media_download'] is True:
            ...
            if media_bin is not None and len(media_bin) > 0:
                try:
                    shutil.copy(media_bin, os.path.join(app_path, chipname, "img_create/media.bin"))
                except Exception as e:
                    bflb_utils.printf(e)
            media_bin_d = True
        ...
        if config["check_box"]['use_romfs'] is True:
            ...
            if romfs_dir is not None and len(romfs_dir) > 0:
                ret = generate_romfs_img(romfs_dir,
                                         os.path.join(app_path, chipname, "img_create/media.bin"))
                if ret != 0:
        ...
        if config["check_box"]['mfg_download'] is True:
            ...
            if mfg is not None and len(mfg) > 0:
                ...
                f_org = os.path.join(app_path, chipname, "img_create/img_create_cfg_mfg.conf")
                f = os.path.join(app_path, chipname, "img_create/img_create_cfg_mfg.ini")
                if os.path.isfile(f) is False:
                    shutil.copy(f_org, f)
    ...
    def flasher_download_thread(self, chipname, chiptype, act, config, callback=None):
        ...
        cpu_type = None   
        ota_path = os.path.join(app_path, chipname, "ota")
        imgcreate2_path = os.path.join(app_path, chipname, "img_create2")
            ...
            # generation partition table/boot2/fw bin files and rf files
                ...
                if act == "download":
                    ...
                    if  config["check_box"]["encrypt"] is True or\
                        ...
                        if  config["check_box"]["encrypt"] is True or\
                            config["check_box"]["sign"] is True:
                            cfg_file = os.path.join(app_path, chipname, "img_create/img_create_cfg.ini")
...
def iot_download_cmd(args):
    ...
    config["check_box"]["ro_params_download"] = False
    ...
    config["check_box"]["download_single"] = False
