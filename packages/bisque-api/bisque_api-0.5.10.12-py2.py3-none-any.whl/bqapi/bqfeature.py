...
class ParallelFeature(Feature):
    ...
    def calculate_request_plan(self, l):
        ...
        if len(l)>self.MaxThread*self.MaxChunk:
            ...
            if old_div(len(l),float(self.MaxThread))>=self.MinChunk:
                return (self.MaxThread, ceil(old_div(self.MaxChunk,float(self.MaxThread))))
            else:
                t = ceil(old_div(len(l),float(self.MinChunk)))
                return (t, ceil(old_div(len(l),float(t))))
