...
from bmlx_components.utils.hdfs_utils import (
    JJA_CLUSTER,
    DP_CLUSTER,
    QW_CLUSTER
)
...
class PusherExecutor(Executor):
    ...
    def copy_to_hk_hdfs(self, sg_path, hk_path):
        try:
            ...
            logging.info(
                f"[Success] HDFS Copy from {sg_path} to {hk_path}"
            )
    ...
    def publish_embeddings(
        self, model_name, model_version, emb_bin_path, target, is_test, timeout_s
    ):
    ...
    def execute(
        ...
        # validate_ahead
        if len(input_dict["validation_samples"]) == 1:
            validation_samples_path = input_dict[
                "validation_samples"
            ][0].meta.uri
