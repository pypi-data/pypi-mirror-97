...
for new, old in _ALIASES:
    _DIST_LOOKUP[new] = _DIST_LOOKUP[old]
    ...
    def __init__(self, data: np.ndarray, *, family: str) -> None:
        ...
        Args:
            ...
            family: Parametric family to fit data to. Options are: `beta`,
                `chi2`, `exp`, `f`, `gamma`, `gaussian`, `lognormal`, t`.
                Many variants of these are also recognised, such as
                `exponential` instead of `exp`, `normal` instead of `gaussian`
                etc.
        """
        super().__init__(data)
