...
def test_shape(n_samples):
    ...
    assert (
        bootstrap(data, n_samples).replications.shape == (n_samples,) + shape
    )
    assert bootstrap(
        data, n_samples, statistic=safe_mean
    ).replications.shape == (n_samples,)
...
def test_returned_values_are_samples():
    n_samples = 5
    samples = bootstrap(TUPLES, n_samples).replications
    assert all(
        [
