...
class PreparedConsign():
    ...
    def prepare_data(self, nature, data, **kwargs):

        if nature is "csv":
            delimiter = kwargs.get("delimiter", ",")
            self.prepare_csv(location, delimiter)
        
        elif nature is "json":
            self.prepare_json(location)


    def prepare_location(self, nature, location):

        if nature in ["csv", "json"]:
