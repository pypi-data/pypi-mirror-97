...
class dbot():
	...
	def status(self, info):
		if "$hour" in info["text"] or "$minute" in info["text"] or "$second" in info["text"]:
			if info["type"] in ["play", "watch", "stream", "listen"]:
				self._status=["time", info["type"], info["text"]]
			else:
				print("Статус бота\nУкажите play, watch, stream либо listen в type!")
		else:
			if info["type"] in ["play", "watch", "stream", "listen"]:
				self._status=["static", info["type"], info["text"]]
			else:
				print("Статус бота\nУкажите play, watch, stream либо listen в type!")
...
def _start(client, info):
	...
	async def on_ready():
		...
		if "ready" in info._codes["events"]:
			...
			print("Бот онлайн!")
		if info._status!=[]:
			info._status[2]=await replaces([info._status[2], info, "", ["none", "none", "", ""], [], True, []])
			if info._status[0]=="time":
				t=0
				while True:
					dt=str(timedelta(seconds=round(t))).split(':')
					stat=info._status[2].replace("$hour", str(dt[0])).replace("$minute", str(dt[1])).replace("$second", str(dt[2]))
					if info._status[1]=="play":
						await client.change_presence(activity=discord.Activity(type=discord.ActivityType.playing,name=stat))
					elif info._status[1]=="watch":
						await client.change_presence(activity=discord.Activity(type=discord.ActivityType.watching,name=stat))
					elif info._status[1]=="listen":
						await client.change_presence(activity=discord.Activity(type=discord.ActivityType.listening,name=stat))
					elif info._status[1]=="stream":
						await client.change_presence(activity=discord.Activity(type=discord.ActivityType.streaming,name=stat))
					t+=5
					await asyncio.sleep(5)
			else:
				if info._status[1]=="play":
					await client.change_presence(activity=discord.Activity(type=discord.ActivityType.playing,name=info._status[2]))
				elif info._status[1]=="watch":
					await client.change_presence(activity=discord.Activity(type=discord.ActivityType.watching,name=info._status[2]))
				elif info._status[1]=="listen":
					await client.change_presence(activity=discord.Activity(type=discord.ActivityType.listening,name=info._status[2]))
				elif info._status[1]=="stream":
					await client.change_presence(activity=discord.Activity(type=discord.ActivityType.streaming,name=info._status[2]))
		...
		if exe:
			for cmd in info._codes["commands"]:
				...
				for trig in info._codes["commands"][cmd]["name"]:
					ex=False
					trig = await replaces([info._codes["commands"][cmd]["prefix"]+trig, info, "", [message, cmd, "commands", ""], [], True, []])
		...
		if any(line.startswith(func) and remember(func) for func in functions["functions"]):
			if functions["functions"][function]=="arg":
				if line.endswith("]"):
					line=line[:-1]
			try:
				line = await replaces([line, info, content.replace("[","⦍").replace("]","⦎"), need, split, allow, collector])
				line=line.replace("/n","\n")
	...
	while "$getVar[" in reline:
		if nowloop==reline:
			...
			if 0 <= 1 < len(reline.split("$getVar[")[-1].split("]")[0].split(";")):
				...
				try:
					reline=reline.replace("$getVar["+reline.split("$getVar[")[-1].split("]")[0]+"]",str(info._db.get(reline.split("$getVar[")[-1].split("]")[0].split(";")[0],"globals",["id",str(need[0].author.id)])))
				except:
					reline=reline.replace("$getVar["+reline.split("$getVar[")[-1].split("]")[0]+"]","none")
