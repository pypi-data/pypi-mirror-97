		...
		if any(line.startswith(func) and remember(func) for func in functions["functions"]):
			if functions["functions"][function]=="arg":
				if line.endswith("]"):
					line=line[:-1]
			try:
				line = await replaces([line, info, content.replace("[","⦍").replace("]","⦎"), need, split, allow, collector, vars])
				line=line.replace("/n","\n")
			...
			reline = await replaces([reline, info, content.replace("[","⦍").replace("]","⦎"), need, split, allow, collector, vars])
			if binstr == "":
				binstr += reline
			else:
				binstr += "\n"+reline
