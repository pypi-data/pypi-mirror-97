...
def update_conffiles(directory):
    ...
    In deb-pkg-tools release 8.4 support for excludes was added: If an entry in
    the ``DEBIAN/conffiles`` starts with an exclamation mark (optionally
    followed by whitespace) that entry will be omitted from the final file.
    """
    conffiles = set()
    excludes = set()
        ...
        with open(conffiles_file) as handle:
            for line in handle:
                ...
                if filename.startswith("!"):
                    # Enable callers to opt-in to the general logic provided by
                    # update_conffiles() while excluding specific files located
                    # in the /etc directory of the package being built.
    ...
    # (except for explicitly excluded files of course).
    for root, dirs, files in os.walk(os.path.join(directory, 'etc')):
        for filename in files:
            ...
            if os.path.isfile(pathname) and not os.path.islink(pathname):
                ...
                if relpath not in conffiles and relpath not in excludes:
                    logger.debug("Marking configuration file: %s", relpath)
                    conffiles.add(relpath)
    # Update DEBIAN/conffiles. Before deb-pkg-tools 8.4 the file would only be
    # written to when we had something to write, however with the introduction
    # of exclusion logic that is custom to deb-pkg-tools, we now need to make
    # sure we don't leave any excludes behind in DEBIAN/conffiles (because
    # dpkg-deb would not know what to do with such entries).
    if conffiles or excludes:
