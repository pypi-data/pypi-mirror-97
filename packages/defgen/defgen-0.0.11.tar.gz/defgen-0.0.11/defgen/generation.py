...
def generate_html(template_config):
    template_path = os.path.abspath(__file__).replace(__file__, '') + 'static/templates_auto/0/index.html'
    print(template_path)
...
def generate_css(template_config):
    template_path = os.path.abspath(__file__).replace(__file__, '') + 'static/templates_auto/0/main.css'
    with open(template_path, 'r') as fp:
        template_str = Template(fp.read())
        s = template_str.render(
            template_config=template_config
