...
class DiscordStub:
    ...
    CHANNELS = {2001: "channel-1", 2002: "channel-2"}
    CHANNELS_FORBIDDEN = [2100]
    USERS = {1001: "user-1", 1002: "user-2"}
    USERS_FORBIDDEN = [1100]

    async def fetch_channel(self, channel_id):
        if channel_id in self.CHANNELS:
            return DiscordChannel(id=channel_id, name=self.CHANNELS[channel_id])
        elif channel_id in self.CHANNELS_FORBIDDEN:
    ...
    async def fetch_user(self, user_id):
        if user_id in self.USERS:
            return DiscordUser(id=user_id, name=self.USERS[user_id])
        elif user_id in self.USERS_FORBIDDEN:
            raise Forbidden(response=Mock(), message="Test:Forbidden user")
        else:
