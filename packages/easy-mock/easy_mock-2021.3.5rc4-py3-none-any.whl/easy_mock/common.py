...
def gen_random_string(str_len, has_chinese=False):
    """ generate random string with specified length
    """

    char = CHINESE_CHARS if has_chinese else string.ascii_letters
    return "".join(random.choices(char, k=str_len))
...
def check_schema(schema, check_body):
    try:
        # Schema would rather
        validate(check_body, schema)
    except ValidationError as e:
        log().error({"path": e.path, "msg": e.message})
        raise e
