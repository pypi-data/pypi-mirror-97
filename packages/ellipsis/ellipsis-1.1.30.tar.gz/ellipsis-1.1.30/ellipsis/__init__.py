        ...
        trans = rasterio.transform.from_bounds(bounds['x1'], bounds['y1'], bounds['x2'], bounds['y2'], r_total.shape[1], r_total.shape[0])
                ...
                for tileX in x_tiles:
                    ...
                    while retries <= 10:
                        try:
                            ...
                            if token ==None:
                                r = s.get(url + '/tileService/' + mapId + '/' + str(timestamp) + '/data/' + str(zoom) + '/' + str(tileX) + '/' + str(tileY) ,
                                             json = body, timeout = 10 )
                            else:
                                r = s.get(url + '/tileService/' + mapId + '/' + str(timestamp) + '/data/' + str(zoom) + '/' + str(tileX) + '/' + str(tileY)  , headers = {"Authorization":token},
                                             json = body, timeout = 10 )
            ...
            if str(type(geometry)) != str(type(None)):
                ...
                if Type == 'imagery':
                    ...
                    raster_shape = np.expand_dims(raster_shape, axis = -1)
                    r_total = np.stack([r_total[:,:,0], raster_shape[:,:,0]], axis = -1)
