...
class CustomCallback(BaseCallback):
    ...
    def __init__(self, _env: SubprocVecEnv, _model: PPO, verbose=0, eval_th=32.5e4, n_eval_steps=15):
        ...
        self.path = './result/env_path_%s/algorithm_%s/total_%e_n_steps_%e_batch_size_%e_n_epochs_%e_env_num_%d_lr_%e_gama_%.3f/time_%s' % \
                    (self.model.env_path, self.model.__class__.__name__, self.model.total_steps, self.model.n_steps, self.model.batch_size,
    ...
    def _on_rollout_end(self) -> None:
        ...
        if self.mean_profits[-1] > self.eval_th:
            ...
            if self.eval_cnt >= self.n_eval_steps:
                ...
                eval_mean_false = sum(profits)/len(profits)
