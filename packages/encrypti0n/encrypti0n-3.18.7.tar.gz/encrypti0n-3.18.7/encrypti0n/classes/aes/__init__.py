...
class Database(object):
	...
	def load(self, 
		...
		if not os.path.exists(str(path)):
			if default == None:
				return r3sponse.error(f"Specified path [{path}] does not exist.")
			...
			except Exception as e: return r3sponse.error(f"Failed to load content {path}, {e}.")
			if format in ["list", "Array"]:
				...
				return r3sponse.error(f"Unknown format [{format}] for path [{path}].")
	def save(self, 
