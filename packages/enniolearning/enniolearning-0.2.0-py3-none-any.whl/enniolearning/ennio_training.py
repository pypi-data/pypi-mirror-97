...
def train(test_name=hp_test_name, nb_epochs=hp_nb_epochs, learning_rates=None, loss_funcs=None,
    ...
    log('Training parameters :', **kwargs)
    log(f'  - Batch size : {batch_size}', **kwargs)
    log(f'  - sequence length : {sequence_length}. Length of each sample.', **kwargs)
    log(f'  - Training dataset length : {network_input.shape[0]}', **kwargs)
    log(f'  - Training samples count : {len(train_dl)}. (Training dataset length / Batch size)', **kwargs)
