from enniolearning.utils import FindMaxLength, reject_for_repetition, log
from enniolearning.training_fcts import deserialize_score, deserialize_general_note, list_model_files, to_tensor, \
    prepare_sequences, device, reuse_network, get_serialized_model_filename_by_epoch, \
    get_serialized_model_name_by_filename, load_network, parse_epoch
import numpy as np
...
def evaluate_generated_score(prediction_by_track, network_input, enable_evaluation=True,
    ...
    #     log(f"Evaluation : morceau généré à {100 * (len(prediction) - plagia_part)/len(prediction)}%", **kwargs)
...
def create_generation_input(previous_prediction_output_as_int=None, network_input=None, **kwargs):
    if previous_prediction_output_as_int is not None:
        log('From previous prediction', **kwargs)
...
def generate_notes(model, int_to_note, n_vocab, generate_output_length=50, **kwargs):
    ...
    for note_index in range(generate_output_length):
        #         log('pattern', pattern, **kwargs)
        prediction_input = np.reshape(pattern, (
        1, pattern.shape[0], pattern.shape[1]))  # shape (1,sequence_length,len(instrument_index))
        ...
        if prediction.shape[0] != 1:
            log(
                f"expected batch output size == 1, was {prediction.shape[0]}", level=logging.WARN,
                **kwargs)  # could it be some feature to return several sequence in a batch (batch_size > 1) -> check perf
...
def create_midi(prediction_output, instruments_index,
    ...
    if create_midi_file:
        ...
        log(f"File {output_filename} created !", level=logging.DEBUG, **kwargs)
...
def post_processing(prediction_output, prediction_output_as_int, enable_post_processing=True,
                    min_no_rest_sequence_accepted=3, max_rest_sequence_accepted=10, serialize_prediction=False,
                    **kwargs):
    """
    Performs tasks :
    ...
    log(f"Prediction post processed", level=logging.DEBUG, **kwargs)
    return prediction_output[post_processing_indexing], prediction_output_as_int[
        post_processing_indexing], post_processing_score
...
def generate(notes=None, instruments_index=None, model=None, batch_size=1, network_input=None, int_to_note=None,
    ...
    log(f"Model loaded {str(model)}", level=logging.DEBUG, **kwargs)
    prediction_output, prediction_output_as_int = generate_notes(model,
                                                                 int_to_note,
                                                                 n_vocab,
                                                                 network_input=network_input,
                                                                 **kwargs)
    log(f"{len(prediction_output)} notes predicted", level=logging.DEBUG, **kwargs)
    ...
    return output_filename, prediction_output_as_int, (
...
def evaluate_models(path, exec_nb=1, loss_func_names=['BCELoss()'], lrs=[1e-3, 1e-4, 1e-5], select_epochs=False,
                    generate_output_length=300, print_progress=True, log_progress=False, notes=None,
                    instruments_index=None, batch_size=1, jordan_instance=None, **kwargs):
    """Path : use make_path()
    ...
    for loss_function_passed, loss_func_name in enumerate(loss_func_names):
        for lr_passed, lr in enumerate(lrs):
            ...
            # log(f"In folder {subfolder}", level=logging.DEBUG, **kwargs)
            evaluation[subfolder] = {}

            model_files_in_folder = list_model_files(subfolder, **kwargs)
            ...
            for model_passed, model_filename in enumerate(model_files):
                ...
                for i in range(exec_nb):
                    ...
                    if print_progress or log_progress:
                    ...
                    (diversity_rate, plagia_part, rejected_for_repetition, post_processing_score) = \
                        generate(notes, instruments_index,
                                 ...
                                 **kwargs)
                    if jordan_instance:
                        jordan_instance.send_status(
                            f"Diversity rate : {diversity_rate}. Plagia part : {plagia_part}. Rejected for repetition : {rejected_for_repetition}.")
...
def extract_epoch_from_filename(dic, first_key, **kwargs):
    """dic from evaluate_models()
    first_key is folder
    """
    extract = []
    for model_filename, model_stats in dic[first_key].items():
        extract.append(parse_epoch(model_filename, **kwargs))
    return extract
...
def plot_model_evaluation(evaluation, feature, **kwargs):
    ...
    for model_type in evaluation.keys():
        plt.plot(extract_epoch_from_filename(evaluation, model_type, **kwargs),
