...
def img_pos(data, column, row):
    ...
    img_south_capt = sum(size_of(data['element_config']['captions'], 'south')) * (row-1)

    top = data['padding']['north'] + title_top + col_title_top + (data['row_space'] + img_height)*(row-1) + img_south_capt
    left =  data['padding']['west'] + title_left + row_title_left + (data['column_space'] + img_width)*(column-1)
    return top, left

def south_caption_pos(data, column, row):
    top, left = img_pos(data, column, row)
    top += data['element_config']['img_height'] + data['element_config']['captions']['south']['offset']
    return top, left
...
def titles_pos_and_size(data, direction):
    ...
    if direction == 'north' or direction == 'south':
        ...
        if direction == 'south':
            offset_top += sum(size_of(data['element_config']['captions'], 'south')) * data['num_rows']
        ...
        height += sum(size_of(data['element_config']['captions'], 'south')) * (data['num_rows'] -1)
