...
# TODO
# class VideoCrop(BaseTransformation):
#
#     def __init__(self, crop, input_format: str = 'CHW'):
#         assert isinstance(crop, (tuple, list)), f'crop arg must be tuple or list but {type(crop)} found'
#         self.shape_x, self.shape_y = crop
#         self._y_dim = input_format.find('H')
#         self._x_dim = input_format.find('W')
#
#     def __call__(self, video, crop_pos_x=0.5, crop_pos_y=0.5):
#         X = video.shape[self._x_dim]
#         Y = video.shape[self._y_dim]
#         assert X >= 0, f'The X cropping size is {self.shape_x} but video size is {video.shape[self._x_dim]}'
#         assert Y >= 0, f'The X cropping size is {self.shape_y} but video size is {video.shape[self._y_dim]}'
