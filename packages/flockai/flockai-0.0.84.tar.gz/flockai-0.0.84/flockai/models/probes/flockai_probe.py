...
class FlockAIProbe(Probe):
    ...
    def collect(self):
        for name, metric in self.probe_metrics.items():
            ...
            elif name == ProcessCpuTimeMetric.__name__:
                ct = self.proc.cpu_times()
                metric.set_val(sum(ct[:4]))
            elif name == ProcessIOTimeMetric.__name__:
                ct = self.proc.cpu_times()
                metric.set_val(ct[4])
            elif name == ProcessAliveTimeMetric.__name__:
                metric.set_val(time() - self.proc.create_time())
            elif name == ProcessMemoryMetric.__name__:
                metric.set_val(self.proc.memory_percent())
            elif name == ProbeAliveTimeMetric.__name__:
                metric.set_val(time() - self.col_start)
            elif name == SimpleMetric.__name__:
                pass
            elif name == DiffMetric.__name__:
                pass
            elif name == TimerMetric.__name__:
                pass
            elif name == CounterMetric.__name__:
                pass
            # Expand this list on demand

    def get_metric(self, name):
        for key in self.probe_metrics.keys():
            print(key)
