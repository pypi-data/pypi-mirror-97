...
def wait_predicate(ex):
  return retry.if_transient_error(ex) or retry.if_exception_type(job_still_running_exception.JobStillRunningException)
...
class CaipJobExecutor:
  ...
  @retry.Retry(predicate = wait_predicate, deadline = constants.JOB_GET_TIMEOUT_IN_SECONDS)
  ...
  def wait_for_job_completion(self):
      ...
      self.__wait_for_job_completion(get_request)
