...
class SuiteFile(Node):
    ...
    def _load_case(self, source):
        ...
        for case in source['case']:
            if 'name' in case:
                cases.append({'name': case['name'],
                              'tag': self.load_tag(case)})
        return cases
...
class SuiteFileTree(object):
    ...
    def parse_selection(self, options):
        ...
        if selected_tag:
            if not self._parse_selection(self.root, selected_tag):
                raise NoCaseMatch(selected_tag)
    ...
    def match_tag(self, suite_tag, selected_tag):
        for tag in suite_tag:
