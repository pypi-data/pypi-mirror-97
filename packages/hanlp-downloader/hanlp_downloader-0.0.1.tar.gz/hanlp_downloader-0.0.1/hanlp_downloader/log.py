    ...
    def __call__(self, downloader: Downloader):
        ...
        if downloader.get_state() == Downloader.DOWNLOADING:
            ...
            written_update = written_update.format(
                percent_downloaded,
                down.readable_bytes(downloader.total_length).strip(),
                downloader.readable_speed.strip(),
                report_time_delta((downloader.total_length - downloader.total_downloaded) // downloader.speed)
            )
