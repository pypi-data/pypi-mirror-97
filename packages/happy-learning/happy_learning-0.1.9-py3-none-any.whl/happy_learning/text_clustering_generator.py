...
from .text_clustering import GibbsSamplingDirichletMultinomialModeling, LatentDirichletAllocation, LatentSemanticIndexing, NonNegativeMatrixFactorization, SelfTaughtShortTextClustering
...
class Clustering:
    ...
    def latent_dirichlet_allocation(self) -> LatentDirichletAllocation:
        ...
        :return LatentDirichletAllocation:
            Model object
        """
        return LatentDirichletAllocation(doc_term_matrix=self.cluster_params.get('doc_term_matrix'),
                                         vocab=self.cluster_params.get('vocab'),
                                         n_clusters=10 if self.cluster_params.get('n_clusters') is None else self.cluster_params.get('n_clusters'),
                                         n_iterations=5 if self.cluster_params.get('n_iterations') is None else self.cluster_params.get('n_iterations'),
                                         decay=1.0 if self.cluster_params.get('decay') is None else self.cluster_params.get('decay'),
                                         )
    ...
    def latent_semantic_indexing(self) -> LatentSemanticIndexing:
        ...
        :return LatentSemanticIndexing:
            Model object
        """
        return LatentSemanticIndexing(doc_term_matrix=self.cluster_params.get('doc_term_matrix'),
                                      vocab=self.cluster_params.get('vocab'),
                                      n_clusters=10 if self.cluster_params.get('n_clusters') is None else self.cluster_params.get('n_clusters'),
                                      n_iterations=5 if self.cluster_params.get('n_iterations') is None else self.cluster_params.get('n_iterations'),
                                      decay=1.0 if self.cluster_params.get('decay') is None else self.cluster_params.get('decay'),
                                      training_algorithm='multi_pass_stochastic' if self.cluster_params.get('training_algorithm') is None else self.cluster_params.get('training_algorithm'),
                                      )
    ...
    def non_negative_matrix_factorization(self) -> NonNegativeMatrixFactorization:
        ...
        return NonNegativeMatrixFactorization(lang=self.cluster_params.get('lang'),
                                              doc_term_matrix=self.cluster_params.get('doc_term_matrix'),
                                              vocab=self.cluster_params.get('vocab'),
                                              n_clusters=10 if self.cluster_params.get('n_clusters') is None else self.cluster_params.get('n_clusters'),
                                              n_iterations=5 if self.cluster_params.get('n_iterations') is None else self.cluster_params.get('n_iterations'),
                                              )
