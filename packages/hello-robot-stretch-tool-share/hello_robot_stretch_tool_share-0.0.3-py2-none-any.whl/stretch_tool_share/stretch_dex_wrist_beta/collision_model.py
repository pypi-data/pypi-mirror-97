...
class CollisionStretchDexWristToBase(RobotCollisionModel):
    ...
    def step(self, status):
        ...
        height = -1* pose[1][3] + .005  # Height , pos is up, pad out
        extension = -1*pose[2][3]  # Extension, pos is reach
        if 0:
            print('------')
        ...
        if tool_clear_of_base:
            ...
            lift_lower_limit = min(max(self.params['palm_height'], height), status['lift']['pos'])  # Only low it to reduce the ROM

        if status['lift']['pos']>self.params['palm_height'] and status['lift']['pos']<self.params['lift_up']:
            q = math.atan2(status['lift']['pos'],self.params['arm_clear_base'])  # How far pitch can fold back before hitting base
            pitch_upper_limit= max(status['end_of_arm']['wrist_pitch']['pos'],q)  # Only allow pitch to fold back further as lift raises, when lowering limit lift descent


        return {'lift': [lift_lower_limit, None], 'arm': [None, None], 'wrist_yaw': [None, None], 'wrist_pitch': [None, pitch_upper_limit], 'wrist_roll': [None, None]}
