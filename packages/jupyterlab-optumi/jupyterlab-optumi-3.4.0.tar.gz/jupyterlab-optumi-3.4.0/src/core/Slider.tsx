...
export class Slider extends React.Component<IProps, IState> {
    ...
    private handleTextBoxChange = async (event: any) => {
        ...
        if ( value.length == 5 && value.startsWith('unsp')) {
            ...
            this.checkAndSetState({ value: this.min, weGotADot: '' });
        ...
        if (value.endsWith('.') || value.endsWith('.0')/* || value.endsWith('.00')*/) {
            this.checkAndSetState({ weGotADot: value });
    ...
    private handleChange = async (event: React.ChangeEvent<{}>, newValue: number | number[]) => {
		if (newValue instanceof Array) {
            ...
            this.checkAndSetState({ value: cappedValue, weGotADot: '' });
    ...
    private parseTextAndSetValue(value: string, unit: string) {
        if (!isNaN(+value)) {
            ...
            if (unscaledValue < this.min) {
                ...
                this.checkAndSetState({ value: this.max, weGotADot: '' });
                this.slowDownSaveValue(this.max);
            } else {
                this.checkAndSetState({ value: unscaledValue, weGotADot: '' });
    ...
    private startTyping = () => {
        this.checkAndSetState({typing: true})
    }
    ...
    private stopTyping = () => {
        ...
        this.checkAndSetState({typing: false})
    }

    private startHover = () => {
        this.checkAndSetState({hovered: true})
    }

    private stopHover = () => {
        this.checkAndSetState({hovered: false})
    }

    public render = (): JSX.Element => {
        if (Global.shouldLogOnRender) console.log('SliderRender (' + new Date().getSeconds() + ')');
    ...
    private handleMetadataChange = () => this.checkAndSetState({ value: this.props.getValue(), weGotADot: '' })
