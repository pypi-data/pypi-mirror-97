...
class WordEmbedding(BaseEmbedding):
    ...
    def _build_token2idx_from_w2v(self):
        ...
        token2idx = {
            self.indexer.token_pad: 0,
            self.indexer.token_unk: 1,
            self.indexer.token_bos: 2,
            self.indexer.token_eos: 3
        }
        ...
        self.indexer.token2idx = self.w2v_token2idx
        self.indexer.idx2token = dict([(value, key) for key, value in self.w2v_token2idx.items()])
        logging.debug('------------------------------------------------')
