...
from kolibri_dnn.tasks.base_model import BaseModel, DefaultEmbedding
...
class BaseClassificationModel(BaseModel):
    ...
    def __init__(self, vocabulary,
                 embedding: Optional[BaseEmbedding] = None, other_input=None,
                 hyper_parameters: Optional[Dict[str, Dict[str, Any]]] = None):
        super(BaseClassificationModel, self).__init__(vocabulary, embedding, hyper_parameters)
        if hyper_parameters is None and \
                self.embedding.indexer.__getattribute__('multi_label') is True:
    ...
    def compile_model(self, **kwargs):
        if kwargs.get('loss') is None and self.embedding.indexer.multi_label:
    ...
    def predict(self,
        ...
        """
        with kolibri_dnn.utils.custom_object_scope():
    ...
    def evaluate(self,
        ...
        if self.indexer.multi_label:
            y_pred_b = self.indexer.multi_label_binarizer.fit_transform(y_pred)
            y_true_b = self.indexer.multi_label_binarizer.fit_transform(y_data)
            report = metrics.classification_report(y_pred_b,
                                                   y_true_b,
                                                   target_names=self.indexer.multi_label_binarizer.classes_,
