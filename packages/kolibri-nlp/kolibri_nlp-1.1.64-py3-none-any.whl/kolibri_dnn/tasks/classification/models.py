...
from kolibri_dnn.layers import L, AttentionWeightedAverageLayer, KMaxPoolingLayer
from kolibri_dnn.tasks.classification.base_model import BaseClassificationModel
...
if __name__ == "__main__":
    ...
    from kolibri_dnn.corpus import ConsumerComplaintsCorpus
    from kolibri.data.vocabulary import Vocabulary
    import kolibri_dnn
    train_data=ConsumerComplaintsCorpus('sample')
    train_dataset=train_data.get_sent_dataset()
#    test_data=ConsumerComplaintsCorpus('test')
#    test_dataset=test_data.get_sent_dataset()

    vocab = Vocabulary.from_instances(train_dataset, min_count={"tokens": 10, "label":1} )
    train_data=ConsumerComplaintsCorpus('sample')
    test_data=None#ConsumerComplaintsCorpus('test')
    ...
    m = BiLSTM_Model(vocabulary=vocab, embedding=embed)
    # m.build_model(x, y)
    m.fit(dataset_train=train_data, dataset_test=test_data, train_len=train_data.length, epochs=5, batch_size=128)
    m.save("issues.hd5")
    sent_data=ConsumerComplaintsCorpus('validate')
    x_valid, y_valid=sent_data.get_sent_dataset().get_data()
