...
def _patched_install_pyfunc_deps(model_path=None, install_mlflow=False):
    ...
    install_server_deps = ["pip install gunicorn[gevent] requests lariat-run==0.1.71"]
...
def patched_build_and_push_container(build, push, container, mlflow_home):
        ...
        def setup_container(_):
            return "\n".join(
                [
                    ...
                    'RUN pip install lariat-run==0.1.71',
...
def patch():
    ...
    mlflow.sagemaker.build_and_push_container = _patched_build_and_push_container
