...
class PatchedPyFuncModel(_PyFuncModel):
    def predict(self, data):
        ...
        tags = {"run_id": self.metadata.run_id, "artifact_path": self.metadata.artifact_path, "flavor": self.metadata.flavors[pyfunc.FLAVOR_NAME]["loader_module"]}
        ddtags = ",".join([f"{k}:{v}" for k, v in tags.items()])

        result = super(PatchedPyFuncModel, self).predict(data)

        logs_dct = {"ddsource": "agent", "ddtags": ddtags, "hostname": "yes", "message": "input: " + str(data) + " output: " + str(result) }
