...
def _patched_install_pyfunc_deps(model_path=None, install_mlflow=False):
    ...
    install_server_deps = ["pip install gunicorn[gevent] requests lariat-run==" + LARIAT_VERSION]
...
def _patched_serve_pyfunc(model):
    ...
    cmd = (
        "LARIAT_GEVENT_PATCH_ALL=1 lariat-run gunicorn --log-level debug -w {cpu_count} ".format(cpu_count=cpu_count)
...
class PatchedPyFuncModel(_PyFuncModel):
    def predict(self, data):
        ...
        base_tags = {"run_id": self.metadata.run_id, "artifact_path": self.metadata.artifact_path, "flavor": self.metadata.flavors[pyfunc.FLAVOR_NAME]["loader_module"]}
        ...
        for i, row in data.iterrows():
            ...
            logs_dct = {"ddsource": "agent", "ddtags": ddtags, "hostname": "yes", "message": "input: " + str(row) + " output: " + str(result[i])}
            messages.append(logs_dct)

        requests.post("https://http-intake.logs.datadoghq.com/v1/input", json=messages, headers=headers)
        return result
...
def patch():
    ...
    mlflow.sagemaker.cli.commands.commands['build-and-push-container'].callback = _patched_build_and_push_container
