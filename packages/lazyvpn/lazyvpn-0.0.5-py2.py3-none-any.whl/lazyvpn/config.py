...
class Config(object):
    ...
    def update_config_file(self):
        ...
        self._set_okta_password(config_dict['okta_username'])
        config_dict['company_vpn_region'] = self._get_company_vpn_region(str(DEFAULT_VPN_REGION))
    ...
    def _get_user_input(self, message, default=None, hidden=False):
    ...
    def _set_okta_password(self, okta_username):
        """Get and validate okta password. [Optional]"""
        ui.default.message(
            "Set Okta Password that will be stored under system keychain")
        okta_password = self._get_user_input(
            "Okta Pass Word", default=None, hidden=True)
        keyring.set_password(LAZY_VPN_KEYCHAIN_SERVICE_NAME, okta_username, okta_password)

    def _get_company_vpn_region(self, default_entry):
        """ Get Okta AWS App name """
        ui.default.message(
            "Enter the VPN Region Name")
        company_vpn_region = self._get_user_input("VPN Region Name", default_entry)
