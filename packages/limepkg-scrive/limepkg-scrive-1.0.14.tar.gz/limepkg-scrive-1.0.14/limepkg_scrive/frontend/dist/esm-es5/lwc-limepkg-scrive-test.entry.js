...
var Test = /** @class */ (function () {
    ...
    Test.prototype.render = function () {
        ...
        var signableFiles = files.filter(this.isSignable, this);
        var noSignableFiles = signableFiles.length === 0;
        var tooManySignableFiles = signableFiles.length > 1;
        var signingDisabled = noSignableFiles || tooManySignableFiles;
        var items = files.map(function (f) {
            if (_this.isSignable(f)) {
                return { text: f.filename + " is signable." };
            }
            return { text: f.filename + " is not signable." };
        });
        return (h("div", null, h("limel-collapsible-section", { header: "Sign with Scrive" }, h("limel-button", { label: "Design Contract", outlined: true, disabled: signingDisabled, onClick: function () { return _this.goToScrive(_this.context.id); } }), h("limel-checkbox", { label: "Add associated person as signing party", id: "fab", checked: this.includePerson, required: false, disabled: signingDisabled, onChange: function () { return _this.includePerson = !_this.includePerson; } }), noSignableFiles && (h("limel-dialog", { heading: "No Signable Files", open: true }, "There are no signable files.", h("limel-list", { items: items }), "Please keep in mind that only certain types (", this.allowedExtensions.join(", "), ") of these types of files are signable.")), tooManySignableFiles && (h("limel-dialog", { heading: "Too Many Signable Files", open: true }, "Only one file is signable at a time.", h("limel-list", { items: items }))))));
    };
...
export { Test as lwc_limepkg_scrive_test };
