...
const Test = class {
    ...
    render() {
        ...
        return (core.h("div", null, core.h("limel-collapsible-section", { header: "Sign with Scrive" }, core.h("limel-button", { label: `Design Contract`, outlined: true, disabled: signingDisabled, onClick: () => this.goToScrive(this.context.id) }), core.h("limel-checkbox", { label: "Add associated person as signing party", id: "fab", checked: this.includePerson, required: false, disabled: signingDisabled, onChange: () => this.includePerson = !this.includePerson }), noSignableFiles && (core.h("limel-dialog", { heading: "No Signable Files", open: true }, "There are no signable files.", core.h("limel-list", { items: items }), "Please keep in mind that only certain types (", this.allowedExtensions.join(", "), ") of these types of files are signable.")), tooManySignableFiles && (core.h("limel-dialog", { heading: "Too Many Signable Files", open: true }, "Only one file is signable at a time.", core.h("limel-list", { items: items }))))));
    }
...
exports.lwc_limepkg_scrive_test = Test;
