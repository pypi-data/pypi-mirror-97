...
def main():
    ...
    if args.list_bypass:
        def add_color(x): return x.replace(r"[id]", "\x1b[33m[id]\x1b[0m")
    ...
    if "proxy" in getattr(extractor, "required", []) and not args.skip_proxy:
         if not args.proxy:
             parser.error(f"{extractor.__class__.__name__} required arguments --proxy, or skip using the --skip-proxy argument if already using vpn")

    extractor.to_list = args.json or args.json_dump
