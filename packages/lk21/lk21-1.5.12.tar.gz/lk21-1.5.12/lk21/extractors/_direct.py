...
class _direct(BaseExtractor):
        ...
        while not end:
            ...
            for title, (pattern, klsname) in directRules.items():
                if klsname and pattern.search(url):
                    ...
                    func = getattr(self, f"extract_{klsname}")
                    try:
    ...
    def extract_mediafire(self, url):
        raw = self.session.get(url)
        soup = self.soup(raw)
    ...
    def extract_zippyshare(self, url):
        raw = self.session.get(url)
        res = re.search(
