...
def simple(resource, doc, env, *args, **kwargs):
...
def explicit_dataframe(resource, doc, env, *args, **kwargs):
    '''Construct a dataframe and iterate it though a PandasDataframeSource'''
    import pandas as pd
    ...
    yield from PandasDataframeSource('<df>', df, get_cache())


def implicit_dataframe(resource, doc, env, *args, **kwargs):
    """Return a dataframe and let metapack figure out how to iterate it"""
    import pandas as pd
