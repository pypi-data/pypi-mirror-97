...
class ML_Logger:
    ...
    def save_module(self, module, path="weights.pkl"):
        ...
        return self.save_torch(state_dict, path=path, overwrite=False)
    ...
    def save_torch(self, obj, *keys, path=None, overwrite=True):
        path = pJoin(self.prefix, *keys, path)
        import io, torch
        buf = io.BytesIO()
        torch.save(obj, buf)
        buf.seek(0)
        self.client.multipart_upload(key=path, buf=buf, overwrite=overwrite)
