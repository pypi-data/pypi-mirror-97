...
class SessionRequests:
    def __init__(self, proxy: str = None, **kwargs):
        """
        :param max_workers:
        ...
        if proxy:
            ...
            if proxy.startswith("https://"):
                self.session.session.proxies = {"https": proxy[8:]}
            elif proxy.startswith("http://"):
                self.session.session.proxies = {"http": proxy[7:]}
            else:
                self.session.session.proxies = {"http": proxy}

    def get(self, *args, **kwargs):
        _ = self.session.get(*args, **kwargs)
        return FutureResponse(_)
