...
import pymongo
from BaseColor.base_colors import red, yellow, cyan, green, hred, blue
...
class MongodbCopy(object):
    ...
    def _show_dbs(self, db):
        ...
        for i, name in enumerate(db_names):
            printer(f"[ {red(i)} ]: {yellow(name)}")
        printer('*', fill_with='*')
        print(f"input the {red('num')} of the database name")
        sel = input(f"""(Or "q!" to exit; "n**{yellow(db_names[0])}" or "n**{yellow('other_db_name')}" to input real db name): """).strip()
        if sel == "q!":
            print("\nbye\n")
            exit(0)
