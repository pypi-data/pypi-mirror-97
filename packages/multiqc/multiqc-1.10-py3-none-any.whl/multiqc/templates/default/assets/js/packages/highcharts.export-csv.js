...
!(function (a) {
  ...
  function f(b, c, d, f, g) {
    var h,
      i,
      j,
      k = (b.options.exporting || {}).csv || {},
      l = k.url || "http://www.highcharts.com/studies/csv-export/download.php";
    (j = b.options.exporting.filename
      ? b.options.exporting.filename
      : b.title
      ? b.title.textStr.replace(/ /g, "-").toLowerCase()
      : "chart"),
      window.Blob && window.navigator.msSaveOrOpenBlob
        ? ((i = new Blob([f])), window.navigator.msSaveOrOpenBlob(i, j + "." + d))
        : e
        ? ((h = document.createElement("a")),
          ...
          h.click(),
          h.remove())
        : a.post(l, { data: f, type: g, extension: d });
  }
  var b = a.each,
    c = a.pick,
    d = a.seriesTypes,
  ...
  a.setOptions({
    ...
    (a.Chart.prototype.getDataRows = function () {
      var e,
        ...
        o =
          ...
          function (b, c, d) {
            return b instanceof a.Axis
              ? (b.options.title && b.options.title.text) || (b.isDatetimeAxis ? "DateTime" : "Category")
              : b
              ? b.name + (d > 1 ? " (" + c + ")" : "")
              : "Category";
          },
            ...
            b(d.points, function (a, b) {
              ...
              for (
                q = 0,
                  ...
                  g[e].x = a.x,
                  g[e].xValues[n] = a.x,
                  (d.xAxis && "name" !== d.exportKey) || (g[e].name = a.name);
                q < i;

              )
                (f = h[q]), (j = a[f]), (g[e][k + q] = c(m[f][j], j)), (q += 1);
            }),
              (k += q);
          }
        });
      ...
      for (i = [j], k = p.length; k--; )
        (r = p[k][0]),
          ...
          b(h, function (b) {
            ...
            d ||
              (e.isDatetimeAxis
                ? (b.x instanceof Date && (b.x = b.x.getTime()), (d = a.dateFormat(n, b.x)))
                : (d = e.categories ? c(e.names[b.x], e.categories[b.x], b.x) : b.x)),
              b.splice(s, 0, d);
          });
      return (i = i.concat(h));
    }),
    (a.Chart.prototype.getCSV = function (a) {
      ...
      return (
        b(d, function (b, e) {
          for (var h = "", i = b.length, j = a ? (1.1).toLocaleString()[1] : "."; i--; )
            (h = b[i]),
              "string" == typeof h && (h = '"' + h + '"'),
              "number" == typeof h && "," === j && (h = h.toString().replace(".", ",")),
              (b[i] = h);
    ...
    (a.Chart.prototype.getTable = function (a) {
      ...
      return (
        b(d, function (b, d) {
          ...
          for (c += "<tr>", g = 0; g < b.length; g += 1)
            (f = b[g]),
              "number" == typeof f
                ? ((f = f.toString()),
                  "," === h && (f = f.replace(".", h)),
                  (c += "<" + e + ' class="number">' + f + "</" + e + ">"))
                : (c += "<" + e + ">" + (void 0 === f ? "" : f) + "</" + e + ">");
    ...
    a.getOptions().exporting &&
      a.getOptions().exporting.buttons.contextButton.menuItems.push(
        {
          textKey: "downloadCSV",
          onclick: function () {
            this.downloadCSV();
