...
def compute_loss(predictions, targets, , device, criterion=None):
    rearranged_output = predictions.view(
        predictions.shape[0] * predictions.shape[1], -1)
    rearranged_target = targets.view(-1)
    loss = criterion(rearranged_output.to(device), rearranged_target.to(device))
    return loss
...
def train_model(model, classifier, dataloader, optimizer, criterion, device, id_fold, epochs):
    ...
    for epoch in tqdm(range(epochs), position=0, leave=False, desc='epoch'):
        ...
        for batch in tqdm(dataloader, position=1, leave=False, desc='learning batch'):
            ...
            loss = compute_loss(predictions_scores, batch['target'], device, criterion)
            loss.backward()
