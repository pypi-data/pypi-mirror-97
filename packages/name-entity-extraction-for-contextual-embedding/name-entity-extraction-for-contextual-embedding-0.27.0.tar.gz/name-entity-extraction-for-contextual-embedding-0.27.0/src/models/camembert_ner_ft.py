...
def train_model(
    ...
    for epoch in range(epochs):
        ...
        for batch in dataloader:
            optimizer.zero_grad()
            if "GRU" not in str(type(classifier)):
                loss = make_predictions(batch, classifier, criterion, device, model)
            else:
                loss = classifier.neg_log_likelihood(
                    model(
                        input_ids=batch["tokens"].to(device),
                        attention_mask=batch["attention_mask"].to(device),
                    ).last_hidden_state,
                    batch["target"].to(device),
                    dataloader.dataset.pad_id,
                )
            loss.backward()
...
def make_predictions(batch, classifier, criterion, device, model):
...
def eval_model(model, classifier, dataloader, criterion, device, labels):
    ...
    with torch.no_grad():
        ...
        for batch in dataloader:
            ...
            predictions_scores = (
                f.log_softmax(predictions, dim=2)
                if "GRU" not in str(type(classifier))
                else predictions
            )
            loss = compute_loss(
