        ...
        # caso não haja relação de pai e filho
            ...
            if detail.get('attributes'):
                ...
                for attribute in detail['attributes']:
                    ...
                    if presentations.get('cor') and not str(attribute['values'][0]['id']) in [c['id'] for c in presentations['cor']['options']]:
                        option = {
                            "id": str(attribute['values'][0]['id']),
                            "label": attribute['values'][0]['value']
                        }
                    ...
                    if attribute['name'].lower() == "tamanho":
                        ...
                        if presentations.get('tamanho') and not str(attribute['values'][0]['id']) in [c['id'] for c in presentations['tamanho']['options']]:
                            option = {
                                "id": str(attribute['values'][0]['id']),
                                "label": str(attribute['values'][0]['value'])
                            }
