...
@pg.production("limit_clause : LIMIT ARG")
def f(p): return p[1]

@pg.production("offset_clause : OFFSET INTEGER")
@pg.production("offset_clause : OFFSET ARG")
def f(p): return p[1]
...
def f(p):
  r = SimpleTripleBlock()
  for i in p[1]:
    if   isinstance(i, ExistsBlock): # Can be coalesced with the simple triple block
      r.extend(i)
    elif isinstance(i, Block) or isinstance(i, Bind) or isinstance(i, Filter):
      r.append(i)
...
# r = []
  ...
  #     if currents:
  #       r.append(SimpleTripleBlock(currents))
  #       currents = []
  #     r.append(i)
  #   elif isinstance(i, list):
  #     currents.extend(i)
  #   elif isinstance(i, Bind) or isinstance(i, Filter):
  #     currents.append(i)
  #   else:
  #     raise ValueError
  # if currents: r.append(SimpleTripleBlock(currents))
  # print("XXX", r)
  # return r
...
def f(p):
  ...
  #return [y for x in p[0] for y in x]
#@pg.production("group_graph_pattern_item : group_graph_pattern_item_triple_content+")
#def f(p):
#  r = SimpleTripleBlock()
#  for i in p:
#    print( "III", i)
#    if isinstance(i, Bind) or isinstance(i, Filter): r.append(i)
#    else:                                            r.extend(i)
#  print("RRR", r)
#  return r
...
def f(p):
  ...
  #p.insert(0, rply.Token("UNION", "UNION")) # Lisp style notation!
  #return p
...
def _expand_triple(triples, s, ps_os):
  for p, o in ps_os:
    if   isinstance(p, rply.Token) and ((p.name == "VAR") or (p.name == "ARG")):
      p.inversed = False
      p.modifier = None
...
def f(p): return p[0]
@pg.production("graph_term : ARG")
def f(p):
  translator = CURRENT_TRANSLATOR.get()
  translator.has_parameter = True
  p = p[0]
...
class Block(list):
  ...
  def get_ordered_vars(self):
    vars = set()
    ordered_vars = []
    self._get_ordered_vars(vars, ordered_vars)
    return ordered_vars
  
  def _get_ordered_vars(self, vars, ordered_vars): raise NotImplementedError
...
class ExistsBlock(SimpleTripleBlock):
  def __init__(self, l):
    SimpleTripleBlock.__init__(self, l)
    _prefix_vars(self, "??exists%s_" % id(self))
  def _get_ordered_vars(self, vars, ordered_vars):
    pass
  
class NotExistsBlock(TripleBlock): pass

class MinusBlock(TripleBlock): pass

class OptionalBlock(TripleBlock): pass
