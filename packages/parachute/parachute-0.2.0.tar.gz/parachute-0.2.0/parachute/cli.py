...
def compare(backup_file, socket):
    ...
    for (parameter, count) in craft.list_params():
...
def restore(backup_file, socket):
...
def get_params(params, socket):
    ...
    for param in params:
        param = craft.get_param(Parameter(name=param))
        click.echo(f"  {param.name} = {param.value}")
