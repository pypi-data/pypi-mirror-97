...
class 善检网api第2版_套餐_百度(善检网api第2版_套餐):
    ...
    '''验证通过'''
    def 修改服务基础(self, 机构id, 服务名称, 套餐ids, 服务基础id):
        org = self.机构概况字典[机构id]
        机构id = org['机构id']
        if org['地址完整性']:
            params = dict(
    ...
    '''验证通过'''
    def 全部机构增量生成服务基础(self, 名称包含=''):
        rs = []
        机构ids = list(set(x['机构id'] for x in self.机构概况字典.values() if 名称包含 in x['机构名称']))
        for x in 机构ids:
    ...
    def 机构增量生成服务基础(self, 机构id):
        ...
        if org['地址完整性']:
            套餐s = self.获取机构套餐列表(机构id=机构id, 去除test=True)  # 所有套餐
            服务基础字典 = self.获取机构服务基础字典(机构id=机构id, key='名称', 套餐s=套餐s)
            服务基础字典 = {self.名称标准化(k): v for k, v in 服务基础字典.items()}
            df = pd.DataFrame([x for x in 套餐s if not [y for y in x['服务列表'] if y['机构id'] == 机构id]])
            if len(df):
                df['待加入的服务基础名称'] = df.套餐名称.apply(self.名称标准化)
                for 待加入的服务基础名称, df in df.groupby('待加入的服务基础名称'):
                    套餐ids = df['套餐id'].to_list()
                    if 待加入的服务基础名称 in 服务基础字典:
                        # 添加到已有的服务基础
                        服务基础id = 服务基础字典[待加入的服务基础名称]['服务id']
                        服务基础详情 = self.获取服务基础详情(服务基础id=服务基础id)
                        原套餐ids = [套餐['套餐id'] for 套餐 in 服务基础详情['套餐s']]
                        套餐ids = list(set(原套餐ids + 套餐ids))
                        r = self.修改服务基础(机构id=机构id, 服务名称=待加入的服务基础名称, 套餐ids=套餐ids, 服务基础id=服务基础id)
                    else:
                        # 创建新的服务基础
                        r = self.新增服务基础(机构id=机构id, 服务名称=待加入的服务基础名称, 套餐ids=套餐ids)
                    (成功 if type(r) is int else 失败)[待加入的服务基础名称] = r
            else:
                处理结果['data'] = '未发现新套餐'
        else:
            处理结果['data'] = '地址不完整'
