...
def load_and_rename_ci_files(model, encoding, tree_size, target=False):
    ...
    if model == 'BD' & tree_size == 'SMALL':
        predicted, target = phylodeep_data_BD_small.get_ci()
    elif model == 'BD' & tree_size == 'LARGE':
        predicted, target = phylodeep_data_BD_large.get_ci()
    elif model == 'BDEI' & tree_size == 'SMALL':
        predicted, target = phylodeep_data_BDEI_small.get_ci()
    elif model == 'BDEI' & tree_size == 'LARGE':
        predicted, target = phylodeep_data_BDEI_large.get_ci()
    elif model == 'BDSS' & tree_size == 'LARGE':
        predicted, target = phylodeep_data_BDSS_large.get_ci()

    if target:
        df = pd.read_csv(target + model + '_' + tree_size + '.csv.gz', header=None)
        df.columns = TARGET_NAMES[model]

    else:
        df = pd.read_csv(predicted + model + '_' + tree_size + '_' + encoding + '.csv.gz', header=None)
