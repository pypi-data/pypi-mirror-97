...
def pca_scale_load(tree_size, model):
    pca_reload = pk.load(open(os.path.join(PRETRAINED_PCA_DIR, 'models', model + '_' + tree_size + '_PCA' + '.pkl'), 'r'))
...
def pca_data_load(tree_size, model):
    pca_data = pd.read_csv(PRETRAINED_PCA_DIR+ 'models/' + model + '_' + tree_size + '_PCA' + '.csv.', header=None)
