...
def pca_plot(tree_d, sim_d, variance_explained, png_out):
    ...
    xy = np.vstack([sim_d.iloc[:, 0], sim_d.iloc[:, 1]])
    z = gaussian_kde(xy)(xy)

    fig, (ax1, ax2) = plt.subplots(1, 2)
    fig.tight_layout(pad=3.0)

    ax1.scatter(sim_d.iloc[:, 0], sim_d.iloc[:, 1], c=z, s=100, edgecolor='None')
    ax1.plot(tree_d[:, 0], tree_d[:, 1], '*', color='r', markersize=20)

    ax1.set_xlim([min(min(sim_d.iloc[:, 0]), tree_d[:, 0]), max(max(sim_d.iloc[:, 0]), tree_d[:, 0])])
    ax1.set_ylim([min(min(sim_d.iloc[:, 1]), tree_d[:, 1]), max(max(sim_d.iloc[:, 1]), tree_d[:, 1])])
    ...
    xy2 = np.vstack([sim_d.iloc[:, 2], sim_d.iloc[:, 3]])
    z2 = gaussian_kde(xy2)(xy2)

    ax2.scatter(sim_d.iloc[:, 2], sim_d.iloc[:, 3], c=z2, s=100, edgecolor='None')
    ax2.plot(tree_d[:, 2], tree_d[:, 3], '*', color='r', markersize=20)

    ax2.set_xlim([min(min(sim_d.iloc[:, 2]), tree_d[:, 2]), max(max(sim_d.iloc[:, 2]), tree_d[:, 2])])
    ax2.set_ylim([min(min(sim_d.iloc[:, 3]), tree_d[:, 3]), max(max(sim_d.iloc[:, 3]), tree_d[:, 3])])
