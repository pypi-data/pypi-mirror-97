...
def STRING_annotation(genes, species, outfile, pubmed=0):
    """

    :param gene: list of genes for functional annotation analysis
    :param species:
    :param pubmed:
    :return:
    """
    ...
    request_url += "&" + "species=%s" % species
    request_url += "&" + "allow_pubmed=%s" % pubmed
    ...
    with open(outfile, 'w') as out_handle:
        for line in response.readlines():
            out_handle.write(line.decode('utf-8'))
    out_handle.close()
...
def STRING_net(gene, species, max_nodes=500, required_score=400, exclude_tm=False):
    ...
    request_url += "&" + "species=%s" % str(species)
