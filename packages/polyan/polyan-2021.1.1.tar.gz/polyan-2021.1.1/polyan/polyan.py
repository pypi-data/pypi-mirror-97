...
def fp2poly(df, df_columns=['ORF', 'Ribo_Prints', 'RNA_Prints'], parset = 'Scer', include_idle = True, poly_limit=30, remove_spurious_RNAs=True):
    ...
    gene names, Ribo-Seq data and RNA-Seq data in that order.
    ...
    #evaluate whether default columns are specified:
    has_RNA = False
    has_length = False
    if df_columns == ['ORF','Ribo_Prints','RNA_Prints','length']:
        dats = df[df_columns]
        has_RNA = True
        has_length = True
    elif df_columns == ['ORF', 'Ribo_Prints', 'RNA_Prints']:
        dats = df[df_columns]
        has_RNA = True
    elif df_columns == ['ORF', 'Ribo_Prints', 'length']:
        dats = df[df_columns]
        has_length = True
    elif df_columns == ['ORF','Ribo_Prints']:
        dats = df[df_columns]
    elif set(df_columns).issubset(df.columns):
        dats = df[df_columns]
        if len(df_columns) == 2:
            df_columns = ['ORF','Ribo_Prints']
        elif len(df_columns) == 3:
            df_columns = ['ORF','Ribo_Prints','RNA_Prints']
            has_RNA = True
        elif len(df_columns) == 4:
            df_columns = ['ORF','Ribo_Prints','RNA_Prints','length']
            has_RNA == True
            has_length = True
    else:
    ...
    if not (has_RNA and has_length):
        ...
        if not found_testname:
            print('No valid gene or transcript name data found')
            return
        if not has_RNA:
            add_columns.append('RNA_Prints')
        if not has_length:
            add_columns.append('length')
        dats = dats.merge(genes[add_columns],
