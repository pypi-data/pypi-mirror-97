...
def full_load_file(file: str):
    return _sandbox_execute_pybuild(file, "cache", Permissions())
...
# FIXME: Should this be removed? It's no longer a security risk, but it might
# be useful not to allow code for portability reasons. There would be a performance cost, as well as require
# re-introducing the module cloning code
