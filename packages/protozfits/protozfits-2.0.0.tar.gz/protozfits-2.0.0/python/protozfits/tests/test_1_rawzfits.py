...
def test_import_and_open():
    ...
    rawzfitsreader.ProtobufIFits(
        file_path=relative_test_file_path,
        table_name="Events"
    )
...
def test_import_open_and_read():
    ...
    relative_test_file_path = os.path.relpath(example_file_path)
    ifits = rawzfitsreader.ProtobufIFits(
        file_path=relative_test_file_path,
        table_name="Events"
    )
...
def iterate():
    ...
    ifits = rawzfitsreader.ProtobufIFits(example_file_path, "Events")
    for i in range(len(ifits)):
        event = L0_pb2.CameraEvent.FromString(ifits.read_serialized_message(i + 1))
        yield i, event
