...
import os
import warnings  ##revisar si es necesario instalar
...
import platform  ##revisar si es necesario instalar
...
##############.-----------------CASI FINAL
#------------------revisar cambiar crval de projeccion 
##comentarios tambien 
...
class prueba_ifsUW():
 ...
 #   operative_system()
    ...
    window.geometry("1300x730") #Configurar tamaño
    ...
    def __init__(self): 
        '''
        Funtion that define the type of Operative System necesarry to know how access to the files 
        like image of filters
        '''
        def operative_system():
            ...
            if sistema == "Windows":
                ...
                (self.flag_system) = 1
        '''
        Funtion that defines the maps_array and maps_array_inv necessary to 
        ...
        and control the function to select the pixel for integrate_region()
        '''
        def onclick_(event):
            if (self.flag_file)==1:
                if (self.flag_integrate_region) == 0:
                    ...
                    if (self.flag_integrate_region2) == 0:
                        try:
                            ...
                            print(e)
        '''
        Function to control the movement of the mousse inside the canvas that display the
        FITS image to show the coordentaes about the pixel in the function coordinates_()
        '''
        ...
        '''
        Function to set wavelength range, with varla5 and varla6 for the minimun and maximun
        values, is only for varla5 < varla6, with 
        minumun value of array lambda <=  varla5 < varla6 <= maximun value of array lambda
        Not accept negatives values
        '''
        ...
        '''
        Funtion to reset wavelength range to the origins values that are show in the variables of:
            varla5 = minimun value of the array lambda
            varla6 = maximun value of the array lambda
        '''
        ...
        '''
        Funtion to set the flux range with varla3 and varla4 for the minimun and maximun
        values, is only for varla3 < varla4, accept negatives values
        '''
        def set_flux_range():
            if (self.flag_file)==1:
                try:
                ...
                MessageBox.showerror("Error!","Please, first choose a file") 
        '''
        Funtion to reset flux range
        varla3 = minimun value of the actual spectrum
        varla4 = maxinum value of the actual spectrum
        '''
        def reset_flux_range():
            if (self.flag_file)==1:
                ...
                MessageBox.showerror("Error!","Please, first choose a file") 
        '''function to set the label taht show the value of the band
        is affecting for the scale bar
        '''
        def set_bar(bar_1):
            (self.varla11).set(bar_1)
        '''function to set the value of the band definida for the variable varla11
        when the user enter the number of the band directamente that affect to change
        ...
        '''function to set scaling, affect only to the image of the FITS file
        the image should be clear to apply a scaling
        '''    
        def set_scaling():
            if (self.flag_file)==1:
        ...
        or apply the invert color map, depends for the variable maps_array and maps_array_inv
        ...
        def set_color_map(event=''):
            if (self.flag_file)==1:
                ...
                MessageBox.showerror("Error!","Please, first choose a file") 
        '''
        Funtion to clean the marks of the wavelenth
        '''
        ...
        '''
        Funtion to draw one or many lines in the graph for mark a value to the wavelength
        the sintax is number separaters by a coma
        example: 400,500
        the values should be
        minimun value of the array lambda <= x <= maxinum value of the array lambda
        '''
        ...
        def mark_wavelength():
            if (self.flag_file)==1:
                ...
                MessageBox.showerror("Error!","Please, first choose a file")
        '''
        Funtion to create the labels for show the values of the image FITS in arcsec 
   ...
   #     print("This script file path is ", filePath)
    ...
    #    imprimir()
     ...
     #   combo1.config(bg="#E6E6FA")
        ...
        (self.canvas2).get_tk_widget().place_configure(x=840, y=270, width=440, height=410)
