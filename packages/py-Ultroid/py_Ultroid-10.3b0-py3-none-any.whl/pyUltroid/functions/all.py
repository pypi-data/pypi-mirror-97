...
async def upload_file(http, file_path, file_name, mime_type, event, parent_id):
    ...
    file.http.redirect_codes = file.http.redirect_codes - {308}
    times = time.time()
    response = None
    display_message = ""
...
async def safeinstall(event):
    if event.reply_to_msg_id:
        ...
        ok = await eor(
            event, f"Please use `{Var.HNDLR}install` as reply to a .py file."
        )
        await asyncio.sleep(4)
        await ok.delete()
