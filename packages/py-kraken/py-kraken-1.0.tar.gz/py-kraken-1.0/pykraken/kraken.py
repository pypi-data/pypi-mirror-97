...
class Kraken:
    ...
    def get_download_link(self, page_link: str) -> str:
        ...
        dl_link_resp = self.session.post(f"{self.KRAKEN_BASE_URL}/download/{hash}", data=payload, headers=headers)
        dl_link_json = dl_link_resp.json()

        if self.URL_KEY in dl_link_json:
            return dl_link_json[self.URL_KEY]
        else:
            raise LinkPostFailure(f"Failed to acquire download URL from kraken for page_link: {page_link}")
