...
def main() -> None:
    ...
    parser.add_argument(
        "--format", type=str, default="svg", help="File format of the generated cfg"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--debug", action="store_true", default=False, help="Generate graphs at run time"
    )
    parser.add_argument(
        "--port",
        type=int,
    ...
    parser.add_argument("--diffable", type=str, help="generate diffable ast traversal")
    ...
    cfg.build_visual(
        cfg_name[:-3] + "_cfg", format=args.format, calls=args.calls, show=args.show, diffable=args.diffable
    )
