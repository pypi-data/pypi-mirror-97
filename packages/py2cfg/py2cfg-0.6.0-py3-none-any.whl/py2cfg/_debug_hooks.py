...
class QueueMGR:
    ...
    def __init__(self, mp_queue, factory, line_cache: LineCache) -> None:
        ...
        self._requests: Dict[
            Tuple[str, str, int],
            Union[Tuple[Condition, List[int]], list]
        ] = {}
...
class ProcessFactory:
    ...
    def __new__(cls, frame: FrameWrapper, func: Callable, stop: Any, continue_fn: Callable=None):
    ...
    def user_line(dbg, _frame):
        ...
        if _pid == os.getpid() and _entry_frame is not None:
            ...
            if (k := (frame.filename, frame.lineno)) in cfg_db.at_lineno:
                ...
                logger.warn(
                    f"{str(subcfg)} @ {frame.filename}:{frame.lineno}"
                )
                threading.Thread(
                    target=dispatch_cfg,
                    args=(subcfg,),
