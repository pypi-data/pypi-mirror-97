...
class Link(object):
    ...
    def __init__(
        self, source: Block, target: Block, exitcase: Optional[ast.Compare] = None
    ) -> None:
...
class CFG(object):
    ...
    def _build_key_subgraph(self, format: str) -> Digraph:
        ...
        for typeobj, (shape, color) in zip(
            ...
            key_subgraph.node(
                ...
                _attributes={"shape": shape, "style": "filled", "fillcolor": color},
            )
    ...
    def _build_visual(self, format: str = "pdf", calls: bool = True, interactive: bool = False, build_own=False) -> Digraph:
        graph = gv.Digraph(
    ...
    def outline_block(self, lineno, lst=None) -> int:
        ...
        if (block := self.bsearch(lineno, lst)) is None:
            return -1
    ...
    def highlight_blocks(self, lines, blocks=None) -> int:
        ...
        for line in list(lines):
            if (block := self.bsearch(line, blocks)) is not None: #type: ignore
