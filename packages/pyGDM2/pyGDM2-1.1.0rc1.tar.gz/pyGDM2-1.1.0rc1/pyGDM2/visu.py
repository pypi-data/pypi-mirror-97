...
def vectorfield(NF, struct=None, projection='auto', complex_part='real', 
    ...
    if slice_level is not None:
        if slice_level == -9999: 
            slice_level = np.min(SLICE)
        elif slice_level == 9999: 
            slice_level = np.max(SLICE)
        else:
...
def vectorfield_color(NF, struct=None, projection='auto', complex_part='real', 
    ...
    if projection.lower() == 'xy':
        if slice_level == -9999: 
            slice_level = np.min(Z)
        elif slice_level == 9999: 
            slice_level = np.max(Z)
        else:
            slice_level_closest = np.unique(Z)[np.argmin(np.abs(np.unique(Z) - slice_level))]
            if slice_level_closest != slice_level:
    ...
    if projection.lower() == 'xz':
        if slice_level == -9999: 
            slice_level = np.min(Y)
        elif slice_level == 9999: 
            slice_level = np.max(Y)
        else:
            slice_level_closest = np.unique(Y)[np.argmin(np.abs(np.unique(Y) - slice_level))]
            if slice_level_closest != slice_level:
                warnings.warn("slice level Y={} contains no meshpoints. Using closest level containing meshpoint at Y={}".format(
                                              slice_level, slice_level_closest))
                slice_level = slice_level_closest
    ...
    if projection.lower() == 'yz':
        if slice_level == -9999: 
            slice_level = np.min(X)
        elif slice_level == 9999: 
            slice_level = np.max(X)
        else:
            slice_level_closest = np.unique(X)[np.argmin(np.abs(np.unique(X) - slice_level))]
            if slice_level_closest != slice_level:
                warnings.warn("slice level X={} contains no meshpoints. Using closest level containing meshpoint at X={}".format(
                                              slice_level, slice_level_closest))
                slice_level = slice_level_closest
        XYZList = np.transpose([X[(Y == slice_level)],Z[(Y == slice_level)], EF[(Y == slice_level)]])
