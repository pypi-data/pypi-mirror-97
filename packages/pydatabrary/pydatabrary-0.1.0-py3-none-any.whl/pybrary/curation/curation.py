...
from pybrary.curation.asset import Asset

from pybrary.utils import utils
from pybrary.curation.container import Container
from pybrary.curation.record import Record
from pybrary.curation.types.category import Category
...
class Curation:
        ...
        for root, dirs, files in os.walk(folder):
            for idx, file in enumerate(files):
                try:
                    ...
                    assets.append(Asset(file_path, id=idx).to_dict(template=True))
                except Exception as e:
                    warnings.warn('Error in file {} - '.format(file, e))

        utils.dump_into_csv(assets, output)
            ...
            for i in range(value):
                records.append(
                    Record(
                        ...
                        key="{}".format(idx),
                    ).to_dict(template=True)
                )
                idx = idx + 1

        utils.dump_into_csv(records, output)
    ...
    def generate_containers_list(value=1, output=None):
        ...
        containers = []
        for i in range(value):
            containers.append(Container(key="{}".format(i)).to_dict(template=True))

        utils.dump_into_csv(containers, output)
