...
from lambdex.ast_parser import find_lambdex_ast_in_code, LambdexASTLookupKey, LambdexASTLookupTable
...
class _LambdexBlock:
    ...
    __slots__ = (
        ...
        # Should be the offset of MAKE_TUPLE, or None if no closure built
...
def _find_lambdex_blocks(code: types.CodeType) -> Sequence[_LambdexBlock]:
    ...
    for offset, op, arg in dis._unpack_opargs(code.co_code):
        ...
        if op in {LOAD_GLOBAL, LOAD_NAME} and names[arg] == 'def_':
        ...
        if not blocks or curr_block.offset_jump is not None:
            ...
            curr_block.freevars.append(cellvars[arg])
        elif op == BUILD_TUPLE and prev_op == LOAD_CLOSURE:
            # If building the closure tuple, record the offset
...
def _rewrite_code(
    ...
    3) make sure that the function itself is the last element of the closure tuple.
       This is done by generating closure tuple containing the cellvar mentioned above
       if no closure needed in the original code, or otherwise, appending the cellvar at
       the back of the closure tuple;
    ...
    while rblocks or hblocks:
        ...
        if rblocks and offset >= rblocks[-1].offset_start:
            ...
            lambdex_code, _ = _compile(astdef, code.co_filename, curr_block.freevars)
        ...
        if curr_block is None:
            ...
            # If reached MAKE_TUPLE, push the new cellvar
            new_instrs.append(_Instruction(LOAD_CLOSURE, extra_cellvar_idx))
            first_added_instr = new_instrs[-1]

            new_instrs.append(_Instruction(BUILD_TUPLE, curr_instr.arg + 1))
            ...
            new_instrs.append(_Instruction(STORE_DEREF, extra_cellvar_idx))
...
def _make_lnotab(instrs: Sequence[_Instruction], firstlineno: int) -> bytes:
    """
...
def transpile(code: types.CodeType, asttab: Optional[LambdexASTLookupTable] = None) -> types.CodeType:
    """
    Recursively rewrite `code` to eliminate lambdex runtime compiling.
    """
    if asttab is None:
        asttab = find_lambdex_ast_in_code(code)
