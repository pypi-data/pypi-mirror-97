...
class NN():
    ...
    def preprocess(self, descriptors, features):
        ...
        # Descriptors in CuPy mode.
        for i in range(self.no_of_structures):
            for element in self.elements:
                self.x[i][element] = cp.asarray(self.x[i][element])
                if self.use_force:
                    self.dxdr[i][element] = cp.asarray(self.dxdr[i][element])

        self.energy = features['energy']
        self.force = features['force']
    ...
    def calculate_loss(self, batch, lossprime):
        ...
        energyloss = 0.
        forceloss = 0.
        dEnergydWeights = np.zeros((self.ravel.count,))
        dForcedWeights = np.zeros((self.ravel.count,))
        dLossdW = None
        ...
        for i in batch:
            ...
            if self.dxdr[i] is not None:
                ...
                forceloss += self.softmax[i] * np.sum((nnForce - true_force) ** 2.) \
                                * fc / 3. / no_of_atoms
            ...
            if lossprime:
                ...
                # dFdW
                if self.dxdr[i] is not None:
                    temp = np.zeros(self.ravel.count)
        ...
        flat_weights = self.ravel.to_vector(W0)
        regTerm = self.alpha * np.sum(flat_weights**2)
    ...
    def calculate_energy_forces(self, x, dxdr=None, w=None):
        ...
        energy = 0.
        forces = cp.zeros([no_of_atoms, 3])
        o, ohat, aprime, aprime2, dodx, dohatdx = self.forward(x, w)
        
        for e in elements:
            _slope, _intercept = scalings[e]['slope'], scalings[e]['intercept']
            energy += cp.sum(_slope * o[e][len(o[e])-1] + _intercept)
            if dxdr is not None:
                dEdx = cp.sum(dodx[e][len(dodx[e])-1], axis=1)
                forces += - _slope * cp.einsum('ik, ijkl->jl', dEdx, dxdr[e]) 
    ...
    def calculate_nnResults(self, x, dxdr=None, ravel_count=None):
        ...
        dnnEnergydWeights = cp.zeros(ravel_count)
        dnnForcedWeights = cp.zeros((no_of_atoms, 3 , ravel_count))
        ...
        for e in elements:
            ...
            for i in range(1, len(W[e])+1):
                # dE/dW
                dEdW[e][i] *= _slope
                Etemp = dEdW[e][i].ravel()
                _size = len(Etemp)
                dnnEnergydWeights[wcount:(wcount+_size)] += Etemp
                
                # dF/dW
                if dxdr is not None:
                    s1, s2, s3, s4 = dFdW[e][i].shape
                    dFdW[e][i] *= - _slope
        ...
        return cp.asnumpy(energy), cp.asnumpy(forces), \
                cp.asnumpy(dnnEnergydWeights), cp.asnumpy(dnnForcedWeights)
    ...
    def forward(self, x, w=None):
        ...
        for e in elements:
            ...
            o[e][0] = _x
            dohatdx[e][0] = cp.zeros([_x.shape[0], _x.shape[1]+1, _x.shape[1]])
            dodx[e][0] = cp.zeros([_x.shape[0], _x.shape[1], _x.shape[1]])
            for i in range(_x.shape[1]):
                dodx[e][0][:, i,i] = 1
                dohatdx[e][0][:, i,i] = 1

            ohat[e][0] = cp.hstack((_x, cp.ones((len(_x), 1))))
            aprime[e][0] = None
            aprime2[e][0] = None
            N_layer = len(_W)
            for i in range(N_layer):
                term = cp.dot(ohat[e][i], _w[i+1])
                o[e][i+1] = ACTIVATION(term, _activation[i], 0)
                N_x = len(o[e][i+1]) # number of neurons
                ohat[e][i+1] = cp.hstack((o[e][i+1], cp.ones([N_x, 1])))
                aprime[e][i+1] = ACTIVATION(o[e][i+1], _activation[i], 1)


                if i == 0:
                    dodx[e][i+1] = cp.einsum('ij,jk->ijk', 
                                             aprime[e][i+1], _W[i+1].T)
                else:
                    term = cp.einsum('ij,jk->ijk', aprime[e][i+1], _W[i+1].T)
                    dodx[e][i+1] = cp.einsum('ijk,ikl->ijl', term, dodx[e][i])
                
                #used for only force training
                aprime2[e][i+1] = ACTIVATION(o[e][i+1], _activation[i], 2)
                if i < N_layer-1:
                    dohatdx[e][i+1] = cp.zeros([_x.shape[0], 
                                                o[e][i+1].shape[1]+1, 
                                                _x.shape[1]])
    ...
    def backprop(self, o, ohat, aprime, aprime2, dodx, dohatdx, 
        ...
        for e in elements:
            ...
            for i in range(N, 0, -1):
                term = cp.einsum('jk,ikl->ijl', _w[i].T, dodx[e][i-1])
                if i == N:
                    delta[e][i] = aprime[e][i]
                    if _dxdr is not None:
                        delta_prime[e][i] = cp.einsum('ij,ijk->ijk', 
                                                      aprime2[e][i], term)
                else:
                    delta[e][i] = cp.dot(delta[e][i+1], _w[i+1].T) * aprime[e][i] 
                    if _dxdr is not None:
                        term0 = cp.einsum('ijk,jl->ilk', 
                                          delta_prime[e][i+1], _w[i+1].T)
                        term1 = cp.einsum('ijk,ij->ijk', term0, aprime[e][i])
                        term2 = cp.dot(delta[e][i+1], _w[i+1].T) * aprime2[e][i]
                        term3 = cp.einsum('ij,ijk->ijk', term2, term)
                        delta_prime[e][i] = term1 + term3

                dEdW[e][i] = cp.einsum('ij,ik->kj', delta[e][i], ohat[e][i-1])

                if _dxdr is not None:
                    term1 = cp.einsum('ij,ikl->ikjl', 
                                      delta[e][i], dohatdx[e][i-1])
                    term2 = cp.einsum('ijk,il->iljk', 
                                      delta_prime[e][i], ohat[e][i-1])
                    d2EdxdW[e][i] = term1 + term2
                    dFdW[e][i] = cp.einsum('ijkl,imln->mnjk', 
                                 d2EdxdW[e][i], _dxdr, optimize='greedy')
    ...
    def clean_bias(self, weights):
        ...
        for key in weights.keys():
            ...
            for i in range(len(weights[key])):
                    ...
                    _WEIGHTS[key][i+1] = cp.vstack((tmp, 
                                                    cp.zeros([1,tmp.shape[1]])))
...
class Raveler:
    ...
    def to_vector(self, weights,):
        ...
        for k in self.weightskeys:
            lweights = cp.asnumpy(weights[k['key1']][k['key2']]).ravel()
    ...
    def to_dicts(self, vector):
        ...
        for k in self.weightskeys:
            ...
            weights[k['key1']][k['key2']] = cp.asarray(matrix)
            count += k['size']
...
def ACTIVATION(x, method, order=0):
    ...
    def linear(x, order=0):
        if order == 0:
            ...
            return cp.ones(x.shape)
        elif order == 2:
            return cp.zeros(x.shape)
