...
torch.set_printoptions(edgeitems=55)
...
class NN():
    ...
    def preprocess(self, descriptors, features):
        ...
        # The total atoms in the unit cells for all structures.
        self.no_of_atoms = {}
        self.max_atoms = {}
        for ele in self.elements:
            self.max_atoms[ele] = 0
            self.no_of_atoms[ele] = np.zeros((self.no_of_structures,))
        for i in range(self.no_of_structures):
            for key in self.x[i].keys():
                self.no_of_atoms[key][i] += len(self.x[i][key])
                if len(self.x[i][key]) > self.max_atoms[key]:
                    self.max_atoms[key] = len(self.x[i][key])
        ...
        for e in self.elements:
            ...
            # ohat
            self.ohat[e] = {}
            self.ohat[e][0] = torch.cuda.FloatTensor(self.max_atoms[e], shp0+1)
            self.ohat[e][0][:, -1] = 1.
            
            # dodx0
            self.dodx0[e] = torch.cuda.FloatTensor(self.max_atoms[e], shp0, shp0).fill_(0)
            self.dodx0[e][:, np.arange(shp0), np.arange(shp0)] = 1.

            # dohatdx
            self.dohatdx[e] = {}
            self.dohatdx[e][0] = torch.cuda.FloatTensor(self.max_atoms[e], shp0+1, shp0).fill_(0)
            ...
            for i in range(len(self.weights[e])-1):
                ...
                # ohat
                self.ohat[e][i+1] = torch.cuda.FloatTensor(self.max_atoms[e], shp+1).fill_(0)
                self.ohat[e][i+1][:, -1] = 1.
                
                # dohatdx
                self.dohatdx[e][i+1] = torch.cuda.FloatTensor(self.max_atoms[e], shp+1, shp0).fill_(0)
    ...
    def forward(self, x, w=None):
        ...
        for e in elements:
            ...
            ohat[e][0] = self.ohat[e][0][:_x.shape[0], :]
            ohat[e][0][:, :-1] = _x

            dodx[e][0] = self.dodx0[e][:_x.shape[0], :, :]
            dohatdx[e][0] = self.dohatdx[e][0][:_x.shape[0], :, :]
            ...
            for i in range(N_layer):
                ...
                if i < N_layer-1:
                    ...
                    ohat[e][i+1] = self.ohat[e][i+1][:N_x, :]
                    ohat[e][i+1][:, :-1] = o[e][i+1]

                    dohatdx[e][i+1] = self.dohatdx[e][i+1][:N_x, :, :]
                    
                    #torch.cuda.FloatTensor(N_x, shp+1, _x.shape[1]).fill_(0)
