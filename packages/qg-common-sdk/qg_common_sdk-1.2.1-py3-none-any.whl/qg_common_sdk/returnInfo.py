...
def return_info_func(func):
    ...
    def wrapped_function(*args, **kwargs):
        ...
        operate_log["parameter"] = jsonable_encoder(
        ...
        try:
            ...
            return_data = {"data": result,
                           "code": http_status_code, "msg": "操作成功"}
            operate_log["workTime"] = str(process_time)
            if process_time > 3:
                logging.warning(f"{func.__name__} 调用时间过长", extra={
                                "operate_log": operate_log})
            else:
                # TODO 所有的日志不应该在这记录
                logging.info(f"请求成功 {operate_log}", extra={"operate_log": operate_log})
            return JSONResponse(
            ...
            logging.error(traceback.format_exc(), extra={
                          "method": func.__name__, "params": args, "exec": e.data, "operate_log": operate_log})
            exception = e
            ...
            logging.exception(e)
            logging.error(traceback.format_exc(), extra={
                          "method": func.__name__, "params": args, "operate_log": operate_log})
            exception = e
        except Exception as e:
            http_status_code = StatusCode.INTERNAL_SERVER_ERROR
            operate_log["status"] = http_status_code
            return_data = {"data": None,
                           "code": http_status_code, "msg": str(e)}
            logging.exception(e)
            logging.error(traceback.format_exc(), extra={
                          "method": func.__name__, "params": args, "operate_log": operate_log})
            return JSONResponse(
...
async def add_process_time_header(request: Request, call_next):
    try:
        ...
        if not has_request_id:
            ...
            scope.get('headers').append(
                (str('requestid').encode(), request_id.encode()))
...
async def unicorn_exception_handler(request: Request, exc: ValueError) -> Response:
    http_status_code = StatusCode.INTERNAL_SERVER_ERROR
    return_data = {"data": exc.args[0],
                   "code": http_status_code, "msg": exc.args[0]}
    return JSONResponse(
        status_code=http_status_code,
        content=jsonable_encoder(return_data),
    )
