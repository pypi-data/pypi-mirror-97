...
def qz_fetch_get_stock_list_tushare(list_status='l'):  
    if list_status=='l':
        ...
        stock_list=pd.concat([stock_list,stock_list_P],axis=0) # L上市 D退市 P暂停上市
    stock_list.rename(columns={'symbol':'code'}, inplace=True)
...
def qz_fetch_get_stock_trade_cal_tushare():   
    trade_cal=pro.query('trade_cal')    
    return trade_cal

@retry(stop_max_attempt_number=10, wait_random_min=50, wait_random_max=1000)
def qz_fetch_get_stock_day_tushare(ts_code='000001.SZ',start_date='20180701',end_date=datetime.datetime.now().strftime("%Y%m%d" ),trade_date=None):
    try:
    ...
    if trade_date:
        ...
        res=pd.merge(df_daily,df_factor,how='left').sort_values(by = 'ts_code')
    else:
        df_daily=pro.query('daily', ts_code=ts_code, start_date=start_date, end_date=end_date).sort_values(by = 'ts_code')
        df_factor=pro.query('adj_factor', ts_code=ts_code)
        res=pd.merge(df_daily,df_factor,how='left').sort_values(by = 'trade_date')      
...
@retry(stop_max_attempt_number=10, wait_random_min=50, wait_random_max=1000)
def qz_fetch_get_stock_daily_basic_tushare(ts_code='000001.SZ',start_date='20180701',end_date=datetime.datetime.now().strftime("%Y%m%d" ),trade_date=None):
    try:
        trade_date = trade_date.replace('-', '') #兼容设置以防日期格式为2001-10-20格式
        start_date = start_date.replace('-', '')
        end_date = end_date.replace('-', '') 
        ...
        #print("{}  {}。".format(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),lastEx))
    if trade_date:
        res=pro.query('daily_basic',trade_date=trade_date).sort_values(by = 'ts_code')
    else:
        res=pro.query('daily_basic', ts_code=ts_code, start_date=start_date, end_date=end_date).sort_values(by = 'ts_code')     
    res['code']=res['ts_code'].apply(lambda x:x[:6]) #x[7:9].lower()
    res.rename(columns={'trade_date':'date'}, inplace=True)
    res['date'] =[ i[0:4]+'-'+i[4:6]+'-'+i[6:8]  for i in res['date']] 
    res = res.assign(
...
def qz_fetch_get_index_list_tushare():   
...
if __name__ == '__main__': 
    ...
    stock_list=qz_fetch_get_stock_list_tushare()
    trade_cal=qz_fetch_get_stock_trade_cal_tushare()
    stock_day=qz_fetch_get_stock_day_tushare()
    daily_basic=qz_fetch_get_stock_daily_basic_tushare()
    index_list=qz_fetch_get_index_list_tushare()
    """需要对于datetime 和date 进行转换, 以免直接被变成了时间戳"""
    if 'datetime' in data.columns:
        data.datetime = data.datetime.apply(str)
    if 'date' in data.columns:
        data.date = data.date.apply(str)
    json_data = json.loads(stock_day.reset_index().to_json(orient='records')) 
    json_data2 = json.loads(stock_day.reset_index().to_json()) 
