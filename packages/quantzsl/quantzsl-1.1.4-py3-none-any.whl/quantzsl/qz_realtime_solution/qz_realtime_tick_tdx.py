...
#        self.type=_type
    ...
    def get_ticks(self):
        while 1:
            ...
            if aa>0:
                break            
            print("{}  通达信可用ip数量为：{}，重试中.".format(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),self.QA_Tdx_Executor._queue.qsize()))
        while 1:                
    ...
    #                        self._queue.qsize()))
            ...
            data2=data_
            if len(data2.columns)<20:
                pass
                print("{}  通达信获取数据失败，重试中.".format(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
        
            else:
                data2=data2.sort_values(by="code" )
    ...
    def get_ticks_continue(self):
        ...
        return data                          
    def run(self):
        threading.Thread(target=self.get_ticks_continue).start()    
