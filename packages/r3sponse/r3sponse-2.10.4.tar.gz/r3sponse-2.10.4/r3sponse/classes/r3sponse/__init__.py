...
# imports.
from Response.classes.config import *
from Response.classes import utils
from Response.classes.utils import color, symbol
from Response.classes import exceptions
	...
	def log(self, 
		...
		try:
			...
			if "not supported between instances of 'dict' and 'int'" in f"{e}":
				raise TypeError(f"You most likely returned a Response.error_response when you meant a Response.success_response, error: {e}")
			else:
				raise TypeError(e)
		if comparison:
	...
	def __init__(self, 
		...
		if json != None:
			if isinstance(json, str):
				...
				raise exceptions.InvalidUsage("The Response.ResponseObject json parameter must be str / dict format.")
		else:
