...
class R3sponse(object):
	...
	def response(self, 
		...
		# do not serialize (always keep default).
		if not safe:
			return ResponseObject(response)

		# serialize the response (serialize from dumped json).
		else:
...
class Parameters(object):
	...
	def get(self, 
		...
		#	list instance: return a parameters object class & return an error response when a parameter is undefined.
		#	dict instance: return a parameters object class & return the parameter's value from the dict as a default when undefined.
		parameters=[], 
		...
		if request == None:
			raise exceptions.InvalidUsage("<r3sponse.paramters.get>: Define parameter: [request].")
		
		# single parameter.
		if isinstance(parameters, (str,String)):
			parameters = str(parameters)
			response = r3sponse.response()
			format = None
			if ":" in parameters:
			...
			if variable in ["", None]:
				if traceback != None:
					return variable, r3sponse.error(f"{traceback}: Define parameter: [{parameters}].")
				else:
					return variable, r3sponse.error(f"Define parameter: [{parameters}].")
			elif format != None:
				if format in ["str", "string"]: variable = str(variable)
					...
					raise ValueError(f"Unrecognized <r3sponse.parameters.get> format: {format}.")
			
			# handler.
			return variable, r3sponse.success(f"Succesfully retrieved request parameter [{parameters}].", {
				"key":parameters,
				"value":variable,
			})
		...
		# list recursive.
			...
			return params, r3sponse.success(f"Succesfully retrieved {len(params)} request parameter(s).")
	...
	def check(self, 
		...
		if isinstance(parameters, tuple):
			...
			if parameter == default: 
				if traceback != None:
					return r3sponse.error(f"{traceback}: Define parameter [{name}].")
				else:
					return r3sponse.error(f"Define parameter [{name}].")
			if ":" in name:
				...
				if param_format not in formats:
					return r3sponse.error(f"Incorrect parameter [{name}:{param_format}] format, correct format(s): {Array(path=False, array=formats).string(joiner=', ')}.")
			return r3sponse.success(f"Succesfully checked parameter [{name}].")
		...
		# recursive.
			...
			return r3sponse.success(f"Succesfully checked {len(parameters)} parameter(s).")
		# invalid.
		else: 
...
# teh response & parameters object class.
...
# initialized objects.
# must be initialized as r3sponse since Parameters requires it.
r3sponse = R3sponse()
