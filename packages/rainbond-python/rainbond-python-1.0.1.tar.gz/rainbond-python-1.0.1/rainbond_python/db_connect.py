...
class DBConnect:
    ...
    def find_paging(self, parameter: Parameter) -> dict:
        ...
        # offset表示当前页第一条在记录集中的位置，接收即可，无需处理
        param = parameter.verification(
            checking=checking,
            verify={
                'limit': int, 'current': int, 'select_list': list, 'orderby_list': list,
                'filter_list': list, 'start_date': str, 'end_date': str, 'date_type': str,
                'offset': int,
            },
            optional={'start_date': '', 'end_date': '', 'date_type': 'update_time', 'offset': 0})
        try:
            limit = int(param['limit'])  # 每页条数
            current = int(param['current'])  # 当前页数
            if current < 1 or limit < 1:
                raise Exception('当前页数和每页条数都从 1 开始计算')
            else:
                current -= 1
            select_list = param['select_list']  # 受控列
            # 排序顺序（对应受控列），asc=升序，desc=降序
            orderby_list = param['orderby_list']
            # 筛选值（对应受控列）
            filter_list = param['filter_list']
            start_date = handle_date(date=param['start_date'])  # 可选——开始日期
            # 可选——结束日期，与开始日期组成日期区间
            end_date = handle_date(date=param['end_date'], date_type='end')
            if len(select_list) != len(orderby_list) or len(select_list) != len(filter_list):
            ...
            if len(select_list):
                for i in range(len(select_list)):
                    ...
                    if type(filter_list[i]) == int:
                        find_dict[select_list[i]] = filter_list[i]
                    elif select_list[i] == 'id' or select_list[i] == '_id':
                        find_dict['_id'] = ObjectId(str(filter_list[i]))
                    else:
                        if filter_list[i].strip() != '':
                            find_dict[select_list[i]] = {
                                '$regex': filter_list[i]}
                    # 整理请求参数中的筛排序列表
                    if type(orderby_list[i]) != str:
                        abort(Response('计算组件分页参数异常: 排序参数不支持 int 类型', 400, {}))
                    sort_value = orderby_list[i].strip()
                    if sort_value == 'asc':
                        sort_list.append((select_list[i], pymongo.ASCENDING))
                    elif sort_value == 'desc':
                        sort_list.append((select_list[i], pymongo.DESCENDING))
                    elif sort_value == '':
                        pass
                    else:
            ...
            if find_data.count():
                ...
                return {
                    'total': self.mongo_collection.estimated_document_count() - remove_count,
                    'filtered': records_filtered,
                    'items': query_result,
                    'pages': math.ceil(records_filtered / limit),
