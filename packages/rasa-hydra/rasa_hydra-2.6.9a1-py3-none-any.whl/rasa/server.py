...
def create_app(
    ...
    async def ready(request: Request):
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                results = await asyncio.gather(*[fetch(session, url, timeout) for url in app.health_check_urls])
                duration = time.perf_counter() - start
                ...
                elif endpoints.action and (not services_status[0] or services_status[0] != 200):
                    raise ErrorResponse(500, "DucklingConnectionError", "Rasa Duckling is not ready yet.")
                elif endpoints.duckling and (not services_status[1] or services_status[1] != 200):
                    raise ErrorResponse(500, "ActionConnectionError", "Action server is not ready yet.")
                elif endpoints.idp and (not services_status[2] or services_status[2] != 200):
                    raise ErrorResponse(500, "IDPConnectionError", f'IDP is not ready yet.')
                elif endpoints.upm and (not services_status[3] or services_status[3] != 200):
    ...
    async def model(request: Request, fileName):
        try:

            with open('data/' + fileName) as f:
                searchTitle = re.search(r'<!---\s+(.*?)\s+--->', f.readline())
                if (searchTitle and searchTitle.group(1)):
                    body = {"fileName": searchTitle.group(1)}
    ...
    async def download_file(request: Request, fileName):
        try:
            ...
            body = {"message": "failed to retrieve nlu file: " + fileName }
            return response.json(body=body, status=500)
