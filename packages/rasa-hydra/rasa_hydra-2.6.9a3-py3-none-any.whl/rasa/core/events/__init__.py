...
class UserUttered(Event):
    ...
    def __init__(
        ...
        self.text = text
        self.text_redacted = self.redact_msg(self.text)
    ...
    def normalize_redaction(self, word):
        if '{REDACTED}' in word:
            return '****'
        else:
            return word

    def redact_msg(self, message):
        if message:
            text_numeric = t2d.convert(message)
            phrase_arr = re.sub('\d', '{REDACTED}', text_numeric).split()
            res = ' '.join(list(map(self.normalize_redaction, phrase_arr)))
            return res
        else:
            return message
