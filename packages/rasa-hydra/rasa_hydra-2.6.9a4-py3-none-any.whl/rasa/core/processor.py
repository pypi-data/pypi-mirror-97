...
class MessageProcessor:
    ...
    def create_metadata(
        ...
        if metadata and metadata.get("errorMessage"):
            metadata["error"] = True
    ...
    def log_user_msg(self, message: UserMessage, tracker: DialogueStateTracker):
        ...
        elif '/greet' in message.text: 
            logger.info(f"[{tracker.sender_id}] - User msg: {message.text}")
        else:
            redacted_msg = None
            try:
                redacted_msg = UserUttered.redact_msg(message.text)
            except:
                redacted_msg = message.text

            logger.info(f"[{tracker.sender_id}] - User msg: {redacted_msg}")
    ...
    async def _handle_message_with_tracker(
        ...
        tracker.update(
            UserUttered(
                ...
                text_redacted=UserUttered.redact_msg(message.text)
            ),
            self.domain,
        )
