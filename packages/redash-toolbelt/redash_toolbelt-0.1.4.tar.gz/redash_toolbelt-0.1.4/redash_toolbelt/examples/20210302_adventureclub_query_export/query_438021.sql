/*
Name: Subscriptions without recent orders
Data source: 30635
Created By: Jeremy Bueler
Last Update At: 2020-11-23T21:36:52.042Z
*/
WITH subs AS (
    SELECT 
    subscriptions.id,
   ...
   FROM accounts_service.subscriptions subscriptions
   LEFT JOIN accounts_service.users users on subscriptions.user_id = users.id
...
LEFT JOIN 
    accounts_service.invoices invoices on unused_subs.id = invoices.subscription_id where invoices.created_at > unused_subs.most_recent_order_date
