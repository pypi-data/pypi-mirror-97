...
class ALT_probability_plot_Weibull:
    ...
    def __init__(
        ...
        if common_shape_method not in ["BIC", "weighted_average", "average"]:
            raise ValueError(
                "common_shape_method must be either BIC, weighted_average, or average. Default is BIC."
            )
        if len(failures) != len(failure_stress):
            raise ValueError(
                "The length of failures does not match the length of failure_stress"
            )
        ...
        if right_censored is not None:
            if len(right_censored) != len(right_censored_stress):
                raise ValueError(
                    "The length of right_censored does not match the length of right_censored_stress"
                )
        ...
        def __BIC_minimizer(common_shape_X):  # lgtm [py/similar-function]
            ...
            for stress in unique_stresses_f:
                ...
                if right_censored is not None:
                    ...
                    RIGHT_CENSORED = None
                weibull_fit_common_shape = Fit_Weibull_2P(
                    failures=FAILURES,
                    right_censored=RIGHT_CENSORED,
                    show_probability_plot=False,
                    print_results=False,
                    force_beta=common_shape_X,
                )
                BIC_tot += weibull_fit_common_shape.BIC
            return BIC_tot

        if common_shape_method == "BIC":
            optimized_beta_results = minimize(
                __BIC_minimizer, x0=common_shape_guess, method="nelder-mead"
            )
            ...
            common_shape = (
                common_shape_guess  # this was just the numerical average obtained above
            )
        ...
        for i, stress in enumerate(unique_stresses_f):
            ...
            if show_plot is True:
                weibull_fit_common_shape.distribution.CDF(
                    linestyle="--", color=color_list[i], xvals=xvals, plot_CI=False
                )  # plotting of the confidence intervals has been turned off
                Probability_plotting.Weibull_probability_plot(
                    failures=FAILURES,
                    right_censored=RIGHT_CENSORED,
                    plot_CI=False,
                    color=color_list[i],
            ...
            x, y = Probability_plotting.plotting_positions(
        ...
        beta_difs = (common_shape - np.array(weibull_fit_beta_array)) / np.array(
            weibull_fit_beta_array
        )
        beta_differences = []
        for item in beta_difs:
            if item > 0:
                ...
                beta_differences.append(str(str(round(item * 100, 2)) + "%"))
        results = {
            "stress": unique_stresses_f,
            "original alpha": weibull_fit_alpha_array,
            "original beta": weibull_fit_beta_array,
            "new alpha": weibull_fit_alpha_array_common_shape,
            "common beta": np.ones_like(unique_stresses_f) * common_shape,
        ...
        if show_plot is True:
            ...
            if common_shape_method == "BIC":
                plt.title(
                    str(
                        "ALT Weibull Probability Plot\nOptimal BIC "
                        + r"$\beta$ = "
                        + str(round(common_shape, 4))
                    )
                )
            elif common_shape_method == "weighted_average":
                plt.title(
    ...
    def __init__(
        ...
        for i, stress in enumerate(unique_stresses_f):
            ...
            if right_censored is not None:
                ...
                RIGHT_CENSORED = None
            exponential_fit = Fit_Exponential_1P(
                failures=FAILURES,
                right_censored=RIGHT_CENSORED,
                show_probability_plot=False,
                print_results=False,
            )
            weib_fit = Fit_Weibull_2P(
                failures=FAILURES,
                right_censored=RIGHT_CENSORED,
                show_probability_plot=False,
                print_results=False,
                force_beta=np.average(weibull_fit_beta_array),
            )
            ...
            if show_plot is True:
                exponential_fit.distribution.CDF(
                    linestyle="--", color=color_list[i], xvals=xvals, plot_CI=False
                )  # plotting of the confidence intervals has been turned off
                Probability_plotting.Weibull_probability_plot(
                    failures=FAILURES,
        ...
        beta_difs = (1 - np.array(weibull_fit_beta_array)) / np.array(
            weibull_fit_beta_array
        )
        beta_differences = []
        for item in beta_difs:
        ...
        results = {
            "stress": unique_stresses_f,
            "weibull alpha": weibull_fit_alpha_array,
            "weibull beta": weibull_fit_beta_array,
            "new 1/Lambda": 1 / np.array(exponential_fit_lambda_array),
            "common shape": np.ones_like(unique_stresses_f),
        ...
        if self.BIC_sum > self.BIC_sum_weibull:
            colorprint(
                "WARNING: The Weibull distribution would be a more appropriate fit for this data set as it has a lower BIC (using the average method to obtain BIC) than the Exponential distribution.",
                text_color="red",
            )
...
class ALT_probability_plot_Lognormal:
    ...
    def __init__(
        ...
        for stress in unique_stresses_f:
            ...
            weights_array.append(len_f + len_rc)
            lognormal_fit = Fit_Lognormal_2P(
                failures=FAILURES,
                right_censored=RIGHT_CENSORED,
                show_probability_plot=False,
                print_results=False,
        ...
        def __BIC_minimizer(common_shape_X):  # lgtm [py/similar-function]
            ...
            for stress in unique_stresses_f:
                ...
                if right_censored is not None:
                    ...
                    RIGHT_CENSORED = None
                lognormal_fit_common_shape = Fit_Lognormal_2P(
                    failures=FAILURES,
                    right_censored=RIGHT_CENSORED,
                    show_probability_plot=False,
                    print_results=False,
                    force_sigma=common_shape_X,
                )
                BIC_tot += lognormal_fit_common_shape.BIC
            return BIC_tot

        if common_shape_method == "BIC":
            optimized_sigma_results = minimize(
                __BIC_minimizer, x0=common_shape_guess, method="nelder-mead"
            )
        ...
        for i, stress in enumerate(unique_stresses_f):
            ...
            if show_plot is True:
                lognormal_fit_common_shape.distribution.CDF(
                    linestyle="--", color=color_list[i], xvals=xvals, plot_CI=False
                )  # plotting of the confidence intervals has been turned off
                Probability_plotting.Lognormal_probability_plot(
                    failures=FAILURES,
                    right_censored=RIGHT_CENSORED,
                    plot_CI=False,
                    color=color_list[i],
        ...
        if AICc is True:
            ...
            self.AICc_sum = "Insufficient Data"
        sigma_difs = (common_shape - np.array(lognormal_fit_sigma_array)) / np.array(
            lognormal_fit_sigma_array
        )
        sigma_differences = []
        ...
        results = {
            "stress": unique_stresses_f,
            "original mu": lognormal_fit_mu_array,
            "original sigma": lognormal_fit_sigma_array,
            "new mu": lognormal_fit_mu_array_common_shape,
            "common sigma": np.ones_like(unique_stresses_f) * common_shape,
        ...
        if show_plot is True:
            ...
            if common_shape_method == "BIC":
                plt.title(
                    str(
                        "ALT Lognormal Probability Plot\nOptimal BIC "
                        + r"$\sigma$ = "
                        + str(round(common_shape, 4))
                    )
                )
            ...
            probability_plot_xylims(
                x=x_array, y=y_array, dist="lognormal", spacing=0.05
            )
            probability_plot_xyticks()
            plt.tight_layout()
...
class ALT_probability_plot_Normal:
    ...
    def __init__(
        ...
        def __BIC_minimizer(common_shape_X):  # lgtm [py/similar-function]
            ...
            for stress in unique_stresses_f:
                ...
                if right_censored is not None:
                    ...
                    RIGHT_CENSORED = None
                normal_fit_common_shape = Fit_Normal_2P(
                    failures=FAILURES,
                    right_censored=RIGHT_CENSORED,
                    show_probability_plot=False,
                    print_results=False,
        ...
        for i, stress in enumerate(unique_stresses_f):
            ...
            if show_plot is True:
                normal_fit_common_shape.distribution.CDF(
                    linestyle="--", color=color_list[i], xvals=xvals, plot_CI=False
                )  # plotting of the confidence intervals has been turned off
                Probability_plotting.Normal_probability_plot(
                    failures=FAILURES,
                    right_censored=RIGHT_CENSORED,
                    plot_CI=False,
                    color=color_list[i],
        ...
        if AICc is True:
            ...
            self.AICc_sum = "Insufficient Data"
        sigma_difs = (common_shape - np.array(normal_fit_sigma_array)) / np.array(
            normal_fit_sigma_array
        )
        sigma_differences = []
        for item in sigma_difs:
        ...
        results = {
            "stress": unique_stresses_f,
            "original mu": normal_fit_mu_array,
            "original sigma": normal_fit_sigma_array,
            "new mu": normal_fit_mu_array_common_shape,
            "common sigma": np.ones_like(unique_stresses_f) * common_shape,
            "sigma change": sigma_differences,
        }
        results_df = pd.DataFrame(
