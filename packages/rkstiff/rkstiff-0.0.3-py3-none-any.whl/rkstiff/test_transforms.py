...
def test_hankel():
    ...
    omega = 1./(4*(sigma**2+w**2))
    fsp2_ex = -2*omega*np.exp(-sigma*omega*HT.kr**2)*(-w*np.cos(w*omega*HT.kr**2)+sigma*np.sin(w*omega*HT.kr**2))
    fsp2 = HT.ht(f2)
    ...
    HT.rmax = 3
    f3 = np.exp(-a*HT.r)
    fsp3_ex = a*np.power(a**2+HT.kr**2,-3./2)
    fsp3 = HT.ht(f3)
