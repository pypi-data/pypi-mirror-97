...
import exceptions
import settings
from views.validator import ModelInfo
...
class ViewBase(HTTPMethodView):
    ...
    def validate(self, data, partial=False):
        options = ModelInfo.build(model_klass=self.model).validate(item=data, partial=partial)
        errors = self._validate_item(item=data, options=options, partial=partial)
        if errors:
...
class ListView(ViewBase, abc.ABC):
    ...
    def _parse_query_args(self, request: Request) -> Tuple[Dict[str, Any], int, int]:
        ...
        try:
            ...
            raise exceptions.ValidationError(f"page and page_size must be integers")
        if page < 1:
    ...
    async def perform_options(self) -> PayloadType:
        return ModelInfo.build(model_klass=self.model).as_dict
