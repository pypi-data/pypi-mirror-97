...
from savvihub.cli import utils
from savvihub.cli.commands.input.experiment import (
    ...
    volume_file_mount_callback, env_vars_callback,
)
from savvihub.cli.commands.input.project import project_name_option
from savvihub.cli.commands.input.workspace import workspace_name_option
...
from savvihub_client import (
    ProtoVolumeMountRequest,
    ProtoVolumeMountRequestSourceProject, ProtoVolumeMountRequests,
)
...
def logs(
    ...
    typer.echo(log_str)
    typer.echo(
        utils.get_full_info(workspace_name, project_name, experiment_number_or_name, 'logs')
    )
...
def run(
    ...
    typer.echo(f'Experiment {experiment.number} is running. Check the experiment status at below link')
    typer.echo(f'{WEB_HOST}/{workspace_name}/{project_name}/experiments/{experiment.number}')
...
def ls(
    ...
    """
    client = ctx.authenticated_client
    experiment = client.experiment_read(workspace_name, project_name, experiment_number_or_name)
    for volume_mount in experiment.volume_mounts.mounts:
