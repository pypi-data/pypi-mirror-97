...
from savvihub_client import ProtoVolumeMountRequest, ProtoVolumeMountRequestSourceDataset, \
...
def dataset_mount_callback(ctx: Context, dataset_mounts: List[str]) -> List[str]:
    ...
    for dataset_mount in dataset_mounts:
        ...
        mount_path, dataset_name = splitted
        if '@' in dataset_name:
            dataset_name, snapshot_ref = dataset_name.split('@', 1)
        else:
            snapshot_ref = 'latest'
        ...
        try:
            ...
            raise ExitException(f'Invalid dataset: {dataset_name}\n'
                                f'Please check your dataset exist in workspace `{workspace_name}`.')

        if snapshot_ref != 'latest':
            try:
                client.snapshot_read(dataset_obj.volume_id, snapshot_ref)
            except NotFoundAPIException:
                raise ExitException(f'Invalid dataset snapshot: {dataset_mount}\n'
