...
from savvihub.exceptions import PathException
from savvihub_client import ResponseDatasetInfo
...
def parse_remote_dataset_path(ctx: Context, remote_path: str) -> (ResponseDatasetInfo, str, str):
    ...
    try:
        return ctx.authenticated_client.dataset_read(workspace, split_snapshot[0]), snapshot, file_path
    except NotFoundAPIException:
        raise ExitException('Dataset not found!\n You should specify dataset with workspace. ex) savvihub/mnist')
...
@dataset_files_app.command(auth_required=True)
def ls(
    ctx: Context,
    ...
    dataset, snapshot, path = parse_remote_dataset_path(ctx, source_path)
    volume_file_list(ctx, dataset.volume_id, snapshot, path, recursive, directory)
...
def rm(
    ...
    source_path, is_source_remote = refine_dataset_path(source_path_arg, raise_if_not_exist=True, remote_only=True)
    dataset, snapshot, path = parse_remote_dataset_path(ctx, source_path)
    volume_file_remove(ctx, dataset.volume_id, snapshot, path, recursive)
...
def cp(
    ...
    if is_source_remote:
        source_dataset, source_snapshot, source_path = parse_remote_dataset_path(ctx, source_path)
        source_volume_id = source_dataset.volume_id
    if is_dest_remote:
        dest_dataset, dest_snapshot, dest_path = parse_remote_dataset_path(ctx, dest_path)
