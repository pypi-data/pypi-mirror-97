...
class SeeqURL:
    ...
    def __init__(self, *, protocol=None, hostname=None, port=None, route=None, folder_id=None, workbook_id=None,
                 worksheet_id=None, workstep_id=None):
    ...
    ROUTES = {
        ...
        Route.WORKBOOK_PRESENTATION: (re.compile(rf'^/present/worksheet/(?P<workbook_id>{_common.GUID_REGEX})'
                                      ...
                                      dict()),
        Route.JOURNAL_WORKSTEP_LINK: (re.compile(rf'^/links', re.IGNORECASE),
                                      {
                                          'type': 'link_type',
    ...
    def parse(url):
        ...
        if not address_match:
            raise Exception(f'URL "{url}" does not start with typical http(s)://hostname(:port)')
        ...
        for route in SeeqURL.ROUTES.keys():
            ...
            if route == Route.JOURNAL_WORKSTEP_LINK and groups['link_type'] != 'workstep':
                raise ValueError(f'Unknown JOURNAL_WORKSTEP_LINK type "{groups["link_type"]}"')
