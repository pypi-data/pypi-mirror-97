...
class WidgetLogHandler(logging.Handler):
    ...
    def concat(self, value):
        if value not in self._concat_valid:
            raise RuntimeError(f'Unrecognized concat value {value}. Valid values: {self._concat_valid}')
        self._concat = value
    ...
    def delimiter(self, value):
        if value is not None and not isinstance(value, str):
            raise RuntimeError(f'Delimiters must be strings not {type(value)}: {value}')
    ...
    def _trim_lines(self, value, side):
        ...
        if 0 < self.maximum_lines <= len(lines):
            if side == 'start':
                ...
                raise RuntimeError(f'Unknown trim side for log messages {side}')
        return value
