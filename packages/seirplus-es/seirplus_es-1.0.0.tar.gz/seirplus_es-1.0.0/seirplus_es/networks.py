...
def plot_degree_distn(graph, max_degree=None, show=True, use_seaborn=True):
    ...
    pyplot.hist(nodeDegrees, bins=range(max(nodeDegrees)), alpha=0.75, color='tab:blue', label=('Grado medio = %.1f' % meanDegree))
    pyplot.xlim(0, max(nodeDegrees) if not max_degree else max_degree)
    pyplot.xlabel('Grados')
    pyplot.ylabel('Número de Nodos')
    pyplot.legend(loc='upper right')
    if(show):
        pyplot.show()
