    ...
    def run(self, T, dt=0.1, checkpoints=None, verbose=False):
        ...
        if(not checkpoints):
            ...
            if(verbose):
                ...
                print("\t V_E = " + str(self.numQ_E[-1]))
                print("\t V_I = " + str(self.numQ_I[-1]))
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def plot(self, ax=None,  plot_S='line', plot_E='line', plot_I='line',plot_R='line', plot_F='line',
                            ...
                            dashed_reference_results=None, dashed_reference_label='referencia', 
                            shaded_reference_results=None, shaded_reference_label='referencia', 
        ...
        if(combine_Q and plot_Q_E=='stacked' and plot_Q_I=='stacked'):
            ax.fill_between(numpy.ma.masked_where(Dseries<=0, self.tseries), numpy.ma.masked_where(Dseries<=0, topstack+Dseries), topstack, color=color_Q_E, alpha=0.5, label='$V_{all}$', zorder=2)
            ...
            if(any(Q_Eseries) and plot_Q_E=='stacked'):
                ax.fill_between(numpy.ma.masked_where(Q_Eseries<=0, self.tseries), numpy.ma.masked_where(Q_Eseries<=0, topstack+Q_Eseries), topstack, color=color_Q_E, alpha=0.5, label='$V_E$', zorder=2)
            ...
            if(any(Q_Iseries) and plot_Q_I=='stacked'):
                ax.fill_between(numpy.ma.masked_where(Q_Iseries<=0, self.tseries), numpy.ma.masked_where(Q_Iseries<=0, topstack+Q_Iseries), topstack, color=color_Q_I, alpha=0.5, label='$V_I$', zorder=2)
        ...
        if(combine_Q and (any(Dseries) and plot_Q_E=='shaded' and plot_Q_E=='shaded')):
            ax.fill_between(numpy.ma.masked_where(Dseries<=0, self.tseries), numpy.ma.masked_where(Dseries<=0, Dseries), 0, color=color_Q_E, alpha=0.5, label='$V_{all}$', zorder=4)
            ...
            if(any(Q_Eseries) and plot_Q_E=='shaded'):
                ax.fill_between(numpy.ma.masked_where(Q_Eseries<=0, self.tseries), numpy.ma.masked_where(Q_Eseries<=0, Q_Eseries), 0, color=color_Q_E, alpha=0.5, label='$V_E$', zorder=4)
            ...
            if(any(Q_Iseries) and plot_Q_I=='shaded'):
                ax.fill_between(numpy.ma.masked_where(Q_Iseries<=0, self.tseries), numpy.ma.masked_where(Q_Iseries<=0, Q_Iseries), 0, color=color_Q_I, alpha=0.5, label='$V_I$', zorder=4)
        ...
        if(combine_Q and (any(Dseries) and plot_Q_E=='line' and plot_Q_E=='line')):
            ax.plot(numpy.ma.masked_where(Dseries<=0, self.tseries), numpy.ma.masked_where(Dseries<=0, Dseries), color=color_Q_E, label='$V_{all}$', zorder=6)
        else:
            if(any(Q_Eseries) and plot_Q_E=='line'):
                ax.plot(numpy.ma.masked_where(Q_Eseries<=0, self.tseries), numpy.ma.masked_where(Q_Eseries<=0, Q_Eseries), color=color_Q_E, label='$V_E$', zorder=6)
            if(any(Q_Iseries) and plot_Q_I=='line'):
                ax.plot(numpy.ma.masked_where(Q_Iseries<=0, self.tseries), numpy.ma.masked_where(Q_Iseries<=0, Q_Iseries), color=color_Q_I, label='$V_I$', zorder=6)
        ...
        #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        ax.set_xlabel('Días')
        ax.set_ylabel('Porcentaje de la población' if plot_percentages else 'Número de Individuos')
        ...
        if(legend):
            ...
            ax.legend(legend_handles[::-1], legend_labels[::-1], loc='center right', facecolor='white', edgecolor='none', framealpha=0.9, prop={'size': 12})
        if(title):
            ax.set_title(title, size=14)
        if(side_title):
            ax.annotate(side_title, (0, 0.5), xytext=(-45, 0), ha='right', va='center',
                size=14, rotation=90, xycoords='axes fraction', textcoords='offset points')
    ...
    def __init__(self, G, beta, sigma, lamda, gamma, 
        ...
        if(node_groups):
            ...
            for groupName, nodeList in node_groups.items():
                ...
                self.nodeGroupData[groupName]['numQ_sym'][0]  = numpy.count_nonzero(self.nodeGroupData[groupName]['mask']*self.X==self.Q_sym)
                self.nodeGroupData[groupName]['numQ_asym'][0] = numpy.count_nonzero(self.nodeGroupData[groupName]['mask']*self.X==self.Q_asym)
            ...
            if(print_interval):
                if(print_reset and (int(self.t) % print_interval == 0)):
                    ...
                    if(verbose==True):
                        ...
                        print("\t I_sin  = " + str(self.numI_sym[self.tidx]))
                        print("\t I_asin = " + str(self.numI_asym[self.tidx]))
                        print("\t H      = " + str(self.numH[self.tidx]))
                        print("\t R      = " + str(self.numR[self.tidx]))
                        print("\t F      = " + str(self.numF[self.tidx]))
                        print("\t V_S    = " + str(self.numQ_S[self.tidx]))
                        print("\t V_E    = " + str(self.numQ_E[self.tidx]))
                        print("\t V_pre  = " + str(self.numQ_pre[self.tidx]))
                        print("\t V_sin  = " + str(self.numQ_sym[self.tidx]))
                        print("\t V_asin = " + str(self.numQ_asym[self.tidx]))
                        print("\t V_R    = " + str(self.numQ_R[self.tidx]))
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def plot(self, ax=None, plot_S='line', plot_E='line', plot_I_pre='line', plot_I_sym='line', plot_I_asym='line',
                            ...
                            plot_Q_S='line', plot_Q_R='line', combine_Q_infected=True, combine_I_infected=True,
                            ...
                            color_Q_S='tab:green',  color_Q_R='tab:blue', color_Q_infected='tab:purple', color_I_infected='tab:red',
        ...
        I_infectedseries = (Eseries + I_preseries + I_asymseries + I_symseries) 
        ...
        if(dashed_reference_results):
            ...
            ax.plot(dashedReference_tseries, dashedReference_infectedStack, color='#E0E0E0', linestyle='--', label='Total de Infecciones ('+dashed_reference_label+')', zorder=0)
        if(shaded_reference_results):
            shadedReference_tseries       = shaded_reference_results.tseries
            shadedReference_infectedStack = shaded_reference_results.total_num_infected() / (self.numNodes if plot_percentages else 1)
            ax.fill_between(shaded_reference_results.tseries, shadedReference_infectedStack, 0, color='#EFEFEF', label='Total de Infecciones ('+shaded_reference_label+')', zorder=0)
        ...
        if(combine_Q_infected and any(Q_infectedseries) and plot_Q_E=='stacked' and plot_Q_pre=='stacked' and plot_Q_sym=='stacked' and plot_Q_asym=='stacked'):
            ax.fill_between(numpy.ma.masked_where(Q_infectedseries<=0, self.tseries), numpy.ma.masked_where(Q_infectedseries<=0, topstack+Q_infectedseries), topstack, facecolor=color_Q_infected, alpha=0.75, hatch='//////', edgecolor='white', linewidth=0.0, label='$V_{infectados}$', zorder=2)
        ...
        if(not combine_Q_infected and any(Q_Eseries) and plot_Q_E=='stacked'):
            ax.fill_between(numpy.ma.masked_where(Q_Eseries<=0, self.tseries), numpy.ma.masked_where(Q_Eseries<=0, topstack+Q_Eseries), topstack, facecolor=color_Q_E, alpha=0.75, hatch='//////', edgecolor='white', linewidth=0.0, label='$V_E$', zorder=2)
            ax.plot(        numpy.ma.masked_where(Q_Eseries<=0, self.tseries), numpy.ma.masked_where(Q_Eseries<=0, topstack+Q_Eseries),           color=color_Q_E, zorder=3)
            topstack = topstack+Q_Eseries
        ...
        if(not combine_Q_infected and any(Q_preseries) and plot_Q_pre=='stacked'):
            ax.fill_between(numpy.ma.masked_where(Q_preseries<=0, self.tseries), numpy.ma.masked_where(Q_preseries<=0, topstack+Q_preseries), topstack, facecolor=color_Q_pre, alpha=0.75,  hatch='//////', edgecolor='white', linewidth=0.0, label='$V_{pre}$', zorder=2)
        ...
        if(not combine_Q_infected and any(Q_symseries) and plot_Q_sym=='stacked'):
            ax.fill_between(numpy.ma.masked_where(Q_symseries<=0, self.tseries), numpy.ma.masked_where(Q_symseries<=0, topstack+Q_symseries), topstack, facecolor=color_Q_sym, alpha=0.75,  hatch='//////', edgecolor='white', linewidth=0.0, label='$V_{sin}$', zorder=2)
        ...
        if(any(I_symseries) and plot_I_sym=='stacked'):
            ax.fill_between(numpy.ma.masked_where(I_symseries<=0, self.tseries), numpy.ma.masked_where(I_symseries<=0, topstack+I_symseries), topstack, color=color_I_sym, alpha=0.75, label='$I_{sin}$', zorder=2)
        ...
        if(not combine_Q_infected and any(Q_asymseries) and plot_Q_asym=='stacked'):
            ax.fill_between(numpy.ma.masked_where(Q_asymseries<=0, self.tseries), numpy.ma.masked_where(Q_asymseries<=0, topstack+Q_asymseries), topstack, facecolor=color_Q_asym, alpha=0.75,  hatch='//////', edgecolor='white', linewidth=0.0, label='$V_{asin}$', zorder=2)
        ...
        if(any(I_asymseries) and plot_I_asym=='stacked'):
            ax.fill_between(numpy.ma.masked_where(I_asymseries<=0, self.tseries), numpy.ma.masked_where(I_asymseries<=0, topstack+I_asymseries), topstack, color=color_I_asym, alpha=0.75, label='$I_{asin}$', zorder=2)
        ...
        if(any(Q_Rseries) and plot_Q_R=='stacked'):
            ax.fill_between(numpy.ma.masked_where(Q_Rseries<=0, self.tseries), numpy.ma.masked_where(Q_Rseries<=0, topstack+Q_Rseries), topstack, facecolor=color_Q_R, alpha=0.75, hatch='//////', edgecolor='white', linewidth=0.0, label='$V_R$', zorder=2)
        ...
        if(any(Q_Sseries) and plot_Q_S=='stacked'):
            ax.fill_between(numpy.ma.masked_where(Q_Sseries<=0, self.tseries), numpy.ma.masked_where(Q_Sseries<=0, topstack+Q_Sseries), topstack, facecolor=color_Q_S, alpha=0.75, hatch='//////', edgecolor='white', linewidth=0.0, label='$V_S$', zorder=2)
        ...
        if(combine_Q_infected and any(Q_infectedseries) and plot_Q_E=='shaded' and plot_Q_pre=='shaded' and plot_Q_sym=='shaded' and plot_Q_asym=='shaded'):
            ax.fill_between(numpy.ma.masked_where(Q_infectedseries<=0, self.tseries), numpy.ma.masked_where(Q_infectedseries<=0, Q_infectedseries), 0, color=color_Q_infected, alpha=0.75, hatch='//////', edgecolor='white', linewidth=0.0, label='$V_{infectados}$', zorder=4)
        ...
        if(combine_I_infected and any(I_infectedseries) and plot_E=='shaded' and plot_I_pre=='shaded' and plot_I_sym=='shaded' and plot_I_asym=='shaded'):
            ax.fill_between(numpy.ma.masked_where(I_infectedseries<=0, self.tseries), numpy.ma.masked_where(I_infectedseries<=0, I_infectedseries), 0, color=color_I_infected, alpha=0.75, hatch='//////', edgecolor='white', linewidth=0.0, label='$Total_{infectados}$', zorder=4)
            ax.plot(        numpy.ma.masked_where(I_infectedseries<=0, self.tseries), numpy.ma.masked_where(I_infectedseries<=0, I_infectedseries),    color=color_I_infected, zorder=5)
		
        if(not combine_Q_infected and any(Q_Eseries) and plot_Q_E=='shaded'):
            ax.fill_between(numpy.ma.masked_where(Q_Eseries<=0, self.tseries), numpy.ma.masked_where(Q_Eseries<=0, Q_Eseries), 0, facecolor=color_Q_E, alpha=0.75, hatch='//////', edgecolor='white', linewidth=0.0, label='$V_E$', zorder=4)
            ax.plot(        numpy.ma.masked_where(Q_Eseries<=0, self.tseries), numpy.ma.masked_where(Q_Eseries<=0, Q_Eseries),    color=color_Q_E, zorder=5)
        ...
        if(not combine_Q_infected and any(Q_preseries) and plot_Q_pre=='shaded'):
            ax.fill_between(numpy.ma.masked_where(Q_preseries<=0, self.tseries), numpy.ma.masked_where(Q_preseries<=0, Q_preseries), 0, facecolor=color_Q_pre, alpha=0.75,  hatch='//////', edgecolor='white', linewidth=0.0, label='$V_{pre}$', zorder=4)
        ...
        if(not combine_Q_infected and any(Q_symseries) and plot_Q_sym=='shaded'):
            ax.fill_between(numpy.ma.masked_where(Q_symseries<=0, self.tseries), numpy.ma.masked_where(Q_symseries<=0, Q_symseries), 0, facecolor=color_Q_sym, alpha=0.75,  hatch='//////', edgecolor='white', linewidth=0.0, label='$V_{sin}$', zorder=4)
        ...
        if(any(I_symseries) and plot_I_sym=='shaded'):
            ax.fill_between(numpy.ma.masked_where(I_symseries<=0, self.tseries), numpy.ma.masked_where(I_symseries<=0, I_symseries), 0, color=color_I_sym, alpha=0.75, label='$I_{sin}$', zorder=4)
        ...
        if(not combine_Q_infected and any(Q_asymseries) and plot_Q_asym=='shaded'):
            ax.fill_between(numpy.ma.masked_where(Q_asymseries<=0, self.tseries), numpy.ma.masked_where(Q_asymseries<=0, Q_asymseries), 0, facecolor=color_Q_asym, alpha=0.75,  hatch='//////', edgecolor='white', linewidth=0.0, label='$V_{asin}$', zorder=4)
        ...
        if(any(I_asymseries) and plot_I_asym=='shaded'):
            ax.fill_between(numpy.ma.masked_where(I_asymseries<=0, self.tseries), numpy.ma.masked_where(I_asymseries<=0, I_asymseries), 0, color=color_I_asym, alpha=0.75, label='$I_{asin}$', zorder=4)
        ...
        if(any(Q_Rseries) and plot_Q_R=='shaded'):
            ax.fill_between(numpy.ma.masked_where(Q_Rseries<=0, self.tseries), numpy.ma.masked_where(Q_Rseries<=0, Q_Rseries), 0, facecolor=color_Q_R, alpha=0.75, hatch='//////', edgecolor='white', linewidth=0.0, label='$V_R$', zorder=4)
        ...
        if(any(Q_Sseries) and plot_Q_S=='shaded'):
            ax.fill_between(numpy.ma.masked_where(Q_Sseries<=0, self.tseries), numpy.ma.masked_where(Q_Sseries<=0, Q_Sseries), 0, facecolor=color_Q_S, alpha=0.75, hatch='//////', edgecolor='white', linewidth=0.0, label='$V_S$', zorder=4)
        ...
        if(combine_Q_infected and any(Q_infectedseries) and plot_Q_E=='line' and plot_Q_pre=='line' and plot_Q_sym=='line' and plot_Q_asym=='line'):
            ax.plot(numpy.ma.masked_where(Q_infectedseries<=0, self.tseries), numpy.ma.masked_where(Q_infectedseries<=0, Q_infectedseries), color=color_Q_infected, label='$V_{infectados}$', zorder=6)

        if(combine_I_infected and any(I_infectedseries) and plot_E=='line' and plot_Q_pre=='line' and plot_Q_sym=='line' and plot_Q_asym=='line'):
            ax.plot(numpy.ma.masked_where(I_infectedseries<=0, self.tseries), numpy.ma.masked_where(I_infectedseries<=0, I_infectedseries), color=color_I_infected, label='$Total_{infectados}$', zorder=6)
        ...
        if(not combine_Q_infected and any(Q_preseries) and plot_Q_pre=='line'):
            ax.plot(numpy.ma.masked_where(Q_preseries<=0, self.tseries), numpy.ma.masked_where(Q_preseries<=0, Q_preseries), color=color_Q_pre, label='$V_{pre}$', zorder=6)
        ...
        if(not combine_Q_infected and any(Q_symseries) and plot_Q_sym=='line'):
            ax.plot(numpy.ma.masked_where(Q_symseries<=0, self.tseries), numpy.ma.masked_where(Q_symseries<=0, Q_symseries), color=color_Q_sym, label='$V_{sin}$', zorder=6)

        if(any(I_symseries) and plot_I_sym=='line'):
            ax.plot(numpy.ma.masked_where(I_symseries<=0, self.tseries), numpy.ma.masked_where(I_symseries<=0, I_symseries), color=color_I_sym, label='$I_{asin}$', zorder=6)
        
        if(not combine_Q_infected and any(Q_asymseries) and plot_Q_asym=='line'):
            ax.plot(numpy.ma.masked_where(Q_asymseries<=0, self.tseries), numpy.ma.masked_where(Q_asymseries<=0, Q_asymseries), color=color_Q_asym, label='$V_{asin}$', zorder=6)

        if(any(I_asymseries) and plot_I_asym=='line'):
            ax.plot(numpy.ma.masked_where(I_asymseries<=0, self.tseries), numpy.ma.masked_where(I_asymseries<=0, I_asymseries), color=color_I_asym, label='$I_{asim}$', zorder=6)
        
        if(any(Q_Rseries) and plot_Q_R=='line'):
            ax.plot(numpy.ma.masked_where(Q_Rseries<=0, self.tseries), numpy.ma.masked_where(Q_Rseries<=0, Q_Rseries), color=color_Q_R, linestyle='--', label='$V_R$', zorder=6)

        if(any(Rseries) and plot_R=='line'):
            ax.plot(numpy.ma.masked_where(Rseries<=0, self.tseries), numpy.ma.masked_where(Rseries<=0, Rseries), color=color_R, label='$R$', zorder=6)

        if(any(Q_Sseries) and plot_Q_S=='line'):
            ax.plot(numpy.ma.masked_where(Q_Sseries<=0, self.tseries), numpy.ma.masked_where(Q_Sseries<=0, Q_Sseries), color=color_Q_S, linestyle='--', label='$V_S$', zorder=6)
        ...
        # Draw the plot labels:
        #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        F_legend = mpatches.Patch(color='black', label='$F$')
        H_legend = mpatches.Patch(color='violet', label='$H$')
        V_E_legend = mpatches.Patch(color='orange', label='$V_E$')
        E_legend = mpatches.Patch(color='orange', label='$E$')
        V_pre_legend = mpatches.Patch(color='tomato', label='$V_{pre}$')
        I_pre_legend = mpatches.Patch(color='tomato', label='$I_{pre}$')
        V_sin_legend = mpatches.Patch(color='crimson', label='$V_{sin}$')
        I_sin_legend = mpatches.Patch(color='crimson', label='$I_{sin}$')
        V_asin_legend = mpatches.Patch(color='#F0909B', label='$V_{asin}$')
        I_asin_legend = mpatches.Patch(color='#F0909B', label='$I_{asin}$')
        V_R_legend = mpatches.Patch(color='tab:blue', label='$V_R$')
        R_legend = mpatches.Patch(color='tab:blue', label='$R$')
        S_legend = mpatches.Patch(color='tab:green', label='$S$')
        V_S_legend = mpatches.Patch(color='tab:green', label='$V_S$')
        V_infected = mpatches.Patch(color='tab:purple', label='$V_{infectados}$')
        all_labels = [F_legend, H_legend, V_E_legend, E_legend, V_pre_legend, I_pre_legend, V_sin_legend, I_sin_legend, V_asin_legend, I_asin_legend, V_R_legend, R_legend, S_legend, V_S_legend, V_infected]		
     
            
        ax.set_xlabel('Días', size=12)
        ax.set_ylabel('Porcentaje de la Población' if plot_percentages else 'Número de Personas', size=12)
        ax.set_xlim(0, (max(self.tseries) if not xlim else xlim))
        ax.set_ylim(0, ylim)
        if(plot_percentages):
        ...
        if(legend):
            legend_handles, legend_labels = ax.get_legend_handles_labels()
            ax.legend(legend_handles[::-1], legend_labels[::-1], loc='upper right', facecolor='white', edgecolor='none', framealpha=0.9, prop={'size': 14})
			#legend_handles, 
            #legend_labels = ax.get_legend_handles_labels()
            #ax.legend(legend_labels[::-1], all_labels, loc='upper right', facecolor='white', edgecolor='none', framealpha=0.9, prop={'size': 8})
        if(title):
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def figure_infections(self, plot_S=False, plot_E='stacked', plot_I_pre='stacked', plot_I_sym='stacked', plot_I_asym='stacked',
                                ...
                                plot_Q_S=False, plot_Q_R=False, combine_Q_infected=True, combine_I_infected=True,
                                color_S='tab:green', color_E='orange', color_I_pre='tomato', color_I_sym='crimson', color_I_asym='#F0909B', 
                                color_H='violet', color_R='tab:blue', color_F='black',
                                color_Q_E='orange', color_Q_pre='tomato', color_Q_sym='crimson', color_Q_asym='#F0909B', 
                                color_Q_S='tab:green',  color_Q_R='tab:blue', color_Q_infected='tab:purple', color_I_infected='tab:red',
        ...
        self.plot(ax=ax, plot_S=plot_S, plot_E=plot_E, plot_I_pre=plot_I_pre, plot_I_sym=plot_I_sym, plot_I_asym=plot_I_asym,
                        plot_H=plot_H, plot_R=plot_R, plot_F=plot_F,
                        plot_Q_E=plot_Q_E, plot_Q_pre=plot_Q_pre, plot_Q_sym=plot_Q_sym, plot_Q_asym=plot_Q_asym, 
                        plot_Q_S=plot_Q_S, plot_Q_R=plot_Q_R, combine_Q_infected=combine_Q_infected, combine_I_infected=combine_I_infected,
                        color_S=color_S, color_E=color_E, color_I_pre=color_I_pre, color_I_sym=color_I_sym, color_I_asym=color_I_asym, 
                        color_H=color_H, color_R=color_R, color_F=color_F,
                        color_Q_E=color_Q_E, color_Q_pre=color_Q_pre, color_Q_sym=color_Q_sym, color_Q_asym=color_Q_asym, 
                        color_Q_S=color_Q_S,  color_Q_R=color_Q_R, color_Q_infected=color_Q_infected, color_I_infected=color_I_infected,
                        color_reference=color_reference,
                        dashed_reference_results=dashed_reference_results, dashed_reference_label=dashed_reference_label, 
                        shaded_reference_results=shaded_reference_results, shaded_reference_label=shaded_reference_label, 
                        vlines=vlines, vline_colors=vline_colors, vline_styles=vline_styles, vline_labels=vline_labels, 
