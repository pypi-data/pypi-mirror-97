...
# serialize-mcrrs
class TestNameParameter(SerializeMCR):
    schema = [
        # success cases
        {'name': 'var1', 'type': (str,)},
        {'name': 'var2', 'type': (str,)}
...
class TestTypeParameter(SerializeMCR):
...
class TestNullableParameter(SerializeMCR):
    schema = [
...
class TestOptionalParameter(SerializeMCR):
    schema = [
...
class TestDefaultParameter(SerializeMCR):
...
class TestDefaultParameterFailure(SerializeMCR):
    test_regex = '^[A-Za-z0-9]{3}$'
    schema = [
...
class TestCompoundSchemaFunctionality(SerializeMCR):
    schema = [
...
class CompoundSerializerHelper(SerializeMCR):
    schema = [
        {'name': 'num1', 'type': (int,)}
    ]


class TestCompoundSerializerFunctionality(SerializeMCR):
    schema = [
        {'name': 'var1', 'is_compound': True, 'compound_serializer': CompoundSerializerHelper}
    ]

class TestInvalidFields(SerializeMCR):
    schema = [
        # success cases
        {'name': 'var1', 'type': (str,)},
        {'name': 'var2', 'type': (str,)},
