...
from typing import Container, Iterable, Iterator, List, Optional, Union
...
def _parse_version_number_string(version_number_string: str) -> Union[str, int]:
    return (
        ...
        else int(version_number_string)
    )
...
class Version:
    ...
    def _parse_version_string(self, version_string: str) -> None:
        prefix: str
        number_string: str
        version_string_parts: Iterator[str] = iter(
            re.split(r"([^\d*]+)", version_string)
        )
...
def _get_updated_version_identifier(
    installed_version: str, required_version: Optional[str], operator: str
) -> str:
    version_string: str
...
def get_updated_version_requirement(
    requirement: str, default_operator: Optional[str] = None
) -> str:
    """
    ...
    return package_identifier_and_options + ",".join(
        _get_updated_version_specifier(
            package_identifier, version_specifier, default_operator
        )
...
def update_requirements_versions(
    ...
    for index, version_requirement in enumerate(requirements):
        ...
        if (package_identifier not in ignore) and (
            package_identifier.split("[")[0] not in ignore
        ):
            try:
                requirements[index] = get_updated_version_requirement(
                    version_requirement, default_operator=default_operator
                )
            except:  # noqa
