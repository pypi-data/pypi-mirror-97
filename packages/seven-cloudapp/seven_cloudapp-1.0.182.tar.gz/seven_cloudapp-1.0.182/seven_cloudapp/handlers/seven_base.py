...
:LastEditTime: 2021-03-04 11:04:25
...
class SevenBaseHandler(BaseApiHandler):
    ...
    :description: SevenBaseHandler
    """
    def prepare_ext(self, *argc, **argkw):
        """
        :Description: 置于任何请求方法前被调用(请勿重写此函数,可重写prepare_ext)
        :last_editors: HuangJingCan
        """
        dict_par = {}
        dict_par["product_id"] = config.get_value("product_id", 0)
        dict_par["app_id"] = self.get_param("app_id")
        dict_par["open_id"] = self.get_param("open_id")
        dict_par["nick_name"] = self.get_param("nick_name")
