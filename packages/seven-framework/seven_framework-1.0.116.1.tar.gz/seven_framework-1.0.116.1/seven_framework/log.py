...
:LastEditTime: 2021-03-02 16:10:51
            ...
            try:
                ...
                if not exist_result:
                    if level.lower() == "http":
                        ...
                        self.clickhouse_client.execute(
                            f"CREATE TABLE {storage['config']['database']}.{self.table_name}( request_code String, project_name String, level String, host_ip String, record_time datetime, log_msg String) ENGINE = MergeTree() ORDER BY record_time;"
                        )
            except:
