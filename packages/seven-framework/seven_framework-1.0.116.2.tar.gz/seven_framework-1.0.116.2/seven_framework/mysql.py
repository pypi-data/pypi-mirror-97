    ...
    def _execute(self, sql, params=None):
        try:
            if hasattr(self, "_log_sql") and self._log_sql:
                ...
                self._log_sql.info(sql_info,extra={"extra":{"action_mode":"sql"}})
        except:
        ...
        try:
            ...
            try:
                if hasattr(self, "_log_sql") and self._log_sql:
                    log_cur = self._cur
                    if log_cur and log_cur.rowcount>0:
                        execute_result = json.JsonHelper.json_dumps(log_cur.fetchall())
                        result_data = f"{execute_result if execute_result else ''}"
                        self._log_sql.info(result_data,extra={"extra":{"action_mode":"data"}})
            except:
                print(traceback.print_exc())
                pass
