...
class SWord:
    ...
    def sub_words_by_affix_forward(self, affix=None, handled=None, affix_max=False):
        """
        ...
        for i in range(len(word)):
            ...
            if w not in handled:
                data.extend(SWord(w).sub_words_by_affix_forward(affix, handled))  # 递归
    ...
    def sub_words_by_affix_backward(self, affix=None, handled=None):
        """
        :param list[str] affix: 词缀列表
        :param list[str] handled: 白名单，白名单中的词汇跳过处理
        [为了解决递归重复消费增加的变量]
        ...
        for i in range(len(word)):
            ...
            # 这里不能合并，分开写是因为虽然重复但是想看看最大词缀匹配情况
            if w not in handled:
                data.extend(SWord(w).sub_words_by_affix_backward(affix, handled))  # 递归
    ...
    def sub_words_by_affix(self, affix=None, handled=None, direction=SUB_WORD_DIRECTION_BACKWARD, affix_max=False):
       ...
       """
        if direction == self.SUB_WORD_DIRECTION_BACKWARD:
            return self.sub_words_by_affix_backward(affix)
        else:
            return self.sub_words_by_affix_forward(affix, affix_max=affix_max)
...
def sub_words(word, affix=None, handled=None, direction=SWord.SUB_WORD_DIRECTION_BACKWARD, affix_max=False):
    """
       ...
       :return :   返回所有分解详情
   """
    return SWord(word).sub_words(affix, handled, direction, affix_max)
