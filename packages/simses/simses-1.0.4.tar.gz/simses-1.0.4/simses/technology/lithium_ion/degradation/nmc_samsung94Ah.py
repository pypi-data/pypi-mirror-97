...
class Samsung94AhNMCDegradationModel(DegradationModel):
    """ Degradation Model for Samsung94Ah cell. Specific model unknown, therefore using different existing
    ...
    def __init__(self, cell_type: CellType, cycle_detector: CycleDetector, battery_config: BatteryConfig):
        super().__init__(cell_type, GenericCellCyclicDegradationModel(cell_type, cycle_detector, battery_config),
                         GenericCellCalendarDegradationModel(cell_type, battery_config), cycle_detector, battery_config)
