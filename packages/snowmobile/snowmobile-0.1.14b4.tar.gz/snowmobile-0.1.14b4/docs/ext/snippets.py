...
GIT_ROOT = r'https://github.com/GEM7318/Snowmobile/tree/0.2.1/docs'
...
def git(file_path: Path) -> str:
    """Generates 'git' container"""
    p_rel = list(file_path.relative_to(root).parts)
    p_rel.insert(0, root.name)
    url_git_v0 = '/'.join(p_rel)
    url_git = f"{GIT_ROOT}/{url_git_v0}"
    _div = "{div}"
    return f"""
...
def link_container(file_path: Path) -> str:
...
"""
    ...
    if INCL_PARENT_CONTAINER:
        _links = link_container(file_path)
