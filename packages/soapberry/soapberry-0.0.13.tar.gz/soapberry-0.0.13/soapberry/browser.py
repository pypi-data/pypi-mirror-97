...
class browser:
    ...
    def create_firefox(self, page_load_timeout, wait_timeout, add_log = True):
        self.create_browser(page_load_timeout, wait_timeout, False, add_log)

    def create_chrome(self, page_load_timeout, wait_timeout, add_log = True):
        self.create_browser(page_load_timeout, wait_timeout, True, add_log)

    def create_browser(self, page_load_timeout, wait_timeout, chrome, add_log = True):
    ...
    def add_and_print_log(self, message, add_log = True):
        if (add_log):
            log = soapberry.classes.soapberry_log()
            log.time = datetime.now().strftime("%d/%m/%Y %H:%M:%S")
            log.message = message
            soapberry.helpers.print_log(log)
