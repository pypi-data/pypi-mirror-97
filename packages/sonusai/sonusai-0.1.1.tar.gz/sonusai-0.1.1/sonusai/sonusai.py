...
# Older version from eps:
#def reshape_inputs(feature, truth, batch_size, timesteps, flatten, add1ch):
#     # Check and reshape sonusai feature BxTxF and truth NCxF into one of several options:
#     #   no-flatten, no-channel (keep 2D feature):  samples x tsteps x B   x T     (4-dim)
#     #   flatten, no-channel:                       samples x tsteps x B*T         (3-dim)
#     #   no-flatten, add-1channel:                  samples x tsteps x B   x T x 1 (5-dim)
#     #   flatten, add-1channel:                     samples x tsteps x B*T x 1     (4-dim)
#     #   timestep == 0, then do not add tstep dimension
#     #
#     # Both feature, truth F (input #frames) are trimmed so that samples is a multiple of batch_size
#     # Channel is added to last/outer dimension for channel_last support in Keras/TF
#     #
#     # Returns:
#     #   f, t,       reshaped feature and truth
#     #   in_shape    input shape for model (tsteps x feature)
#     #   t_n:        truth nclass = output length of nn model

#     f = feature
#     t = truth

#     (f_frames, f_ns, f_nt) = f.shape
#     (t_frames, t_n) = t.shape

#     if flatten:
#         logger.info('Flattening {}x{} feature to {}'.format(f_ns, f_nt, f_ns * f_nt))
#         f = np.reshape(f,(f_frames,f_ns*f_nt),order='F')

#     # Reshape for Keras/TF recurrent models that require timestep/sequence length dimension
#     if timesteps > 0:
#         sequences = f_frames // timesteps

#         # Remove frames if remainder, not fitting into a multiple of new # sequences
#         frem = f_frames % timesteps
#         brem = (f_frames // timesteps) % batch_size
#         bfrem = brem * timesteps
#         sequences = sequences - brem
#         fr2drop = frem + bfrem
#         if fr2drop:
#             logger.info('Dropping {} frames for new # of sequences to fit in multiple of batch_size'.format(fr2drop))
#             if f.ndim == 2:
#               f = f[0:-fr2drop,:]      # Flattened input
#             elif f.ndim == 3:
#               f = f[0:-fr2drop,:,:]    # Un-flattened input
...
#         # Do the reshape
#         logger.info('Reshape for timestep = {}, new # of sequences (batches) = {}'.format(timesteps, sequences))
#         if f.ndim == 2:                  #  Flattened input
#           #logger.debug('Transposing 2 dim')
#           #f = np.transpose(f, (1,0));    # now frames x B*T
#           #t = np.transpose(t, (1,0));    # now frames x nclass
#           logger.debug('Reshaping 2 dim')
#           f = np.reshape(f, (sequences, timesteps, f_ns * f_nt), order='F')
#           t = np.reshape(t, (sequences, timesteps, t_n), order='F')
#         elif f.ndim == 3:                # Unflattened input
#           #logger.debug('Transposing 3 dim')
#           #f = np.transpose(f, (2,0,1));  # now frames x B x T
#           #t = np.transpose(t, (1,0));    # now frames x nclass
#           logger.debug('Reshaping 3 dim')
#           f = np.reshape(f, (sequences, timesteps, f_ns, f_nt), order='F')
#           t = np.reshape(t, (sequences, timesteps, t_n), order='F')
# ##    else:
# ##        if f.ndim == 2:                 #  Flattened input
# ##          logger.info('Transposing 2 dim (no timesteps)')
# ##          f = np.transpose(f, (1,0))      # now frames x B*T
# ##          t = np.transpose(t, (1,0))      # now frames x nclass
# ##        elif f.ndim == 3:               # Unflattened input
# ##          logger.info('Transposing 3 dim (no timesteps)')
# ##          f = np.transpose(f, (2,0,1))    # now frames x B x T
# ##          t = np.transpose(t, (1,0))

#     # Add channel dimension if required for input to model (i.e. for cnn type input)
#     if add1ch:
#         logger.info('Adding channel dimension to feature')
#         f = np.expand_dims(f,axis=f.ndim) # add as last/outermost dim

#     in_shape = f.shape
#     in_shape = in_shape[1:]      # remove frame dim size

#     logger.debug('Feature final shape: {}'.format(f.shape))
#     logger.debug('Input shape final: {}'.format(in_shape))
#     logger.debug('Truth final shape: {}'.format(t.shape))
#     #pdb.set_trace()
#     return f, t, in_shape, t_n
