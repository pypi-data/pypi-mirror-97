...
from spsdk.crypto import generate_certificate, load_private_key, load_public_key, save_crypto_item
from spsdk.crypto import x509
...
class CertificateParametersConfig:
    ...
    def __init__(self, config_data: dict) -> None:
        ...
        self.BasicConstrains_path_length = config_data['extensions']['BASIC_CONSTRAINTS']['path_length']
...
def generate_name(config_data: dict) -> x509.Name:
    ...
    attributes = [
        x509.NameAttribute(getattr(x509.NameOID, key), value)
...
@click.command()
@click.option('-j', '--json-conf', type=click.File('r'),
              help='Path to json configuration file containing the parameters for certificate.')
@click.option('-c', '--cert-path', type=click.Path(), help='Path where certificate will be stored.')
def main(json_conf: click.File, cert_path: str) -> None:
    ...
    certificate = generate_certificate(subject=cert_config.subject_name, issuer=cert_config.issuer_name,
