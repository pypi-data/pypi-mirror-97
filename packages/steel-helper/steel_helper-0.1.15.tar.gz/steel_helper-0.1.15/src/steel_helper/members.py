...
class Member:
    def __init__(self, designation):
        ...
        if designation != "none":
            ...
            # Check if not PL
            if designation.find("PL") > -1:
                self.Type = "PL"
                # print("PL!")

            else:
                for column in myshape.columns:
                    shape_attribute = str(myshape[column]).split()[1]

                    try:
                        exec(f"self.{column} = float(shape_attribute)")

                    except:
                        if shape_attribute != "–":
                            # print(shape_attribute)
                            exec(f"self.{column} = shape_attribute")
            ...
            if self.Type == "C":
                self.t_member = self.tw * u.inch
                # print("C is type")
            elif self.Type == "W":
                self.t_member = self.tf * u.inch
                # print("C is type")

            # Determine Width and thickness
            elif len(mydimensions) == 3:
                self.w_member = convert_to_float(mydimensions[1]) * u.inch
                self.t_member = convert_to_float(mydimensions[2]) * u.inch
            else:
                self.w_member = convert_to_float(mydimensions[0]) * u.inch
                self.t_member = convert_to_float(mydimensions[1]) * u.inch
            ...
            try:
                ...
                self.A_g = self.w_member * self.t_member
    ...
    def bolts(self, bolt_size, holes, load_transfer=1, *stagger):
        ...
        if self.bolt_size < 1 * u.inch:
            ...
            self.myeqns.additem(
                ...
                [
                    ...
                    "$$size_{bolt} + 3 / 16 * u.inch$$",
                    "Table J3.3 and b4.36 (16.1-130,16.1-20)",
                ],
            )

        self.A_hole = self.t_member * self.hole_size
        ...
        self.myeqns.additem(
            ...
            [
                "Area of holes",
                self.A_holes,
                f"{self.t_member} * {self.hole_size} * {self.holes}",
                "$$t_{member} * size_{holes} * holes$$",
                "",
            ],
        )
        ...
        if len(stagger) > 0:
            ...
            self.myeqns.additem(
                ...
                [
                    ...
                    "$$(A_g - A_{holes} + stagger equations) * load transfer)$$",
    ...
    def design_strength(self):
        ...
        yield_strength_LRFD = P_n_gross * phi
        self.myeqns.additem(
            "LRFD Yield strength",
            [
                "LRFD Yield strength",
        ...
        phi = 0.75
        rupture_strength_LRFD = 0.75 * P_n_net
        self.myeqns.additem(
        ...
        if yield_strength_LRFD < rupture_strength_LRFD:
            design_strength_LRFD = yield_strength_LRFD
            try:
                if design_strength_LRFD > self.LRFD_max:
                    self.myeqns.greens.append("LRFD Yield strength")
                else:
                    self.myeqns.reds.append("LRFD Yield strength")
            except:
                self.myeqns.maxes.append("LRFD Yield strength")

        else:
            design_strength_LRFD = rupture_strength_LRFD
            try:
                if design_strength_LRFD > self.LRFD_max:
                    self.myeqns.greens.append("LRFD Rupture strength")
                else:
                    self.myeqns.reds.append("LRFD Rupture strength")
            except:
                self.myeqns.maxes.append("LRFD Rupture strength")

        omega = 1.67
        yield_strength_ASD = P_n_gross / omega
        self.myeqns.additem(
            "ASD Yield strength",
            [
                "ASD Yield strength",
                P_n_gross / omega,
                f"{P_n_gross:.3g} / {omega}",
                r"$$\frac{P_n}{\omega}$$",
                "D2-1",
            ],
        )
        omega = 2
        rupture_strength_ASD = P_n_net / 2
        self.myeqns.additem(
        ...
        if yield_strength_ASD < rupture_strength_ASD:
            design_strength_ASD = yield_strength_ASD
            try:
                if design_strength_ASD > self.ASD_max:
                    self.myeqns.greens.append("ASD Yield strength")
                else:
                    self.myeqns.reds.append("ASD Yield strength")
            except:
                self.myeqns.maxes.append("ASD Yield strength")

        else:
            design_strength_ASD = rupture_strength_ASD
            try:
                if design_strength_ASD > self.ASD_max:
                    self.myeqns.greens.append("ASD Rupture strength")
                else:
                    self.myeqns.reds.append("ASD Rupture strength")
            except:
                self.myeqns.maxes.append("ASD Rupture strength")

    def reduction_factor(self, shape_type, x_bar=0, l_plate=0, w_member=0, fasteners=0):
        l = l_plate
        w = w_member
        x = x_bar
        if fasteners >= 4:
    ...
    def block_shear(self, x_bolts, y_bolts, rows=1, effective_factor=1):
        ...
        A_nv = t_member * ((x_bolts) - (holes - 0.5 * rows) * (d_hole))
        A_nt = t_member * ((y_bolts) - (0.5 * rows) * (d_hole))
        ...
        self.myeqns.additem(
            ...
            [
                "Net area in shear",
                A_nv,
                f"{t_member} * (({x_bolts}) - ({holes} - 0.5 * {rows}) * ({d_hole}))",
                "$$t_{member} * ((x_{bolts}) - (holes - 0.5 * rows) * (d_{hole}))$$",
                "J4-5",
            ],
        )
        self.myeqns.additem(
            ...
            [
                "Net area in tension",
                A_nt,
                f"{t_member} * (({y_bolts}) - (0.5 * {rows}) * ({d_hole}))",
                "$$t_{member} * ((y_{bolts}) - (0.5 * rows) * (d_{hole}))$$",
                "J4-5",
            ],
        )
        self.myeqns.additem(
            ...
            [
                ...
                "$$0.6 * F_u * A_{nv} + effective factor * F_u * A_{nt}$$",
                "Eq 3.3",
            ],
        )
        self.myeqns.additem(
            ...
            [
                ...
                "$$0.6 * F_y * A_g + effective factor * F_u * A_{nt}$$",
                "Eq 3.4",
            ],
        )
        self.myeqns.additem(
