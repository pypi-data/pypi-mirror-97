...
class Ds18b20Sensor(TempSensor):
    ...
    def _list_detected_ds18b20(self):
        ...
        if 'not found.' in w1_ids:
            raise SensorDisconnectedError(f"{w1_listfile}: '{w1_ids}'" )

        if not all(a.startswith("28-") for a in w1_ids):
            # something funky is going on, if this error happens
            # then investigate
            #
            # it started happening on ber1:
            # temp_pub:main:74 - waiting 30s for sensor to come back
            # temp_pub:main:68 - unexpected values in '/sys/bus/w1/devices/w1_bus_master_slaves': ['not found.']
            # systemd[1]: sudoisbot@temp_pub.service: Main process exited, code=exited, status=1/FAILURE
            # systemd[1]: sudoisbot@temp_pub.service: Failed with result 'exit-code'.
