...
class HedgeFundNAVReader:
    ...
    def _dump_to_db(self, df: pd.DataFrame):
        ...
        if now_df is not None and not now_df.empty:
            ...
            now_df = now_df.drop(columns=['update_time', 'create_time', 'is_deleted', 'change_rate', 'adjusted_net_value']).sort_values(by=['fund_id', 'datetime']).drop_duplicates(subset=['fund_id', 'datetime'], keep='last')
