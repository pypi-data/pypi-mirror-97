...
class ManagerProcessorDev:
    ...
    def init(self, start_date=str, end_date=str, print_time=False, is_mng_index=False, is_mng_indicator=False):
        self.start_date = start_date if isinstance(start_date, datetime.datetime) else datetime.datetime.strptime(start_date,'%Y-%m-%d').date()
        self.end_date = end_date if isinstance(end_date, datetime.datetime) else datetime.datetime.strptime(end_date,'%Y-%m-%d').date()
...
if __name__ == '__main__':
    self = ManagerProcessorDev(DerivedDataApi())
    start_date = '2005-01-01'
    end_date = '2020-10-31'
    self.init(start_date=start_date,end_date=end_date,is_mng_index=True)
    self.calc_index()
