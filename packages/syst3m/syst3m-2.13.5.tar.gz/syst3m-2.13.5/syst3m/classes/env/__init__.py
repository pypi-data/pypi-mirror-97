...
class Env(object):
	...
	def import_(self, env=None):
		if env == None: return r3sponse.error("Define parameter: env.")
		dictionary = {}
		if isinstance(env, (str, String, FilePath)):
			...
			dictionary = env
		else: return r3sponse.error(f"Invalid usage, env requires to be a [str, String, FilePath, dict, Dictionary], not [{env.__class__.__name__}].")
		for key,value in dictionary.items():
			os.environ[str(key)] = str(value)
		return r3sponse.success(f"Successfully imported {len(dictionary)} env variables.")
		#
	# export env.
	def export(self, env=None, export=None):
		...
		if export != None:
			...
			Files.save(export, exported)
		return r3sponse.success(f"Successfully exported {len(dictionary)} env variables.")
		#
	# get.
