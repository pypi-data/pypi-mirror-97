...
class Spawn(objects.Object):
	...
	def start(self):
		...
		if self.child.exitstatus not in [0, None]:
			return Output(error=f"{self.response_str} returned exit status: [{spawn.child.exitstatus}].")
		self.__output__ = ""
	...
	def closed(self):
		"""if self.async_:
			boolean = yield from self.child.closed
		else:
			boolean = self.child.closed
		"""
		boolean = self.child.closed
		return boolean
		#
...
# the output object class (very similair to ResonseObject).
...
def execute(
	# Notes:
	#   returns a syst3m.console.Output object (very similair to ResponseObject).
	#
	# Mode:
	#   option 1:
	...
	if not response.success: return Output(error=f"Failed to start {response_str}, error: {response.error}")
	
	# check exit status.
	if spawn.child.exitstatus not in [0, None]:
		return Output(error=f"{response_str} returned exit status: [{spawn.child.exitstatus}].")

	# await.
	for i in range(10):
		if spawn.running: break
		time.sleep(1)
	if not spawn.running: return Output(error=f"Spawned {response_str} is not running.")
	...
	if not async_ and isinstance(input, (list, Array, dict, Dictionary)) and len(input) > 0:
		...
		if not spawn.expecting:
			if defaults.options.log_level >= 1:
				return Output(error=f"Failed to send {str_input} to {response_str}, child is not expecting any input [{spawn.child}].")
			else:
				return Output(error=f"Failed to send {str_input} to {response_str}, child is not expecting any input.")
				...
				if not response.success: 
					if "End of file" in response.error:
						...
						if isinstance(_input_, (list, Array)):
							...
							str_input = f"[{_input_}]"
						return Output(error=f"Failed to send one of the expected input(s) [{str_input}], error: {response.error}")
		...
		# send one of the keys (dict instance).
			...
			if not response.success: 
				if "End of file" in response.error:
					error_end_of_file = True
				else: return Output(error=f"Failed to send one of the expected input(s) [{str_input}], error: {response.error}")

		""" check no input left. 
		if not error_end_of_file and spawn.expecting:
			return Output(error=f"Failed to execute {response_str}, still expecting: [{spawn.child.after.decode()}].")
		"""
	...
	if not async_:
		...
		if not response.success: 
			if not response.success: return Output(error=f"Failed to retrieve output from spawn {response_str}, error: {response.error}")
		if spawn.child.exitstatus not in [0, None]:
			return Output(error=f"{response_str} returned exit status: [{spawn.child.exitstatus}].")

		# await.
		#response = spawn.wait()
		#if not response.success: return Output(error=response.error) # exit status.

		# output.
		response = spawn.read(wait=True, timeout=None) # always await sync.
		if not response.success: 
			if not response.success: return Output(error=f"Failed to retrieve output from spawn {response_str}, error: {response.error}")
		...
		if error_end_of_file != None and error_end_of_file: 
			if defaults.options.log_level >= 1:
				return Output(error=f"Failed to send the expected input to {response_str} (#234343) (output: {output}) (child: {spawn.child}).")
			else:
				return Output(error=f"Failed to send the expected input to {response_str} (#234343) (output: {output}).")

		# check kill.
		if kill and spawn.running:
			if defaults.options.log_level >= 8: print(f"Killing process {response_str}.")
			response = spawn.kill()
			if not response.success: return Output(error=f"Failed to kill {response_str}, error: {response.error}")
	...
	# async.
		...
		if wait:
			...
			# await.
			response = spawn.wait(timeout=None)
			if not response.success: return Output(error=response.error) # exit status.

			# output.
			response = spawn.read(timeout=1.0) # since spawn is awaited.
			if not response.success: 
				if not response.success: return Output(error=f"Failed to retrieve output from spawn {response_str}, error: {response.error}")
			output = response.output
			while True:
	...
	if serialize:
		...
		if not response.success: 
			return Output(error=f"Encoutered an error in the serialized response, error: {response.error}")
