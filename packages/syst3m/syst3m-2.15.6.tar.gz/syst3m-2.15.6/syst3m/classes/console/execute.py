	...
	def expect(self,
		...
		if isinstance(expect, str):
			...
			try:
				...
				c = 2
				if defaults.options.log_level >= 8: print(f"Found expected: {expect}")
				if send != None:
					if defaults.options.log_level >= 8: print(f"Attempting to send: {send}")
					#if self.async_:
					...
					if defaults.options.log_level >= 8: print(f"Succesfully sended: {send}")
		...
		# list expect.
			...
			for _expect_ in expect:
				if isinstance(send, (list,Array)):
					...
					_send_ = str(send)
				if defaults.options.log_level >= 8: print(f"Checking optional expect: {_expect_}")
...
def execute(
	...
	if defaults.options.log_level >= 8: print(f"Spawn: [ $ {executive} {path} {arguments}]",)
	spawn = Spawn(
	...
	if not response.success: return OutputObject(error=f"Failed to start {response_str}, error: {response.error}")
	
	# check crashed
	response = spawn.crashed()
	if not response.success:  return OutputObject(error=response.error)
	...
	# has already exited.
		...
		if not response.success: return OutputObject(error=response.error) # exit status.
	...
	# proceed if already finished.
		...
		for i in range(10):
			...
			time.sleep(1)
		if not spawn.running: return OutputObject(error=f"Unable to start {response_str}.")
		...
		if not async_ and isinstance(input, (list, Array, dict, Dictionary)) and len(input) > 0:
			...
			def process_dict_input(dictionary):
				...
				# expect .
				if defaults.options.log_level >= 8:
			...
			if not spawn.expecting:
				if defaults.options.log_level >= 1:
					return OutputObject(error=f"Failed to send {str_input} to {response_str}, child is not expecting any input [{spawn.child}].")
				else:
					return OutputObject(error=f"Failed to send {str_input} to {response_str}, child is not expecting any input.")
					...
					if not response.success: 
						if "End of file" in response.error:
							...
							if isinstance(_input_, (list, Array)):
								...
								str_input = f"[{_input_}]"
							return OutputObject(error=f"Failed to send one of the expected input(s) [{str_input}], error: {response.error}")
			...
			# send one of the keys (dict instance).
				...
				if not response.success: 
					if "End of file" in response.error:
						error_end_of_file = True
					else: return OutputObject(error=f"Failed to send one of the expected input(s) [{str_input}], error: {response.error}")
			...
			if not error_end_of_file and spawn.expecting:
				...
				return OutputObject(error=f"Failed to execute {response_str}, still expecting: [{after}].")
			"""
		...
		if not async_:
			...
			if error_end_of_file != None and error_end_of_file: 
				if defaults.options.log_level >= 1:
					return OutputObject(error=f"Failed to send the expected input to {response_str} (#234343) (output: {output}) (child: {spawn.child}).")
				else:
					return OutputObject(error=f"Failed to send the expected input to {response_str} (#234343) (output: {output}).")

			# check kill.
			if kill and spawn.running:
				if defaults.options.log_level >= 8: print(f"Killing process {response_str}.")
				response = spawn.kill()
				if not response.success: return OutputObject(error=f"Failed to kill {response_str}, error: {response.error}")
		...
		# async.
			...
			if not response.success: 
				if not response.success: return OutputObject(error=f"Failed to retrieve output from spawn {response_str}, error: {response.error}")
			if spawn.child.exitstatus not in [0, None]:
				return OutputObject(error=f"{response_str} returned exit status: [{spawn.child.exitstatus}].")
	...
	if serialize:
		try: response = r3sponse.ResponseObject(json=output)
			...
			return OutputObject(error=f"Failed to serialize (output: {output}).")
		if not response.success: 
			return OutputObject(error=f"Encoutered an error in the serialized response, error: {response.error}")
	return OutputObject(message=f"Succesfully executed {response_str}.", attributes={
		"output":output,
		"process":spawn,
		"pid":spawn.child.pid,
	...
	# excute default without input.
		...
		try:
			...
			if defaults.options.log_level <= 0:
				return OutputObject(error=f"Failed to execute {response_str}, (exception: {exception}), (error: {error_}).")
			else:
				return OutputObject(error=f"Failed to execute {response_str}, (exception: {exception}), (output: {output}), (error: {error_}).")
		error_, output = proc.stderr, proc.stdout
		if isinstance(error_, bytes): error_ = error_.decode()
		if isinstance(output, bytes): output = output.decode()
		if error_ != "":
			if defaults.options.log_level <= 0:
				return OutputObject(error=f"Failed to execute {response_str}, (error: {error_}).")
			else:
				return OutputObject(error=f"Failed to execute {response_str}, (output: {output}), (error: {error_}).")
