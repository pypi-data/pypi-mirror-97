...
class Spawn(objects.Object):
	...
	def read(self, 
		...
		timeout=None,
		# the live boolean (bool) (prints live logs to console when enabled) (leave None to use self.log_level >= 0).
		live=None, 
		# system variables.
		...
		# checks.
		if live == None: live = self.log_level >= 0
		...
		if not wait:
			...
			for line in range(1000000000):
				try:
					...
					if "Timeout" in str(e):
						response = r3sponse.error(f"{e} (most likely the command is still expecting input).")
						if __safe__: response.output = ""
						return response
					else:
						response = r3sponse.error(f"{e}.")	
						if __safe__: response.output = ""
						return response
	...
	def crashed(self):
		...
		if self.exit_status not in [0, None]:
			return r3sponse.error(f"{self.response_str} returned exit status: [{self.exit_status}] (output: {self.read(wait=False, __safe__=True).output}).")
