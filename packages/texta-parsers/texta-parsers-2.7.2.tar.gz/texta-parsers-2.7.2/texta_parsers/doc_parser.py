...
from texta_parsers.settings import META_FIELD, FACE_VECTOR_FIELD_NAME
...
class DocParser:
    ...
    def _parse_file(document, languages):
        ...
        if document["extension"] in Extension.IMAGE_EXTENSIONS:
            face_vectors = FACE_ANALYZER.vectorize_photo(document["path"])
            if face_vectors:
