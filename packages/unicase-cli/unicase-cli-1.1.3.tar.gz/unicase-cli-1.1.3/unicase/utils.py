...
class Tapd:
    ...
    def post_tapd_task(self, **kwargs):
        ...
        if not json_data.get('data'):
            raise Exception('用例上传失败, 原因：', json_data.get('info', 'no data'))
    ...
    def post_tapd_test_plans_and_tcase_relation(self, **kwargs):
        ...
        if json_data.get('status') != 1:
            raise Exception('创建测试计划和测试用例关联关系失败, 原因：', json_data.get('info', 'status != 1'))
    ...
    def validate_data(self):
        ...
        for row_index in range(3, self.case_ws.max_row + 1):
            ...
            if self.case_ws[case_title_cell].value:
                if not self.case_ws[custom_field_1_cell].value:
                    results.append(f'第 {row_index} 行用例【一级模块】不能为空')
                else:
                    cases_count += 1
