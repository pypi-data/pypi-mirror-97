...
class Face_cropper:
    ...
    def generate_json_result(self,checks, treshold):
        data_set = {"up_left": {"difference_rate":checks[0][1],"is_problem" : True if checks[0][1] > treshold else False},
                    "up_center": {"difference_rate":checks[1][1],"is_problem" : True if checks[1][1] > treshold else False},
                    "up_right":{"difference_rate":checks[2][1],"is_problem" : True if checks[2][1] > treshold else False},
                    "middle_left": {"difference_rate":checks[3][1],"is_problem" : True if checks[3][1] > treshold else False},
                    "middle_center": {"difference_rate":checks[4][1],"is_problem" : True if checks[4][1] > treshold else False},
                    "middle_right": {"difference_rate":checks[5][1],"is_problem" : True if checks[5][1] > treshold else False},
                    "bottom_left": {"difference_rate":checks[6][1],"is_problem" : True if checks[6][1] > treshold else False},
                    "bottom_center": {"difference_rate":checks[7][1],"is_problem" : True if checks[7][1] > treshold else False},
                    "bottom_right": {"difference_rate":checks[8][1],"is_problem" : True if checks[8][1] > treshold else False}}
    ...
    def generate_json_result(self,checks, is_inside, filename = "data.json"):
        ...
        rate_bottom_left = checks[6][1]
        rate_bottom_center = checks[7][1]
        rate_bottom_right = checks[8][1]
        
        if not(is_inside):
            # rate_up_left score
            rate = rate_up_left
            up_left_score = 0 if rate < 0.01 else 1 if rate < 0.02 else 2 if rate < 0.04 else 3 if rate < 0.05 else 4 if rate < 0.07 else 5

            #rate_up_center score
            rate = rate_up_center
            up_center_score = 0 if rate < 0.01 else 1 if rate < 0.02 else 2 if rate < 0.04 else 3 if rate < 0.05 else 4 if rate < 0.07 else 5

            #rate_up_right score
            rate = rate_up_right
            up_right_score = 0 if rate < 0.01 else 1 if rate < 0.02 else 2 if rate < 0.04 else 3 if rate < 0.05 else 4 if rate < 0.07 else 5
            ...
            bottom_left_score = 0 if rate < 0.01 else 1 if rate < 0.02 else 2 if rate < 0.04 else 3 if rate < 0.05 else 4 if rate < 0.07 else 5

            #rate_bottom_center score
            rate = rate_bottom_center
            bottom_center_score = 0 if rate < 0.01 else 1 if rate < 0.02 else 2 if rate < 0.04 else 3 if rate < 0.05 else 4 if rate < 0.07 else 5

            #rate_bottom_right score
            rate = rate_bottom_right
            bottom_right_score = 0 if rate < 0.01 else 1 if rate < 0.02 else 2 if rate < 0.04 else 3 if rate < 0.05 else 4 if rate < 0.07 else 5
        else:
            rate = rate_up_left
            up_left_score = 0 if rate < 0.01 else 1 if rate < 0.05 else 2 if rate < 0.1 else 3 if rate < 0.12 else 4 if rate < 0.15 else 5
            
            #rate_up_center score
            rate = rate_up_center
            up_center_score = 0 if rate < 0.01 else 1 if rate < 0.05 else 2 if rate < 0.1 else 3 if rate < 0.12 else 4 if rate < 0.15 else 5
            
            #rate_up_right score
            rate = rate_up_right
            up_right_score = 0 if rate < 0.01 else 1 if rate < 0.05 else 2 if rate < 0.1 else 3 if rate < 0.12 else 4 if rate < 0.15 else 5
            ...
            #rate_bottom_left score
            rate = rate_bottom_left
            bottom_left_score = 0 if rate < 0.01 else 1 if rate < 0.05 else 2 if rate < 0.1 else 3 if rate < 0.12 else 4 if rate < 0.15 else 5
            
            #rate_bottom_center score
            rate = rate_bottom_center
            bottom_center_score = 0 if rate < 0.01 else 1 if rate < 0.05 else 2 if rate < 0.1 else 3 if rate < 0.12 else 4 if rate < 0.15 else 5
            
            #rate_bottom_right score
            rate = rate_bottom_right
            bottom_right_score = 0 if rate < 0.01 else 1 if rate < 0.05 else 2 if rate < 0.1 else 3 if rate < 0.12 else 4 if rate < 0.15 else 5
        ...
        data_set = {"up_left": {"difference_rate":checks[0][1],"score" : up_left_score},
                    ...
                    "bottom_left": {"difference_rate":checks[6][1],"score" : bottom_left_score},
                    "bottom_center": {"difference_rate":checks[7][1],"score" : bottom_center_score},
                    "bottom_right": {"difference_rate":checks[8][1],"is_problem" : bottom_right_score}}
        json_dump = json.dumps(data_set)
        return json_dump
    ...
    def perform_outside_check(self,fname,nb_t,m_type,m_color, color_diff, pad = 100, show_plot=False,diff_black=100):
        print("tiles1 :",nb_t)
        sizes = [nb_t[0],nb_t[1]]
        ...
        for i in range(sizes[0]*sizes[1]):
            ...
            ori[i].image = image_work.how_close_to_color(ori[i].image,[0, 0, 0],diff_black,dif_color_mark = m_color)[-1]
        
            check = image_work.how_close_to_color(ori[i].image,m_color,color_diff) # 0 = no diffference between image color and color to check
            check = list(check)
            check.append(ori[i])
            check = tuple(check)
            all_checks.append(check)
    ...
    def isolate_mouth_cv(self, filename, mouth_only=True, preImg=None, pre_shape=None, crop_lips=False, keep_teeh=False,lips_only = False,pad=0): #keep teeh if crop_lips
        pos_origin = []
        try:
            if preImg is None:
                img = cv2.imread(filename)
