...
class Face_cropper:
    ...
    def check_outside(self,fname):
        nb_tt = [2,3]
        m_type = 0 #0 == lipstick / 1 == blush / 2 == eye shadow
        parameters = self.get_parameters_outside(fname,nb_color=4,default=22)
        m_color = parameters[0]#get_lipstick_color2(fc,fname,nb_color=3)#[138,  64,  65]
        color_diff = parameters[1]
        print("diff :",color_diff)
        tiles_checks1= self.perform_outside_check(fname = fname,nb_t = nb_tt,m_type = m_type,m_color=m_color, color_diff = color_diff, pad = 30, show_plot=True,diff_black=60)
        results = tiles_checks1[1]
        return self.generate_json_result(tiles_checks1[1],is_inside = False),results
    ...
    def check_inside_v2(self,img_no,img_yes, show_plot = True):
        ...
        makeup_slice = self.slice_img("", row = nb_tiles[0], col =nb_tiles[1], pre_img=image_work.how_close_to_color(mouth,colo_nat,sqrt(dist),dif_color_mark = [255,255,255])[-1])
        ...
        return self.generate_json_result(all_checks,is_inside = True), all_checks
