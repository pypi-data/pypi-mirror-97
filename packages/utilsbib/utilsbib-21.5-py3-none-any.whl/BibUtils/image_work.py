...
class Face_cropper:
    ...
    def check_outside(self,fname):
        ...
        check_normally = self.can_I_check(fname) # color dist > 30 we check normally else we remove the shadow
    ...
    def perform_outside_check(self,fname,nb_t,m_type,m_color, color_diff, pad = 100, show_plot=False,diff_black=100,normal_check =True):
        print("tiles1 :",nb_t)
        sizes = [nb_t[0],nb_t[1]]
    ...
    def isolate_mouth_cv(self, filename, mouth_only=True, preImg=None, pre_shape=None, crop_lips=False, keep_teeh=False,lips_only = False,pad=0,normal_check =False): #keep teeh if crop_lips
        pos_origin = []
        try:
            if preImg is None:
                img = cv2.imread(filename)
