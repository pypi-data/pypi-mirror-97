...
def index(request):
    ...
    paginator, page = paginate(request, Poll.objects.all(), per_page=8)

    return render(
        request,
        "wagtailpolls/index.html",
        {
            "page": page,
            "paginator": paginator,
            "poll_list": poll_list,
...
def search(request):
    ...
    paginator, page = paginate(request, poll_list, per_page=20)

    return render(
        request,
        "wagtailpolls/search.html",
        {
            "page": page,
            "paginator": paginator,
            "poll_list": poll_list,
...
def choose(request):
    ...
    if "q" in request.GET:
        search_form = AdminSearchForm(
            request.GET, placeholder=_("Search %(snippet_type_name)s") % {"snippet_type_name": "Polls"}
        )
    ...
    if request.GET.get("results", None) == "true":
        return render(
            request,
            "wagtailpolls/search_results.html",
            {
