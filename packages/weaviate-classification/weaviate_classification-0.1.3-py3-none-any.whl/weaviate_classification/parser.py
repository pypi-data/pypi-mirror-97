...
def _parse_data_line_excel(dataconfig, data, sheet, row, dataclass):
...
def _parse_file_excel(dataconfig, sheet):
    ...
    while not end:
        ...
        if source is None:
            ...
            for dataclass in dataconfig['classes']:
                point = _parse_data_line_excel(dataconfig, data, sheet, row, dataclass)
...
def _parse_data_line_csv(dataconfig, data, line, row, dataclass):
...
def _parse_file_csv(dataconfig, reader):
    ...
    for line in reader:
        if row == 0:
            ...
            for dataclass in dataconfig['classes']:
                point = _parse_data_line_csv(dataconfig, data, line, row, dataclass)
                data['datapoints'].append(point)
            data['count'] += 1
            row += 1
