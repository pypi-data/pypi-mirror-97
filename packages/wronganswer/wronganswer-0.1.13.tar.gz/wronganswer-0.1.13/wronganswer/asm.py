...
def relabel(code):
    ...
    pattern = re.compile(b"|".join(map(re.escape, sorted(labels, key=len, reverse=True))))

    count = dict.fromkeys(labels, 0)
    for m in pattern.finditer(code):
        count[m.group(0)] += 1

    labels = list(l for l in sorted(labels, key=lambda x: count[x], reverse=True))
    local_label_prefix = b"L"
    if any(l.startswith(b".L") for l in labels):
        local_label_prefix = b".L"

    label_map = {
        l: local_label_prefix+bytes(encode_int(n))
        for n, l in enumerate(filter(lambda l: l.startswith(local_label_prefix), labels))}

    label_map.update({
        l: b"Z"+bytes(encode_int(n))
        for n, l in enumerate(filter(lambda l: not l.startswith(local_label_prefix), labels))})

    return pattern.sub(lambda m: label_map[m.group(0)], code)
