...
class CrawledContent:
    ...
    def extract_tags(self, idx=-1, topK=3, POS=('n', 'a', 'v')):
        key = list(self.next.keys())[0]
        val = self.next[key]
        POS = list(POS)
        for k in range(len(val)):
            try:
                if 'n' in POS:
                    tags_n = ' '.join(jieba.analyse.extract_tags(val[k][idx], topK=topK,
                                                                 allowPOS=('n', 'nr', 'ns', 'nt', 'nz')))
                    self.next[key][k].append(' '.join(tags_n))
                if 'a' in POS:
                    tags_a = ' '.join(jieba.analyse.extract_tags(val[k][idx], topK=topK, allowPOS=('a', 'ad', 'an')))
                    self.next[key][k].append(' '.join(tags_a))
                if 'v' in POS:
                    tags_v = ' '.join(jieba.analyse.extract_tags(val[k][idx], topK=topK, allowPOS=('v', 'vd', 'vn')))
                    self.next[key][k].append(' '.join(tags_v))
